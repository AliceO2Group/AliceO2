...
inline bool parse_cmd_line(int _argc, char* _argv[], DeviceOptions* _options)
{
  ...
  desc.add_options()
    ...
    ("log-rttack-rate", bpo::value<int>(), "Log output rate on socket, 1/0")
...
int main(int argc, char** argv)
{
  ...
  epn.SetProperty(EPNex::OutputAddress, initialOutputAddress, options.numFLPs);
  ...
  ddsKeyValue.putValue("EPNDistributedInputAddress", epn.GetProperty(EPNex::InputAddress, "", 0));
  if (options.testMode == 0) {
    // In regular mode, advertise the bound data output address via DDS.
    ddsKeyValue.putValue("EPNOutputAddress", epn.GetProperty(EPNex::OutputAddress, "", options.numFLPs));
  }
  ...
  if (options.testMode == 1) {
    // In test mode, connect to next step intead of binding and
    // get the value of the FLPSyncSampler input address for feedback socket
