...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < options.numInputs; ++i) {
    ...
    // flp.SetProperty(FLPex::InputAddress, initialInputAddress, i);
    flp.SetProperty(FLPex::LogInputRate, options.logInputRate.at(i), i);
  }

  flp.SetProperty(FLPex::InputAddress, initialInputAddress, 0); // commands
  flp.SetProperty(FLPex::InputAddress, initialInputAddress, 1); // heartbeats
  if (options.testMode == 1) {
    // In test mode, assign address that was received from the FLPSyncSampler via DDS.
    flp.SetProperty(FLPex::InputAddress, values.begin()->second, 2); // FLPSyncSampler signal
  } else {
    // In normal mode, assign placeholder address, that will be set when binding.
    flp.SetProperty(FLPex::InputAddress, initialInputAddress, 2); // data
  }
  ...
  for (int i = 0; i < options.numOutputs; ++i) {
    ...
    flp.SetProperty(FLPex::OutputAddress, "tcp://127.0.0.1:123");
  ...
  // DDS
  if (options.testMode == 1) {
    // Nothing specific to be done after binding in test mode
  } else {
    // In regular mode, advertise the bound data input address to the DDS.
    ddsKeyValue.putValue("FLPInputAddress", flp.GetProperty(FLPex::InputAddress, "", 2));
  }
  ...
  ddsKeyValue.subscribe([&keyCondition](const string& /*_key*/, const string& /*_value*/) {keyCondition.notify_all();});
  ddsKeyValue.getValues("EPNDistributedInputAddress", &values2);
