...
//_______________________________________________________________________________
HistogramManager::HistogramManager(const char* name, const char* title, const int maxNVars) :
  TNamed(name,title),
  fMainList(),
  fNVars(maxNVars),
  fUsedVars(),
...
void HistogramManager::AddHistogram(const char* histClass, const char* hname, const char* title, bool isProfile,
  ...
  if(varX>kNothing) fUsedVars[varX] = kTRUE;
  if(varY>kNothing) fUsedVars[varY] = kTRUE;
  if(varZ>kNothing) fUsedVars[varZ] = kTRUE;
...
void HistogramManager::AddHistogram(const char* histClass, const char* hname, const char* title,
  ...
  THnBase* h=0x0;
  if (useSparse)  h=new THnSparseF(hname,(arr->At(0) ? arr->At(0)->GetName() : ""),nDimensions,nBins,xmin,xmax);
  else            h=new THnF(hname,(arr->At(0) ? arr->At(0)->GetName() : ""),nDimensions,nBins,xmin,xmax);
...
void HistogramManager::FillHistClass(const char* className, Float_t* values) {
  ...
  for(auto varIter=varList.begin(); varIter!=varList.end(); varIter++) {
    ...
    if(isTHn) {
      ...
      varX = varIter->at(3); varY = varIter->at(4); varZ = varIter->at(5);
      varT = varIter->at(6);
    }
