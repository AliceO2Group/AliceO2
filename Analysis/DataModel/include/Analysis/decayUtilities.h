...
{
  double en_ = std::sqrt(mass * mass + px * px + py * py + pz * pz);
  return en_;
}
...
double invmass2prongs(double px0, double py0, double pz0, double mass0,
                      double px1, double py1, double pz1, double mass1)
{
  return std::sqrt(invmass2prongs2(px0, py0, pz0, mass0,
                                   px1, py1, pz1, mass1));
}
...
double invmass3prongs(double px0, double py0, double pz0, double mass0,
                      ...
                      double px2, double py2, double pz2, double mass2)
{
  return std::sqrt(invmass3prongs2(px0, py0, pz0, mass0,
                                   px1, py1, pz1, mass1,
                                   px2, py2, pz2, mass2));
}
...
{
  return std::sqrt((px0 + px1) * (px0 + px1) + (py0 + py1) * (py0 + py1));
}
...
{
  return std::sqrt((xdecay - xvtx) * (xdecay - xvtx) + (ydecay - yvtx) * (ydecay - yvtx) + (zdecay - zvtx) * (zdecay - zvtx));
}

double declengthxy(double xdecay, double ydecay, double xvtx, double yvtx)
{
  return std::sqrt((xdecay - xvtx) * (xdecay - xvtx) + (ydecay - yvtx) * (ydecay - yvtx));
}
