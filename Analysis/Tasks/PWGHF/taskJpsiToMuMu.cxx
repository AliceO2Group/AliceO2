...
/// \file taskJpsiToMuMu.cxx
/// \brief JpsiToMuMu analysis task
///
...
/// jpsitomumu analysis task
struct TaskJpsiToMuMu {
  ...
  {
    registry.add("hMassVsPt", "2-prong candidates;inv. mass (#mu^{#plus} #mu^{#minus}) (GeV/#it{c}^{2});entries", {HistType::kTH2F, {{500, 0., 5.}, {(std::vector<double>)bins, "#it{p}_{T} (GeV/#it{c})"}}});
  ...
  Filter filterSelectCandidates = (aod::hf_selcandidate_jpsitomumu::isSelJpsiToMuMu >= d_selectionFlagJpsiToMuMu);

  void process(soa::Filtered<soa::Join<aod::HfCandProng2, aod::HFSelJpsiToMuMuCandidate>> const& candidates)
  //  void process(aod::HfCandProng2 const& candidates)
  {
    for (auto& candidate : candidates) {
      ...
      registry.fill(HIST("hMassVsPt"), InvMassJpsiToMuMu(candidate), candidate.pt());
...
struct TaskJpsiToMuMuMC {
  ...
  void process(soa::Filtered<soa::Join<aod::HfCandProng2, aod::HFSelJpsiToMuMuCandidate, aod::HfCandProng2MCRec>> const& candidates,
               soa::Join<aod::McParticles, aod::HfCandProng2MCGen> const& particlesMC, aod::BigTracksMC const& tracks)
  {
    ...
    for (auto& candidate : candidates) {
      ...
      if (candidate.flagMCMatchRec() == 1 << JpsiToMuMu) {
    ...
    for (auto& particle : particlesMC) {
      if (particle.flagMCMatchGen() == 1 << JpsiToMuMu) {
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  WorkflowSpec workflow{
    adaptAnalysisTask<TaskJpsiToMuMu>(cfgc, TaskName{"hf-task-jpsi-toMuMu"})};
  const bool doMC = cfgc.options().get<bool>("doMC");
  if (doMC) {
    workflow.push_back(adaptAnalysisTask<TaskJpsiToMuMuMC>(cfgc, TaskName{"hf-task-jpsi-toMuMu-mc"}));
  }
  return workflow;
}
