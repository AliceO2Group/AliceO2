...
struct TaskJpsiToEE {
  ...
  Filter filterSelectCandidates = (aod::hf_selcandidate_jpsitoee::isSelJpsiToEE >= d_selectionFlagJpsiToEE);
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  WorkflowSpec workflow{
    adaptAnalysisTask<TaskJpsiToEE>(cfgc, TaskName{"hf-task-jpsi-toee"})};
  const bool doMC = cfgc.options().get<bool>("doMC");
  if (doMC) {
    workflow.push_back(adaptAnalysisTask<TaskJpsiToEEMC>(cfgc, TaskName{"hf-task-jpsi-toee-mc"}));
  }
  return workflow;
}
