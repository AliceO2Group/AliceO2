...
struct HFCandidateCreator2ProngMC {
               ...
               aod::McParticles const& particlesMC)
  {
    ...
    for (auto& candidate : candidates) {
      ...
      indexRec = RecoDecay::getMatchedMCRec(particlesMC, arrayDaughters, kD0, array{+kPiPlus, -kKPlus}, true, &sign);
      if (indexRec > -1) {
        flag = sign * (1 << D0ToPiK);
      }
      ...
      if (flag == 0) {
        ...
        indexRec = RecoDecay::getMatchedMCRec(particlesMC, std::move(arrayDaughters), kJpsi, array{+kElectron, -kElectron}, true);
        if (indexRec > -1) {
          flag = 1 << JpsiToEE;
        }
      }
    ...
    for (auto& particle : particlesMC) {
      ...
      // D0(bar) → π± K∓
      //Printf("Checking D0(bar) → π± K∓");
      if (RecoDecay::isMatchedMCGen(particlesMC, particle, kD0, array{+kPiPlus, -kKPlus}, true, &sign)) {
        flag = sign * (1 << D0ToPiK);
      }
      ...
      if (flag == 0) {
        //Printf("Checking J/ψ → e+ e-");
        if (RecoDecay::isMatchedMCGen(particlesMC, particle, kJpsi, array{+kElectron, -kElectron}, true)) {
          flag = 1 << JpsiToEE;
        }
      }
