...
struct HfTagSelTracks {
  ...
  void init(InitContext const&)
  {
    ...
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(1, "2-prong selected");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(2, "2-prong rej pT");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(3, "2-prong rej eta");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(4, "2-prong rej track quality");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(5, "2-prong rej dca");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(6, "3-prong selected");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(7, "3-prong rej pT");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(8, "3-prong rej eta");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(9, "3-prong rej track quality");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(10, "3-prong rej dca");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(11, "bachelor selected");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(12, "bachelor rej pT");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(13, "bachelor rej eta");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(14, "bachelor rej track quality");
    registry.get<TH1>(HIST("hRejTracks"))->GetXaxis()->SetBinLabel(15, "bachelor rej dca");
  }
  ...
  )
  {
    ...
    for (auto& track : tracks) {
      ...
      int statusProng = BIT(Cand2Prong) | BIT(Cand3Prong) | BIT(CandV0bachelor); // selection flag , 3 bits on
      const int nCuts = 4;
      bool cutStatus[3][nCuts];
      if (debug.value) {
      ...
      // eta cut
      if ((debug.value || (statusProng & BIT(Cand2Prong))) && std::abs(trackEta) > etaMax2Prong) {
      ...
      if ((debug.value || (statusProng & BIT(Cand3Prong))) && std::abs(trackEta) > etaMax3Prong) {
        statusProng &= ~BIT(Cand3Prong);
        if (debug.value) {
      ...
      if ((debug.value || (statusProng & BIT(CandV0bachelor))) && std::abs(trackEta) > etaMaxBach) {
        statusProng &= ~BIT(CandV0bachelor);
        if (debug.value) {
      ...
      if (doCutQuality.value && (debug.value || statusProng > 0)) { // FIXME to make a more complete selection e.g track.flags() & o2::aod::track::TPCrefit && track.flags() & o2::aod::track::GoldenChi2 &&
      ...
      if ((debug.value || statusProng > 0)) {
        ...
        if ((debug.value || (statusProng & BIT(Cand3Prong))) && !isSelectedTrack(track, dca, Cand3Prong)) {
          statusProng &= ~BIT(Cand3Prong);
          if (debug.value) {
        ...
        if ((debug.value || (statusProng & BIT(CandV0bachelor))) && !isSelectedTrack(track, dca, CandV0bachelor)) {
          statusProng &= ~BIT(CandV0bachelor);
          if (debug.value) {
      ...
      if (fillHistograms.value) {
        if (statusProng & BIT(Cand2Prong)) {
          ...
          if (debug.value) {
            registry.get<TH1>(HIST("hRejTracks"))->Fill(1);
          }
        }
        if (statusProng & BIT(Cand3Prong)) {
          ...
          if (debug.value) {
            registry.get<TH1>(HIST("hRejTracks"))->Fill(6);
          }
        }
        if (statusProng & BIT(CandV0bachelor)) {
          ...
          if (debug.value) {
            registry.get<TH1>(HIST("hRejTracks"))->Fill(11);
          }
        }
      }
...
struct HfTrackIndexSkimsCreator {
  ...
  Configurable<double> pTTolerance{"pTTolerance", 0.1, "pT tollerance in GeV/c for applying preselections before vertex reconstruction"};
  ...
  HistogramRegistry registry{
    ...
    {{"hNTracks", ";# of tracks;entries", {HistType::kTH1F, {{2500, 0., 25000.}}}},
     ...
     {"hmassJpsiToEE", "J/#psi candidates;inv. mass (e^{+} e^{#minus}) (GeV/#it{c}^{2});entries", {HistType::kTH1F, {{500, 0., 5.}}}},
     // 3-prong histograms
  ...
  void is2ProngPreselected(const T1& hfTracks, T2& cutStatus, T3& whichHypo, int& isSelected)
  {
    ...
    for (int n2 = 0; n2 < n2ProngDecays; n2++) {
      ...
      double massHypos[2];
      whichHypo[n2] = 3;
      if ((debug || (isSelected & 1 << n2)) && cut2Prong[n2].get(pTBin, "massMin") >= 0. && cut2Prong[n2].get(pTBin, "massMax") > 0.) {
        massHypos[0] = RecoDecay::M(arrMom, arrMass2Prong[n2][0]);
        massHypos[1] = RecoDecay::M(arrMom, arrMass2Prong[n2][1]);
        if (massHypos[0] < cut2Prong[n2].get(pTBin, "massMin") || massHypos[0] >= cut2Prong[n2].get(pTBin, "massMax")) {
          whichHypo[n2] -= 1;
        }
  ...
  void is3ProngPreselected(const T1& hfTracks, T2& cutStatus, T3& whichHypo, int& isSelected)
  {
    ...
    for (int n3 = 0; n3 < n3ProngDecays; n3++) {
      ...
      double massHypos[2];
      whichHypo[n3] = 3;
      if ((debug || (isSelected & 1 << n3)) && cut3Prong[n3].get(pTBin, "massMin") >= 0. && cut3Prong[n3].get(pTBin, "massMax") > 0.) { //no need to check isSelected but to avoid mistakes
        massHypos[0] = RecoDecay::M(arrMom, arrMass3Prong[n3][0]);
        massHypos[1] = RecoDecay::M(arrMom, arrMass3Prong[n3][1]);
        if (massHypos[0] < cut3Prong[n3].get(pTBin, "massMin") || massHypos[0] >= cut3Prong[n3].get(pTBin, "massMax")) {
          whichHypo[n3] -= 1;
        }
  ...
  void is2ProngSelected(const T1& pVecCand, const T2& secVtx, const T3& primVtx, T4& cutStatus, int& isSelected)
  {
    if (debug || isSelected > 0) {
      for (int n2 = 0; n2 < n2ProngDecays; n2++) {
        ...
        // pT
        auto pTBin = findBin(pTBins2Prong[n2], RecoDecay::Pt(pVecCand));
        if (pTBin == -1) { // cut if it is outside the defined pT bins
          isSelected &= ~(1 << n2);
          if (debug.value) {
            cutStatus[n2][0] = false;
  ...
  void is3ProngSelected(const T1& pVecCand, const T2& secVtx, const T3& primVtx, T4& cutStatus, int& isSelected)
  {
    if (debug || isSelected > 0) {
      for (int n3 = 0; n3 < n3ProngDecays; n3++) {
        ...
        // pT
        auto pTBin = findBin(pTBins3Prong[n3], RecoDecay::Pt(pVecCand));
        if (pTBin == -1) { // cut if it is outside the defined pT bins
          isSelected &= ~(1 << n3);
          if (debug.value) {
            cutStatus[n3][0] = false;
        ...
        if ((debug || (isSelected & 1 << n3))) {
          ...
          if (decayLength < cut3Prong[n3].get("decL")) {
            isSelected &= ~(1 << n3);
            if (debug.value) {
              cutStatus[n3][3] = false;
            }
          }
        }
      }
    ...
    SelectedTracks const& tracks)
  {
    ...
    */
    ...
    for (auto trackPos1 = tracks.begin(); trackPos1 != tracks.end(); ++trackPos1) {
      ...
      bool sel2ProngStatusPos = trackPos1.isSelProng() & BIT(Cand2Prong);
      bool sel3ProngStatusPos1 = trackPos1.isSelProng() & BIT(Cand3Prong);
      ...
      for (auto trackNeg1 = tracks.begin(); trackNeg1 != tracks.end(); ++trackNeg1) {
        ...
        bool sel2ProngStatusNeg = trackNeg1.isSelProng() & BIT(Cand2Prong);
        bool sel3ProngStatusNeg1 = trackNeg1.isSelProng() & BIT(Cand3Prong);
              ...
              if (fillHistograms) {
                ...
                for (int n2 = 0; n2 < n2ProngDecays; n2++) {
                  if (isSelected2ProngCand & 1 << n2) {
                    if (whichHypo2Prong[n2] == 1 || whichHypo2Prong[n2] == 3) {
                      auto mass2Prong = RecoDecay::M(arrMom, arrMass2Prong[n2][0]);
                      if (n2 == hf_cand_prong2::DecayType::D0ToPiK) {
                        registry.get<TH1>(HIST("hmassD0ToPiK"))->Fill(mass2Prong);
                      } else if (n2 == hf_cand_prong2::DecayType::JpsiToEE) {
                        registry.get<TH1>(HIST("hmassJpsiToEE"))->Fill(mass2Prong);
                      }
                    }
                    if (whichHypo2Prong[n2] >= 2) {
                      auto mass2Prong = RecoDecay::M(arrMom, arrMass2Prong[n2][1]);
                      if (n2 == hf_cand_prong2::DecayType::D0ToPiK) {
                        registry.get<TH1>(HIST("hmassD0ToPiK"))->Fill(mass2Prong);
                      }
                    }
        ...
        if (do3prong == 1) {
          ...
          for (auto trackPos2 = trackPos1 + 1; trackPos2 != tracks.end(); ++trackPos2) {
            ...
            if (!(trackPos2.isSelProng() & BIT(Cand3Prong))) {
              continue;
            }
          ...
          for (auto trackNeg2 = trackNeg1 + 1; trackNeg2 != tracks.end(); ++trackNeg2) {
            ...
            if (!(trackNeg2.isSelProng() & BIT(Cand3Prong))) {
              continue;
            }
