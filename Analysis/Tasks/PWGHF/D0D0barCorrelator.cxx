...
struct D0D0barCorrelator {
  Produces<aod::D0D0barPair> entryD0D0barPair;
  Produces<aod::D0D0barRecoInfo> entryD0D0barRecoInfo;  
...
struct D0D0barCorrelatorMCRec {
  ...
  {
    //MC reco level
    bool flagD0Signal = kFALSE;
    bool flagD0barSignal = kFALSE;
    for (auto& candidate1 : candidates) {
      //check decay channel flag for candidate1
      if (!(candidate1.hfflag() & 1 << D0ToPiK)) {
        continue;
      ...
      if (candidate1.isSelD0() < dSelectionFlagD0) { //discard candidates not selected as D0 in outer loop
        continue;
      }
      if (candidate1.flagMCMatchRec() == 1 << D0ToPiK) { //candidate matched to D0 (particle)
        flagD0Signal = kTRUE;
      } else { //candidate of bkg, wrongly selected as D0
        flagD0Signal = kFALSE;
      }
      for (auto& candidate2 : candidates) {
        if (!(candidate2.hfflag() & 1 << D0ToPiK)) { //check decay channel flag for candidate2
          continue;
        }
        if (candidate2.isSelD0bar() < dSelectionFlagD0bar) { //discard candidates not selected as D0bar in inner loop
          continue;
        }
        if (candidate2.flagMCMatchRec() == -1 << D0ToPiK) { //candidate matched to D0bar (antiparticle)
          flagD0barSignal = kTRUE;
        } else { //candidate of bkg, wrongly selected as D0bar
          flagD0barSignal = kFALSE;
        }
        if (cutYCandMax >= 0. && std::abs(YD0(candidate2)) > cutYCandMax) {
          continue;
        }
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  ...
  if (!doLikeSign) { //unlike-sign analyses
    if (doMCGen) { //MC-Gen analysis
      workflow.push_back(adaptAnalysisTask<D0D0barCorrelatorMCGen>(cfgc));
    } else if (doMCRec) { //MC-Reco analysis
      workflow.push_back(adaptAnalysisTask<D0D0barCorrelatorMCRec>(cfgc));
    } else if (doMCccbar) { //MC-Reco analysis
      workflow.push_back(adaptAnalysisTask<CCbarCorrelatorMCGen>(cfgc));   
    } else { //data analysis
      workflow.push_back(adaptAnalysisTask<D0D0barCorrelator>(cfgc));
    }
  } else { //ike-sign analyses
    if (doMCGen) { //MC-Gen analysis
      workflow.push_back(adaptAnalysisTask<D0D0barCorrelatorMCGenLS>(cfgc));
    } else if (doMCRec) { //MC-Reco analysis
      workflow.push_back(adaptAnalysisTask<D0D0barCorrelatorMCRecLS>(cfgc));
    } else if (doMCccbar) { //MC-Reco analysis
      workflow.push_back(adaptAnalysisTask<CCbarCorrelatorMCGenLS>(cfgc));  
    } else { //data analysis
      workflow.push_back(adaptAnalysisTask<D0D0barCorrelatorLS>(cfgc));
    }
  }
