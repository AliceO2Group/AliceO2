...
///
/// \author Fabio Colamaria <Fabio.Colamaria@cern.ch> (Bari)
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  WorkflowSpec workflow{
    adaptAnalysisTask<HFDDbarTask>("hf-ddbar_corr")};
  const bool doMC = cfgc.options().get<bool>("doMC");
  if (doMC) {
    workflow.push_back(adaptAnalysisTask<HFDDbarTaskMC>("hf-ddbar_corr-mc"));
  }
  return workflow;
}
