...
struct TrackQATask {
  ...
  OutputObj<TH2F> dcaxyVsPt{
    TH2F("track-dcaXYvsPt", "DCA xy vs pT;dca-xy [cm];p_{T};p_{T} [GeV/c]", 200, -3., 3., 100, 0., 50.),
    OutputObjHandlingPolicy::QAObject};

  // track parameters
  OutputObj<TH1F> x{
    TH1F("trackpar-x", "track x position at dca in local coordinate system;x [cm]", 200, -0.4, 0.4),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> y{
    TH1F("trackpar-y", "track y position at dca in local coordinate system;y [cm]", 100, -4., 4.),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> z{
    TH1F("trackpar-z", "track z position at dca in local coordinate system;z [cm]", 100, -20., 20.),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> alpha{
    TH1F("trackpar-alpha", "rotation angle of local wrt. global coordinate system;#alpha [rad]", 100, -(M_PI + 0.01), (M_PI + 0.01)),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> signed1Pt{
    TH1F("trackpar-signed1Pt", "track signed 1/p_{T};q/p_{T}", 200, -8, 8),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> snp{
    TH1F("trackpar-snp", "sinus of track momentum azimuthal angle;snp", 100, -1., 1.),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> tgl{
    TH1F("trackpar-tgl", "tangent of the track momentum dip angle;tgl;", 1000, -2, 2),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> dcaxy{
    TH1F("track-dcaXY", "distance of closest approach in xy plane;dca-xy [cm];", 200, -3., 3.),
    OutputObjHandlingPolicy::QAObject};
  ...
  OutputObj<TH1F> itsFoundClusters{
    TH1F("its-foundClusters", "number of found ITS clusters;# clusters ITS", 8, -0.5, 7.5),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> itsChi2PerCluster{
    TH1F("its-chi2PerCluster", "chi2 per ITS cluster;chi2 / cluster ITS", 100, 0, 40),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> itsHits{
    TH1F("its-hits", "hitmap ITS;layer ITS", 7, -0.5, 6.5),
    OutputObjHandlingPolicy::QAObject};

  // TPC related quantities
  OutputObj<TH1F> tpcFindableClusters{
    TH1F("tpc-findableClusters", "number of findable TPC clusters;# clusters TPC", 165, -0.5, 164.5),
    OutputObjHandlingPolicy::QAObject};
  ...
  OutputObj<TH1F> tpcCrossedRows{
    TH1F("tpc-crossedRows", "number of crossed TPC rows;# crossed rows TPC", 165, -0.5, 164.5),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> tpcCrossedRowsOverFindableClusters{
    TH1F("tpc-crossedRowsOverFindableClusters", "crossed TPC rows over findable clusters;crossed rows / findable clusters TPC", 110, 0.0, 1.1),
    OutputObjHandlingPolicy::QAObject};

  OutputObj<TH1F> tpcChi2PerCluster{
    TH1F("tpc-chi2PerCluster", "chi2 per cluster in TPC;chi2 / cluster TPC", 100, 0, 10),
    OutputObjHandlingPolicy::QAObject};

  // collision related quantities
  OutputObj<TH2F> collisionPos{
    TH2F("collision-xy", "Position of collision;x [cm];y [cm]", 100, -0.5, 0.5, 100, -0.5, 0.5),
    OutputObjHandlingPolicy::QAObject};
  ...
  void process(aod::Collision const& collision, soa::Join<aod::FullTracks, aod::TracksExtended, aod::TrackSelection> const& tracks)
  {
    ...
    for (auto& track : tracks) {
      ...
      dcaxyVsPt->Fill(track.dcaXY(), track.pt());
