void checkTOFMatching(){
  ...
  for (int ientry = 0; ientry < matchTOF->GetEntries(); ientry++){
	...
	for (int ilabel = 0; ilabel < labelsITScheck.size(); ilabel++){
      ...
      if (!TPCfound) Printf("There is no TPC track found that should have corresponded to this TOF cluster!");
      if (!ITSfound) Printf("There is no ITS track found that should have corresponded to this TOF cluster!");
    }
