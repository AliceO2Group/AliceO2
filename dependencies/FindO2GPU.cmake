...
if(APPLE)
  # macOS ships OpenCL 1.2 only, below the 2.x the OpenCL backend needs; Metal
  # replaces it there. OFF rather than AUTO while the backend is unproven.
  if(NOT DEFINED ENABLE_METAL)
    set(ENABLE_METAL "OFF")
  endif()
elseif(NOT DEFINED ENABLE_OPENCL)
...
function(detect_gpu_arch backend) # Detect GPU architecture, optionally filterring by backend
  ...
  elseif(backend STREQUAL "ALL" OR backend STREQUAL "AUTO") # Return all / enabled backends
    ...
    if(METAL_ENABLED OR backend STREQUAL "ALL")
      list(APPEND TARGET_ARCH "METAL")
    endif()
...
if(GPUCA_DETERMINISTIC_NO_FTZ)
  ...
  set(GPUCA_METAL_DENORMALS_FLAGS "-fno-fast-math")
  ...
  set(GPUCA_METAL_DENORMALS_FLAGS "-ffast-math")
...
set(GPUCA_METAL_NO_FAST_MATH_FLAGS "-fno-fast-math")
...
if(ENABLE_CUDA)
  ...
  if (DEFINED ENV{O2_GPU_CUDA_HOME})
    set(CMAKE_CUDA_COMPILER "$ENV{O2_GPU_CUDA_HOME}/bin/nvcc")
...
if(ENABLE_HIP)
  ...
  if (DEFINED ENV{O2_GPU_ROCM_HOME})
    list(PREPEND CMAKE_PREFIX_PATH "$ENV{O2_GPU_ROCM_HOME}/lib/cmake")
  elseif (DEFINED ENV{ROCM_PATH})
    list(PREPEND CMAKE_PREFIX_PATH "$ENV{ROCM_PATH}/lib/cmake")
  elseif(NOT "$ENV{CMAKE_PREFIX_PATH}" MATCHES "rocm|ROCm" AND NOT CMAKE_PREFIX_PATH MATCHES "rocm|ROCm" AND EXISTS "/opt/rocm/lib/cmake/")
    list(APPEND CMAKE_PREFIX_PATH "/opt/rocm/lib/cmake")
  endif()
  # TODO: Use ROCm folder as provided by alidist-gpu recipe
  if("$ENV{CMAKE_PREFIX_PATH}" MATCHES "rocm|ROCm" OR CMAKE_PREFIX_PATH MATCHES "rocm|ROCm")
    if (DEFINED ENV{O2_GPU_ROCM_HOME})
      set(CMAKE_HIP_COMPILER "$ENV{O2_GPU_ROCM_HOME}/llvm/bin/clang++")
      set(TMP_ROCM_DIR "$ENV{O2_GPU_ROCM_HOME}")
    elseif (DEFINED ENV{ROCM_PATH})
      set(CMAKE_HIP_COMPILER "$ENV{ROCM_PATH}/llvm/bin/clang++")
      set(TMP_ROCM_DIR "$ENV{ROCM_PATH}")
    else()
      set(TMP_ROCM_DIR_LIST "${CMAKE_PREFIX_PATH}:$ENV{CMAKE_PREFIX_PATH}")
      string(REPLACE ":" ";" TMP_ROCM_DIR_LIST "${TMP_ROCM_DIR_LIST}")
      list(FILTER TMP_ROCM_DIR_LIST INCLUDE REGEX /rocm/lib/cmake|/ROCm/lib/cmake)
  ...
  elseif(NOT ENABLE_HIP STREQUAL "AUTO")
    message(FATAL_ERROR "HIP requested, but CMAKE_PREFIX_PATH (${CMAKE_PREFIX_PATH}) env variable does not contain rocm folder!")
  endif()
