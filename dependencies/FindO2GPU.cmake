...
string(
  TOUPPER "${ENABLE_CUDA}"
          ENABLE_CUDA)
string(
  TOUPPER "${ENABLE_OPENCL1}"
          ENABLE_OPENCL1)
string(
  TOUPPER "${ENABLE_OPENCL2}"
          ENABLE_OPENCL2)
string(
  TOUPPER "${ENABLE_HIP}"
          ENABLE_HIP)
...
if(ENABLE_CUDA)
  ...
  if(CUDA_GCCBIN)
    ...
    if(NOT
       CMAKE_CUDA_COMPILER)
      set(CMAKE_CUDA_COMPILER "nvcc") # check_language does not treat the HOST_COMPILER flag correctly, we force it and
                                      # will fail below if wrong.
    endif()
  endif()
  if(CMAKE_CUDA_COMPILER)
    ...
    string(
      ...
      REPLACE "\-std=[^ ]*"
              ""
              CMAKE_CXX_FLAGS_NOSTD
              ${CMAKE_CXX_FLAGS}) # Need to strip c++17 imposed by alidist defaults
    set(CMAKE_CUDA_FLAGS
        "${CMAKE_CUDA_FLAGS} -Xcompiler \"${CMAKE_CXX_FLAGS_NOSTD}\" --expt-relaxed-constexpr --extended-lambda --allow-unsupported-compiler -Xptxas -v"
    )
    set(CMAKE_CUDA_FLAGS_DEBUG
        "${CMAKE_CUDA_FLAGS_DEBUG} -lineinfo -Xcompiler \"${CMAKE_CXX_FLAGS_DEBUG}\" -Xptxas -O0 -Xcompiler -O0")
    if(NOT
       CMAKE_BUILD_TYPE
       STREQUAL
       "DEBUG")
      set(CMAKE_CUDA_FLAGS_${CMAKE_BUILD_TYPE}
          "${CMAKE_CUDA_FLAGS_${CMAKE_BUILD_TYPE}} -Xcompiler \"${CMAKE_CXX_FLAGS_${CMAKE_BUILD_TYPE}}\" -Xptxas -O4 -Xcompiler -O4 -use_fast_math --ftz=true"
      )
    endif()
    if(CMAKE_CXX_FLAGS
       MATCHES
...
if(ENABLE_OPENCL2)
  ...
  if(OPENCL2_COMPATIBLE_CLANG_FOUND
     AND (OpenCL_VERSION_STRING
          VERSION_GREATER_EQUAL
          2.2
  ...
  if(FORCE_OPENCL2_ALL
     AND NOT
         (OPENCL2_ENABLED_AMD AND OPENCL2_ENABLED_SPIRV))
    message(
      FATAL_ERROR
        "Not all OpenCL2 backends available, but requested (AMD ${OPENCL2_ENABLED_AMD} SPIRV ${OPENCL2_ENABLED_SPIRV})")
  endif()
endif()
...
if(ENABLE_HIP)
  ...
  if(EXISTS
    ...
    if(hip_FOUND
      ...
      set(O2_HIP_CMAKE_CXX_FLAGS
          "-fcuda-flush-denormals-to-zero -Wno-invalid-command-line-argument -Wno-unused-command-line-argument -Wno-invalid-constexpr -Wno-ignored-optimization-argument -Wno-unused-private-field"
      )
      if(HIP_AMDGPUTARGET)
