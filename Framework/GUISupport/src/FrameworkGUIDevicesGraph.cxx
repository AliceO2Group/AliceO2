                       ...
                       bool lightMode)
{
  ...
  out << R"(<svg xmlns="http://www.w3.org/2000/svg" width=")" << canvasSize.x << "\" height=\"" << canvasSize.y
  ...
  out << "    </marker>\n";
  out << "  </defs>\n";
  out << R"(  <rect width="100%" height="100%" fill=")" << (lightMode ? "#fafafc" : "#2c2c2e") << "\"/>\n";
  ...
  for (int i = 0; i < nodes.Size; ++i) {
    ...
    out << "      <rect x=\"" << p.x << "\" y=\"" << p.y << "\" width=\"" << node.Size.x
        << "\" height=\"" << node.Size.y << R"(" rx="4" fill=")" << colorToHex(colors.normal)
        << "\" stroke=\"#646464\" stroke-width=\"4\"/>\n";
    out << "      <rect x=\"" << p.x << "\" y=\"" << p.y << "\" width=\"" << node.Size.x
        << R"(" height="24" rx="4" fill=")" << colorToHex(titleColor) << "\"/>\n";
                       ...
                       bool lightMode)
{
  ...
  for (int i = 0; i < nodes.Size; ++i) {
    ...
    out << "  n" << i << " [label=\"" << dotEscape(nodes[i].Name)
        ...
        << R"(", fontcolor="white", pos=")" << positions[i].pos.x / 72.f << "," << -positions[i].pos.y / 72.f << "!\"];\n";
  }
  for (int i = 0; i < links.Size; ++i) {
