...
BOOST_AUTO_TEST_CASE(TestInputRecord)
{
  ...
  // bunch of exceptions.
  InputRecord emptyRecord(schema, {[](std::size_t) { return DataRef{nullptr, nullptr, nullptr}; }, 0});
  ...
  createMessage(dh2, 2);
  createEmpty();
  InputSpan span{[&inputs](std::size_t i) { return DataRef{nullptr, static_cast<char const*>(inputs[2 * i]), static_cast<char const*>(inputs[2 * i + 1])}; }, inputs.size() / 2};
