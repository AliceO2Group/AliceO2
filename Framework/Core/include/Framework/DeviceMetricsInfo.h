...
struct DeviceMetricsHelper {
  ...
  template <typename T>
  static std::function<void(DeviceMetricsInfo&, T value)>
                        ...
                        NewMetricCallback newMetricsCallback = nullptr)
  {
    ...
    auto lastChar = strnlen(name, (ptrdiff_t)MetricLabelIndex::MAX_METRIC_LABEL_SIZE - 1);
    memcpy(metricLabelIdx.label, name, lastChar);
    ...
    return [metricIndex](DeviceMetricsInfo& metrics, T value) {
      MetricInfo& metric = metrics.metrics[metricIndex];
      metrics.changed.at(metricIndex) = true;
      auto& store = getMetricsStore<T>(metrics);
      store[metric.storeIdx][metric.pos++ % store[metric.storeIdx].size()] = value;
    };
  }
};
