...
struct DatumSpec {
  ...
  using datum_t = std::variant<std::monostate, std::size_t, LiteralNode::var_t, std::string>;
  datum_t datum;
  atype::type type = atype::NA;
  explicit DatumSpec(std::size_t index, atype::type type_) : datum{index}, type{type_} {}
