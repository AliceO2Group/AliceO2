...
{
  LOGP(info, "Updating state {} with {}", cmd.id, std::string_view(cmd.data, cmd.size));
  ...
  assert(size < 1000);
  if (size > 1024) {
    throw runtime_error_f("State size is %d. States larger than 1024 bytes not supported for now.", size);
  }
...
void DataProcessingStates::flushChangedStates(std::function<void(std::string const&, int64_t, std::string_view)> const& callback)
{
  ...
  for (size_t mi = 0; mi < updated.size(); ++mi) {
    ...
    if (currentTimestamp - update.lastPublished < spec.minPublishInterval) {
      LOGP(info, "not publishing because of minPublishInterval");
      continue;
    }
    publish = true;
    assert(view.first + view.size <= statesBuffer.size());
    assert(view.first <= statesBuffer.size());
    LOGP(info, "publishing invoked {} {}", view.first, view.size);
