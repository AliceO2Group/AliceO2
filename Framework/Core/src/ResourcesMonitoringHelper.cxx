...
bool ResourcesMonitoringHelper::dumpMetricsToJSON(const std::vector<DeviceMetricsInfo>& metrics) noexcept
{
    ...
    for (const auto& deviceMetrics : metrics)
    {
        ...
        for(const auto& metricLabel : deviceMetrics.metricLabelsIdx)
        {
            ...
            if(std::find(std::begin(performanceMetrics), std::end(performanceMetrics), labelStr) 
                == std::end(performanceMetrics))
            { continue; }
    ...
    if(file.is_open()) { boost::property_tree::json_parser::write_json(file, root); }
    else { return false; }
