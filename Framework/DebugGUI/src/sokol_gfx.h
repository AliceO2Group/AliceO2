...
_SOKOL_PRIVATE void _sg_d3d11_clear_state()
{
  ...
  ID3D11DeviceContext_OMSetRenderTargets(_sg_d3d11.ctx, SG_MAX_COLOR_ATTACHMENTS, _sg_d3d11.zero_rtvs, nullptr);
  ID3D11DeviceContext_RSSetState(_sg_d3d11.ctx, nullptr);
  ID3D11DeviceContext_OMSetDepthStencilState(_sg_d3d11.ctx, nullptr, 0);
  ID3D11DeviceContext_OMSetBlendState(_sg_d3d11.ctx, nullptr, nullptr, 0xFFFFFFFF);
  ...
  ID3D11DeviceContext_IASetIndexBuffer(_sg_d3d11.ctx, nullptr, DXGI_FORMAT_UNKNOWN, 0);
  ID3D11DeviceContext_IASetInputLayout(_sg_d3d11.ctx, nullptr);
  ID3D11DeviceContext_VSSetShader(_sg_d3d11.ctx, nullptr, nullptr, 0);
  ID3D11DeviceContext_PSSetShader(_sg_d3d11.ctx, nullptr, nullptr, 0);
...
_SOKOL_PRIVATE void _sg_create_image(_sg_image* img, const sg_image_desc* desc)
{
  ...
  if (_sg_is_valid_rendertarget_depth_format(img->pixel_format)) {
    ...
    hr = ID3D11Device_CreateTexture2D(_sg_d3d11.dev, &d3d11_desc, nullptr, &img->d3d11_texds);
    ...
    if (img->type != SG_IMAGETYPE_3D) {
      ...
      if (img->sample_count > 1) {
        ...
        hr = ID3D11Device_CreateTexture2D(_sg_d3d11.dev, &d3d11_tex_desc, nullptr, &img->d3d11_texmsaa);
...
_SOKOL_PRIVATE void _sg_create_shader(_sg_shader* shd, const sg_shader_desc* desc)
{
  ...
  for (int stage_index = 0; stage_index < SG_NUM_SHADER_STAGES; stage_index++) {
    ...
    for (int ub_index = 0; ub_index < SG_MAX_SHADERSTAGE_UBS; ub_index++) {
      ...
      hr = ID3D11Device_CreateBuffer(_sg_d3d11.dev, &cb_desc, nullptr, &stage->d3d11_cbs[ub_index]);
...
/* compile shader code */
  ...
  if (vs_ptr && fs_ptr && (vs_length > 0) && (fs_length > 0)) {
    ...
    hr = ID3D11Device_CreateVertexShader(_sg_d3d11.dev, vs_ptr, vs_length, nullptr, &shd->d3d11_vs);
    SOKOL_ASSERT(SUCCEEDED(hr) && shd->d3d11_vs);
    hr = ID3D11Device_CreatePixelShader(_sg_d3d11.dev, fs_ptr, fs_length, nullptr, &shd->d3d11_fs);
  ...
  _sg_image** fs_imgs, int num_fs_imgs)
{
  ...
  ID3D11DeviceContext_VSSetShader(_sg_d3d11.ctx, pip->shader->d3d11_vs, nullptr, 0);
  ...
  ID3D11DeviceContext_PSSetShader(_sg_d3d11.ctx, pip->shader->d3d11_fs, nullptr, 0);
...
_SOKOL_PRIVATE void _sg_apply_uniform_block(sg_shader_stage stage_index, int ub_index, const void* data, int num_bytes)
{
  ...
  SOKOL_ASSERT(cb);
  ID3D11DeviceContext_UpdateSubresource(_sg_d3d11.ctx, (ID3D11Resource*)cb, 0, nullptr, data, 0, 0);
}
...
_SOKOL_PRIVATE MTLPixelFormat _sg_mtl_rendertarget_color_format(sg_pixel_format fmt)
{
  switch (fmt) {
    case SG_PIXELFORMAT_RGBA8:
      return MTLPixelFormatBGRA8Unorm; /* not a bug */
    case SG_PIXELFORMAT_RGBA32F:
      return MTLPixelFormatRGBA32Float;
    case SG_PIXELFORMAT_RGBA16F:
      return MTLPixelFormatRGBA16Float;
...
{
  SOKOL_FREE(_sg_mtl_release_queue);
  _sg_mtl_release_queue = 0;
  SOKOL_FREE(_sg_mtl_free_queue);
  _sg_mtl_free_queue = 0;
  _sg_mtl_pool = nil;
}
...
_SOKOL_PRIVATE void _sg_mtl_destroy_sampler_cache(uint32_t frame_index)
{
  ...
  SOKOL_FREE(_sg_mtl_sampler_cache);
  _sg_mtl_sampler_cache = 0;
  _sg_mtl_sampler_cache_size = 0;
...
{
  NSError* err = nullptr;
  dispatch_data_t lib_data = dispatch_data_create(ptr, num_bytes, nullptr, DISPATCH_DATA_DESTRUCTOR_DEFAULT);
...
_SOKOL_PRIVATE const char* _sg_validate_string(_sg_validate_error err)
{
  switch (err) {
    ...
    case _SG_VALIDATE_PIPELINEDESC_ATTR_SEMANTICS:
      return "D3D11 vertex layouts must have attribute semantics (sem_name and sem_index)";
    ...
    /* sg_apply_draw_state */
    case _SG_VALIDATE_ADS_PIP:
      return "sg_apply_draw_state: pipeline object required";
    case _SG_VALIDATE_ADS_VBS:
      return "sg_apply_draw_state: number of vertex buffers doesn't match number of pipeline vertex layouts";
    case _SG_VALIDATE_ADS_VB_TYPE:
      return "sg_apply_draw_state: buffer in vertex buffer slot is not a SG_BUFFERTYPE_VERTEXBUFFER";
    case _SG_VALIDATE_ADS_NO_IB:
      return "sg_apply_draw_state: pipeline object defines indexed rendering, but no index buffer provided";
    case _SG_VALIDATE_ADS_IB:
      return "sg_apply_draw_state: pipeline object defines non-indexed rendering, but index buffer provided";
    case _SG_VALIDATE_ADS_IB_TYPE:
      return "sg_apply_draw_state: buffer in index buffer slot is not a SG_BUFFERTYPE_INDEXBUFFER";
    case _SG_VALIDATE_ADS_VS_IMGS:
      return "sg_apply_draw_state: vertex shader image count doesn't match sg_shader_desc";
    case _SG_VALIDATE_ADS_VS_IMG_TYPES:
      return "sg_apply_draw_state: one or more vertex shader image types don't match sg_shader_desc";
    case _SG_VALIDATE_ADS_FS_IMGS:
      return "sg_apply_draw_state: fragment shader image count doesn't match sg_shader_desc";
    case _SG_VALIDATE_ADS_FS_IMG_TYPES:
      return "sg_apply_draw_state: one or more fragment shader image types don't match sg_shader_desc";
    case _SG_VALIDATE_ADS_ATT_COUNT:
      return "sg_apply_draw_state: color_attachment_count in pipeline doesn't match number of pass color attachments";
    case _SG_VALIDATE_ADS_COLOR_FORMAT:
      return "sg_apply_draw_state: color_format in pipeline doesn't match pass color attachment pixel format";
    case _SG_VALIDATE_ADS_DEPTH_FORMAT:
      return "sg_apply_draw_state: depth_format in pipeline doesn't match pass depth attachment pixel format";
    case _SG_VALIDATE_ADS_SAMPLE_COUNT:
      return "sg_apply_draw_state: MSAA sample count in pipeline doesn't match render pass attachment sample count";

    /* sg_apply_uniform_block */
    case _SG_VALIDATE_AUB_NO_PIPELINE:
      return "sg_apply_uniform_block: must be called after sg_apply_draw_state()";
    case _SG_VALIDATE_AUB_NO_UB_AT_SLOT:
      return "sg_apply_uniform_block: no uniform block declaration at this shader stage UB slot";
    case _SG_VALIDATE_AUB_SIZE:
      return "sg_apply_uniform_block: data size exceeds declared uniform block size";

    /* sg_update_buffer */
    case _SG_VALIDATE_UPDBUF_USAGE:
      return "sg_update_buffer: cannot update immutable buffer";
    case _SG_VALIDATE_UPDBUF_SIZE:
      return "sg_update_buffer: update size is bigger than buffer size";
    case _SG_VALIDATE_UPDBUF_ONCE:
