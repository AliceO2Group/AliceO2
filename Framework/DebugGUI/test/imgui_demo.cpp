...
void ImGui::ShowDemoWindow(bool* p_open, bool def_open)
{
  ...
  if (ImGui::CollapsingHeader("Widgets")) {
    ...
    if (ImGui::TreeNode("Data Types")) {
      ...
      ImGui::DragScalar("drag double", ImGuiDataType_Double, &f64_v, 0.0005f, &f64_zero, nullptr, "%.10f grams", 1.0f);
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
  ...
  ImGui::ColorEdit3("Color", &col.x);
  {
    ...
    for (int n = 0; n < 2; n++) {
      ...
      x += sz + spacing;
      draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 0.0f, ImDrawCornerFlags_All, curr_thickness);
      x += sz + spacing;
      draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 10.0f, ImDrawCornerFlags_All, curr_thickness);
      x += sz + spacing;
      draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 10.0f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotRight, curr_thickness);
      x += sz + spacing;
      draw_list->AddTriangle(ImVec2(x + sz * 0.5f, y), ImVec2(x + sz, y + sz - 0.5f), ImVec2(x, y + sz - 0.5f), col32, curr_thickness);
      x += sz + spacing;
    ...
    x += sz + spacing;
    draw_list->AddRectFilled(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 10.0f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotRight);
    x += sz + spacing;
