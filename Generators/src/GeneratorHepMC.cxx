...
GeneratorHepMC::GeneratorHepMC()
  : Generator("ALICEo2", "ALICEo2 HepMC Generator"), mStream(), mFileName(), mVersion(3), mReader(NULL), mEvent(NULL)
{
  /** default constructor **/
}
...
GeneratorHepMC::GeneratorHepMC(const Char_t* name, const Char_t* title)
  : Generator(name, title), mStream(), mFileName(), mVersion(3), mReader(NULL), mEvent(NULL)
{
  /** constructor **/
}
...
Bool_t GeneratorHepMC::triggerFired(Trigger* trigger) const
{
  /** trigger fired **/
...
Bool_t GeneratorHepMC::addHeader(PrimaryGenerator* primGen) const
{
  /** add header **/
  ...
  if (cs && cs->is_valid()) {
    ...
    mHeader->removeCrossSectionInfo();
