...
#
# o2_add_test(testName SOURCES ...) adds a test. The test itself (in the CTest sense) is a wrapper around an executable.
# Both the test wrapper and the executable are setup by this function.
#
# If BUILD_TESTING if not set this function does nothing at all.
#
# The test name is the name of the first source file in SOURCES, unless the NAME parameter is given (see below).
#
# This function accepts two sets of parameters : one for the executable and one for the test wrapper.
#
# Parameters of the test executable :
#
# * NO_BOOST_TEST :  we assume most of the tests are using the Boost::Test framework and thus link the test with the
#   Boost::unit_test_framework target. If the test is known to not depend on Boost::Test, then the NO_BOOST_TEST option
#   can be given to forego this dependency
# * INSTALL : by default tests are _not_ installed. If that option is present then the test is installed (under
#   ${CMAKE_INSTALL_PREFIX}/tests, not in ${CMAKE_INSTALL_PREFIX}/bin like other binaries)
#
# Parameters of the test wrapper :
#
# * NAME: if given, will be used verbatim as the test name
# * MAX_ATTEMPTS : the number of time the test will be tried (upon failures) before it is actually considered as failed
# * TIMEOUT : the number of seconds allowed for the test to run. Past this time failure is assumed.
