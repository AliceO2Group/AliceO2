...
configure_file(
  ${CMAKE_CURRENT_LIST_DIR}/rootcling_wrapper.sh.in
...
#
# * HEADERS (required) is a list of relative filepaths needed for the dictionary definition
#
# * LINKDEF (required) is a single relative filepath to the LINKDEF file needed by rootcling.
#
# LINKDEF and HEADERS must contain relative paths only (relative to the CMakeLists.txt that calls this
# add_root_dictionary function).
#
...
function(
  ...
  foreach(h ${headers})
    ...
    if(NOT
       EXISTS
       ${f})
      message(FATAL_ERROR "add_root_dictionary was given an inexistant input include ${f}")
    endif()
  endforeach()
  ...
  set(O2_TARGETPCMMAP_TARGET
      "${O2_TARGETPCMMAP_TARGET};${target}"
      CACHE INTERNAL
            "target/PCM map (target)")
  set(O2_TARGETPCMMAP_PCM
      "${O2_TARGETPCMMAP_PCM};${pcmFile}"
      CACHE INTERNAL
            "target/PCM map (pcm)")
  ...
  if(NOT
     ...
     IN_LIST
     libs)
    # add ROOT::IO if not already there as a target that has a Root dictionary has to depend on ... Root
    target_link_libraries(${target} PUBLIC ROOT::RIO)
  endif()

  # Get the list of include directories that will be required to compile the dictionary itself and add them as private
  # include directories
  foreach(
    h
    IN
