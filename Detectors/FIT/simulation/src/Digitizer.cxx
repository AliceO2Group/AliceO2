...
void Digitizer::process(const std::vector<HitType>* hits,std::vector<Digit>* digits){
  ...
  for (auto& hit : *hits) {
    ...
    timeframe =
      Int_t((mEventTime + hit.GetTime()) ); // to be replaced with uncalibrated time
    mcp = hit.GetDetectorID();
    if ( (mcp<96 &&
	 hit.GetTime()>lowTimeA && hit.GetTime()<highTimeA) ||
	 (mcp>95 &&
	  hit.GetTime()>lowTimeC && hit.GetTime()<highTimeC) )
      {
	cfd[mcp] += hit.GetTime();
	amp[mcp]++;
      }
