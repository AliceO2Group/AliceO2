...
void Digits2Raw::readDigits(const char* fileRaw, const char* fileDigitsName)
{
  ...
  for (Int_t iev = 0; iev < nevD; iev++) {
    ...
    for (const auto& digit : *digArr) {
      std::cout << " #### convert digit " << std::endl;
...
void Digits2Raw::convertDigits(const o2::ft0::Digit& digit, const o2::ft0::LookUpTable& lut)
{
  ...
  for (auto& d : mTimeAmp) {
    ...
    //  std::cout << "@@@@ packed GBT "<<nlink<<" channelID   " << mEventData[nchannels].channelID << " charge " << mEventData[nchannels].charge << " time " << mEventData[nchannels].time << " chain "<<chain<<std::endl;
    //   std::cout << "@@@@ digits channelID   " << d.ChId  << " charge " << d.QTCAmpl << " time " << d.CFDTime <<  " chain "<<chain<< std::endl;
    nchannels++;
  }
  flushEvent(oldlink, mIntRecord, nchannels);
}
...
void Digits2Raw::flushEvent(int link, o2::InteractionRecord const& mIntRecord, uint nchannels)
{
  ...
  // fwrite(&mEventHeader, sizeof(mEventHeader), 1, mFileDest);
  // fwrite(&mEventData, sizeof(mEventData), mEventHeader.Nchannels, mFileDest);
  mFileDest.write(reinterpret_cast<char*>(&mEventHeader), sizeof(mEventHeader));
  //mFileDest.write(&mEventHeader, sizeof(mEventHeader));
  mFileDest.write(reinterpret_cast<char*>(&mEventData), sizeof(mEventData));
  //    mFileDest.write(&mEventData, sizeof(mEventData));
}
