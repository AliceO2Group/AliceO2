...
//
//file DigitBlockFT0.h class  for processing RAW data into Digits
//
// Artur.Furs
// afurs@cern.ch
// TODO:
...
//  static int sEventID;
  ...
  void process(DataBlockType& dataBlock, int linkID)
  {
   ...
   /*
    ...
    else if constexpr (std::is_same<DataBlockType, DataBlockTCMext>::value) { //Filling data from TCM, extended mode. Same process as for normal mode, for now.
      dataBlock.DataBlockWrapper<RawDataTCM>::mData[0].fillTrigger(mDigit.mTriggers);
    }
    */
  }
