...
void RecPointReader::run(ProcessingContext& pc)
{
  ...
  static const std::vector<o2::fv0::RecPoints> noRecPoints;
  static const std::vector<o2::fv0::ChannelDataFloat> noChannelData;
  const auto& recPoints = noEntry ? noRecPoints : *mRecPoints;
  const auto& channelData = noEntry ? noChannelData : *mChannelData;
  LOG(debug) << "FV0 RecPointReader pushes " << recPoints.size() << " recpoints with " << channelData.size() << " channels at entry " << ent;
