...
DataProcessorSpec getTRKDigitWriterSpec(bool mctruth, bool dec, bool calib)
{
  ...
  auto rofSizeGetter = [rofSizes](std::vector<o2::trkft3::ROFRecord> const& inROFs, DataRef const& ref) {
    auto const* dh = DataRefUtils::getHeader<o2::header::DataHeader*>(ref);
  ...
  return MakeRootTreeWriterSpec(("TRKDigitWriter" + std::string(dec ? "_dec" : "")).c_str(),
                                ...
                                BranchDefinition<std::vector<trkft3::Digit>>{vecInpSpecDig,
                                                                             detStr + "Digit", "digit-branch",
                                                                             mLayers,
                                                                             digitSizeGetter,
                                                                             getIndex,
                                                                             getName},
                                BranchDefinition<std::vector<trkft3::ROFRecord>>{vecInpSpecROF,
