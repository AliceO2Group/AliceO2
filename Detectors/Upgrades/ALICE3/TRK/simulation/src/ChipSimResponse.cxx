...
/// \file ChipSimResponse.cxx
/// \brief Definition of the TRK chip simulated response parametrization. Based on ITS2 and ITS3 codes.
...
void ChipSimResponse::initData(int tableNumber, std::string dataPath, const bool quiet)
{
  ...
  mDptMax = -2.e9;
  mDptMin = 2.e9;
  const int npix = ChipRespSimMat::getNPix();
  ...
  for (int ix = 0; ix < mNBinCol; ix++) {
    for (int iy = 0; iy < mNBinRow; iy++) {
      ...
      for (int iz = 0; iz < nz; iz++) {
        ...
        std::array<float, ChipRespSimMat::MatSize>* arr = mat.getArray();
...
//-----------------------------------------------------
void ChipSimResponse::print() const
{
  /*
   * print itself
   */
  printf("Chip response object of %zu matrices to map chagre in xyz to %dx%d pixels\n",
         mData.size(), getNPix(), getNPix());
  printf("VD X(col) range: %+e : %+e | step: %e | Nbins: %d\n", 0.f, mColMaxVD, 1.f / mStepInvCol, mNBinCol);
  printf("ML X(col) range: %+e : %+e | step: %e | Nbins: %d\n", 0.f, mColMaxML, 1.f / mStepInvCol, mNBinCol);
  printf("VD Y(row) range: %+e : %+e | step: %e | Nbins: %d\n", 0.f, mRowMaxVD, 1.f / mStepInvRow, mNBinRow);
  printf("ML Y(row) range: %+e : %+e | step: %e | Nbins: %d\n", 0.f, mRowMaxML, 1.f / mStepInvRow, mNBinRow);
  printf("Z(dpt) range: %+e : %+e | step: %e | Nbins: %d\n", mDptMin, mDptMax, 1.f / mStepInvDpt, mNBinDpt);
}

//-----------------------------------------------------
string ChipSimResponse::composeDataName(int colBin, int rowBin)
{
  /*
...
bool ChipSimResponse::getResponse(float vRow, float vCol, float vDepth, ChipRespSimMat& dest, const int layer) const
{
  /*
...
//____________________________________________________________
const ChipRespSimMat* ChipSimResponse::getResponse(float vRow, float vCol, float vDepth, bool& flipRow, bool& flipCol, const int subDetID, const int nLayer) const
{
  if (subDetID == 0) {
    return getResponse(vRow, vCol, vDepth, flipRow, flipCol, mRowMaxVD, mColMaxVD);
  } else if (subDetID == 1 && nLayer <= 3) { // ML
    return getResponse(vRow, vCol, vDepth, flipRow, flipCol, mRowMaxML, mColMaxML);
  } else if (subDetID == 1 && nLayer >= 4) { // OT
    return getResponse(vRow, vCol, vDepth, flipRow, flipCol, mRowMaxOT, mColMaxOT);
  } else {
    LOG(fatal) << "Unknown subdetector ID " << subDetID << " or layer " << nLayer;
    return nullptr;
  }
}

//____________________________________________________________
const ChipRespSimMat* ChipSimResponse::getResponse(float vRow, float vCol, float vDepth, bool& flipRow, bool& flipCol, float rowMax, float colMax) const
{
  /*
   * get linearized NPix*NPix matrix for response at point vRow(sensor local X, along row)
   * vCol(sensor local Z, along columns) and vDepth (sensor local Y, i.e. depth)
...
void ChipRespSimMat::print(bool flipRow, bool flipCol) const
{
  /*
