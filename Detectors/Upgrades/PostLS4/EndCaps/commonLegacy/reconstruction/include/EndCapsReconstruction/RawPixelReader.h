...
#ifndef ALICEO2_ENDCAPS_RAWPIXELREADER_H_
#define ALICEO2_ENDCAPS_RAWPIXELREADER_H_
...
#include "EndCapsReconstruction/PayLoadSG.h"
...
class RawPixelReader : public PixelReader
{
 ...
 public:
  ...
  {
    mPadding128 = v;
    mGBTWordSize = mPadding128 ? o2::endcaps::GBTPaddedWordLength : o2::endcaps::GBTWordLength;
  }
  ...
  void convertChip(o2::endcaps::ChipPixelData& chipData)
  {
  ...
  int fillGBTLinks()
  {
    ...
    constexpr uint8_t zero16[o2::endcaps::GBTPaddedWordLength] = {0}; // to speedup padding
    ...
    int maxGBTWordsPerPacket = (MaxGBTPacketBytes - RDHUtils::getHeaderSize(rdh)) / o2::endcaps::GBTPaddedWordLength - 2;
    ...
    for (int il = 0; il < RUDecodeData::MaxLinksPerRU; il++) {
      ...
      int loadsize = RDHUtils::getHeaderSize(rdh) + (nGBTWordsNeeded + 2) * o2::endcaps::GBTPaddedWordLength; // total data to dump
      ...
      o2::endcaps::GBTDataHeader gbtHeader(0, link->lanes);
      o2::endcaps::GBTDataTrailer gbtTrailer; // lanes will be set on closing the last page
      ...
      do {
        ...
        if (nGBTWordsNeeded && nGBTWordsInPacket >= maxGBTWordsPerPacket) {
          ...
          loadsize = RDHUtils::getHeaderSize(rdh) + (nGBTWordsNeeded + 2) * o2::endcaps::GBTPaddedWordLength; // update remaining size
          RDHUtils::setMemorySize(rdh, loadsize < MaxGBTPacketBytes ? loadsize : MaxGBTPacketBytes);
          RDHUtils::setOffsetToNext(rdh, mImposeMaxPage ? MaxGBTPacketBytes : RDHUtils::getMemorySize(rdh));
          link->data.ensureFreeCapacity(MaxGBTPacketBytes);
          link->data.addFast(reinterpret_cast<uint8_t*>(&rdh), RDHUtils::getHeaderSize(rdh)); // write RDH for current packet
  ...
  bool findNextRDH(PayLoadCont& buffer)
  {
    ...
    do {
      ...
      scan++;
      ptr += o2::endcaps::GBTPaddedWordLength;
      buffer.setPtr(ptr);
      if (!buffer.isEmpty()) {
        rdh = reinterpret_cast<o2::header::RAWDataHeader*>(ptr);
      } else {
  ...
  uint8_t* decodeRUData(uint8_t* raw, RUDecodeData& ruDecData, bool& aborted)
  {
    ...
    if (!RDHUtils::checkRDH(rdh)) {
      ...
      RDHUtils::printRDH(rdh);
      for (int i = 0; i < 4; i++) {
        auto gbtD = reinterpret_cast<const o2::endcaps::GBTData*>(raw + i * 16);
    ...
    while (1) {
      ...
      auto gbtH = reinterpret_cast<const o2::endcaps::GBTDataHeader*>(raw);    // process GBT header
      ...
      for (int iw = 0; iw < nGBTWords; iw++, raw += mGBTWordSize) {
        auto gbtD = reinterpret_cast<const o2::endcaps::GBTData*>(raw);
      ...
      auto gbtT = reinterpret_cast<const o2::endcaps::GBTDataTrailer*>(raw); // process GBT trailer
#ifdef _RAW_READER_ERROR_CHECKS_
...
//    }
  ...
  uint8_t* skimPaddedRUData(uint8_t* raw, PayLoadCont& outBuffer, bool& aborted)
  {
    ...
    while (1) {
      ...
      int nGBTWords = (RDHUtils::getMemorySize(rdh) - RDHUtils::getHeaderSize(rdh)) / o2::endcaps::GBTPaddedWordLength - 2;
      auto gbtH = reinterpret_cast<const o2::endcaps::GBTDataHeader*>(raw); // process GBT header
      ...
      raw += o2::endcaps::GBTPaddedWordLength;
      for (int iw = 0; iw < nGBTWords; iw++, raw += o2::endcaps::GBTPaddedWordLength) {
        auto gbtD = reinterpret_cast<const o2::endcaps::GBTData*>(raw);
        // TODO: need to clarify if the nGBTWords from the RDHUtils::getMemorySize(rdh) is reliable estimate of the real payload, at the moment this is not the case
...
//    }
 ...
 private:
  ...
  int mGBTWordSize = mPadding128 ? o2::endcaps::GBTPaddedWordLength : o2::endcaps::GBTWordLength;
...
#endif /* ALICEO2_ENDCAPSLAYERS_RAWPIXELREADER_H */
