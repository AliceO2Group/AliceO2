...
class SegmentationSuperAlpide
{
 public:
  ...
  const int mNRows{static_cast<int>(std::ceil(double(mRadii[mLayer] + mDetectorLayerThickness / 2.) * double(constants::math::PI) / double(mPitchRow)))}; ///< number of rows
...
inline void SegmentationSuperAlpide::curvedToFlat(float xCurved, float yCurved, float& xFlat, float& yFlat)
{
  ...
  yFlat = dist - (mRadii[mLayer] + mDetectorLayerThickness / 2.);
...
{
  float dist = yFlat + mRadii[mLayer] + mDetectorLayerThickness / 2.;
