...
void Digitizer::processHit(const o2::itsmft::Hit& hit, uint32_t& maxFr, int evID, int srcID)
{
  ...
  float thickness = innerBarrel ? SuperSegmentation::mDetectorLayerThickness : Segmentation::SensorLayerThickness;
  if (!innerBarrel) {
    xyzLocS.SetY(xyzLocS.Y() + resp->getDepthMax() - Segmentation::SensorLayerThickness / 2.);
  } else {
    xyzLocS.SetY(xyzLocS.Y() * (SuperSegmentation::mSensorLayerThicknessEff / 2. / SuperSegmentation::mDetectorLayerThickness)); // to avoid holes in clusters // FIXME
  }
