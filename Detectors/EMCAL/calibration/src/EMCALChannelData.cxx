...
//_____________________________________________
void EMCALChannelData::buildHitAndEnergyMean(double emin, double emax){     
    ...
    for (int cellID = 0; cellID < NCELLS; cellID++){
        ...
        if(sumVal > 0.)mEsumHisto(cellID, meanVal/(sumVal)); //..average energy per hit
...
// ------------------------------------------------------------------------------------------
void EMCALChannelData::buildHitAndEnergyMeanScaled(double emin, double emax){     
    ...
    for(int iter = 1; iter < 5; iter++){
     ...
     for (Int_t cell = 0; cell < NCELLS; cell++){
        ...
        if(Nsum > 0.)mEsumHistoScaled(cell, Esum/(Nsum)); //..average energy per hit
    }
