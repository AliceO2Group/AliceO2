...
//void FeeDCS_CalibDBTest(const std::string_view ccdbserver = "emcccdb-test.cern.ch")
void FeeDCS_CalibDBTest(const std::string_view ccdbserver = "http://ccdb-test.cern.ch")
{
  ...
  o2::emcal::FeeDCS* feeDCS = new o2::emcal::FeeDCS;
  feeDCS->setRunNumber(101010);
  feeDCS->setDDLlist0(12345);
  feeDCS->setDDLlist1(16789);

  for (int ism = 0; ism < 20; ism++) {
    feeDCS->setSRUFWversion(ism, ism * 2);
    feeDCS->setSRUconfig(ism, ism * 3);
  }
  ...
  feeDCS->setSTUEMCal(stuEMCal);
  feeDCS->setSTUDCal(stuDCal);
  for (int itru = 0; itru < 46; itru++)
    feeDCS->setTRUDCS(itru, tru);
  ...
  auto rangestart = create_timestamp(2017, 4, 23, 0, 58, 40),
       rangeend = create_timestamp(2017, 12, 5, 6, 3, 19);
  std::cout << "Using time stamps " << rangestart << " and " << rangeend << std::endl;
  std::map<std::string, std::string> metadata;

  ccdbhandler.storeFeeDCSData(feeDCS, metadata, rangestart, rangeend);

//  return;


  // Read FEE DCS from CCDB, check whether they are the same
  auto rangetest = create_timestamp(2017, 4, 23, 0, 58, 40); // LHC16
  std::cout << "Using read timestamp " << rangetest << std::endl;
  o2::emcal::FeeDCS* read(nullptr);
  try {
    read = ccdbhandler.readFeeDCSData(rangetest, metadata);
  ...
  if (*feeDCS == *read) {
    std::cout << "FEE DCS data matching - test successfull" << std::endl;
  } else {
    std::cerr << "FEE DCS data don't match" << std::endl;
  }
