...
/// bad or problematic during the calibration process and need
...
/// possible, or warm, indicating that the cell might be recovered
/// by recalibration of the energy.
///
...
///
/// The BadChannelMap is Browsable, when calling in the ROOT
/// browser a 2D histogram with the bad/warm cells with value
/// 1 / 2 shown depending on column/row. As the histogram is
/// created dynamically from the absolute cell ID an instance
/// of the EMCAL Geometry is required - otherwise an empty
...
class BadChannelMap
{
 ...
 private:
  std::bitset<19968> mBadCells;  ///< Container for bad cells (size corresponding to the maximum amount of cells in EMCAL+DCAL including indices in the PHOS region)
  std::bitset<19968> mWarmCells; ///< Contianer for warm cells (size corresponding to the maximum amount of cells in EMCAL+DCAL including indices in the PHOS region)
