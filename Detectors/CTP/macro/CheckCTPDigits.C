 ...
 while(reader.Next()) {
  ...
  for(j = 0; j < del; j++) {
     ...
     if(bc  == ctpdigs[i-j].intRecord.bc) {
       ...
       if(istvx.count() == 0) std::cout << "error: tcx missing " << std::endl;
       break;
     }
  }
  //std::cout << std::endl;
  if(0) {
     ...
     std::cout << std::dec << " =======> BC tm:" << dig->intRecord.bc << "" << " O:" << dig->intRecord.orbit << std::hex << " 0b" << dig->CTPClassMask << std::endl;
