 ...
 protected:
  ...
  uint32_t getRunStartOrbit() const
  {
    ...
    if (orb < 0) {
      ...
      if (mCurrentTFInfo.firstTForbit < 32 && mCurrentTFInfo.firstTForbit > 0) {
        suppressRunStartWarning = true;
      } else if (!suppressRunStartWarning && mCurrentTFInfo.firstTForbit >= 32) { // Otherwise this is most likely a SYNTHETIC run starting at orbit 0, so we don't need to print errors.
