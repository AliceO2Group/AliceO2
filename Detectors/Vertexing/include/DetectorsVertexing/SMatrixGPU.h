...
class SVectorGPU
{
 public:
  ...
  GPUdi() static size_t Dim() { return N; }
...
struct AssignSym {
  ...
  template <class T, size_t D, class A, class R>
...
class MatrixMulOpGPU
{
 public:
  ...
  GPUdi() T operator()(size_t i, size_t j) const
  {
...
class Inverter
{
 public:
  ...
  template <class T>
  GPUd() static int DfactMatrix(MatRepStdGPU<T, D, N>& rhs, T& det, size_t* work);

  // LU inversion of general square matrices. To be called after DFactMatrix
  template <class T>
  GPUd() static int DfinvMatrix(MatRepStdGPU<T, D, N>& rhs, size_t* work);
...
template <size_t D, size_t n>
template <class T>
GPUdi() int Inverter<D, n>::DfactMatrix(MatRepStdGPU<T, D, n>& rhs, T& det, size_t* ir)
{
  if (D != n)
    return -1;
...
template <size_t D, size_t n>
template <class T>
GPUdi() int Inverter<D, n>::DfinvMatrix(MatRepStdGPU<T, D, n>& rhs, size_t* ir)
{
  typedef T* mIter;
  typedef T value_type;
