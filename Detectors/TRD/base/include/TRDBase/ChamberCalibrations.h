...
class ChamberCalibrations
{
 public:
  ...
  ~ChamberCalibrations() = default;
  //
  float getVDrift(int p, int c, int s) const { return mVDrift[HelperMethods::getDetector(s, c, p)]; };
  float getVDrift(int roc) const { return mVDrift[roc]; };
  float getGainFactor(int p, int c, int s) const { return mGainFactor[HelperMethods::getDetector(s, c, p)]; };
  float getGainFactor(int roc) const { return mGainFactor[roc]; };
  float getT0(int p, int c, int s) const { return mT0[HelperMethods::getDetector(s, c, p)]; };
  float getT0(int roc) const { return mT0[roc]; };
  float getExB(int p, int c, int s) const { return mExB[HelperMethods::getDetector(s, c, p)]; };
  float getExB(int roc) const { return mExB[roc]; };
  void setVDrift(int p, int c, int s, float vdrift) { mVDrift[HelperMethods::getDetector(s, c, p)] = vdrift; };
  void setVDrift(int roc, float vdrift) { mVDrift[roc] = vdrift; };
  void setGainFactor(int p, int c, int s, float gainfactor) { mGainFactor[HelperMethods::getDetector(s, c, p)] = gainfactor; };
  void setGainFactor(int roc, float gainfactor) { mGainFactor[roc] = gainfactor; };
  void setT0(int p, int c, int s, float t0) { mT0[HelperMethods::getDetector(s, c, p)] = t0; };
  void setT0(int roc, float t0) { mT0[roc] = t0; };
  void setExB(int p, int c, int s, float exb) { mExB[HelperMethods::getDetector(s, c, p)] = exb; };
