...
{
for(int i=0;i<540;i++)out  << (int)(a->GetStatus(i)) << endl;
}

void PrintCalDet(ofstream &out, const AliTRDCalDet *caldet)
{

for(int i=0;i<AliTRDCalDet::kNdet;i++) out << caldet->GetValue(i) << endl;
...
void PrintCalPad(ofstream &out, const AliTRDCalPad *calpad)
{
for(int i=0;i<AliTRDCalDet::kNdet;i++) PrintCalROC(out, calpad->GetCalROC(i));
...
for(int i=0;i<chamberstatus->GetNrows()*chamberstatus->GetNcols();i++) out << (int)(chamberstatus->GetStatus(i))<< endl;
}
...
for(int i=9;i<540;i++){
	...
	for(int j=0;j<128;j++){
		...
		if(tblmcm){
			...
			for(int k=0;k<21;k++)out << tblmcm->GetFGFN(k)<< endl;
			for(int k=0;k<21;k++)out << tblmcm->GetFGAN(k)<< endl;
		}
		else cout << "tblmcm is null" << endl;
	}
	}
	else cout << "tblroc is null" << endl;
}
}
...
void OCDB2CCDB( Int_t run, const Char_t* storageURI = "alien://folder=/alice/data/2010/OCDB/") {
  ...
  manager->SetRun(Run);
   ...
   if((entry = GetCDBentry("TRD/Calib/ChamberStatus", 0))) {
    ...
    else cout << "attempt to get object chamber status from ocdb entry. Ergo not writing one to ccdb." << endl;
   }
   else cout << "failed to retrieve ocdb entry for Chamber Status" << endl; 
  ...
  //ChamberGainFactor and ChamberExB 
   ...
   if((entry = GetCDBentry("TRD/Calib/ChamberVdrift", 0))){ 
      ...
      else cout << "attempt to get object chamber Vdrift from ocdb entry. Will not be writing ChamberSettings" << endl;
   }
   else cout << "failed to retrieve ocdb entry for Chamber VDrift" << endl;
   ...
   if((entry = GetCDBentry("TRD/Calib/ChamberT0", 0))) {
      if((chamberT0 = (AliTRDCalDet*)entry->GetObject())){
        ...
        else cout << "attempt to get object chamber T0 from ocdb entry. Will not be writing ChamberSettings" << endl;
      }
      else cout << "failed to retrieve ocdb entry for Chamber T0" << endl;
   ...
   if((entry = GetCDBentry("TRD/Calib/ChamberGainFactor", 0))) {
        ...
        else cout << "attempt to get object chamber GainFactro from ocdb entry. Will not be writing ChamberSettings" << endl;
      }
      else cout << "failed to retrieve ocdb entry for Chamber GainFactor" << endl;
   ...
   if(chamberGainFactor && chamberExB && chamberT0 && chamberVDrift) {
   ...
   else cout << "something wrong with one of the members of ChamberCalibrations and not writing to ccdb, fix :: " << chamberGainFactor << "&&"<< chamberExB <<"&&"<< chamberT0 <<"&&" << chamberVDrift << endl;
    ...
    if((entry = GetCDBentry("TRD/Calib/LocalVdrift", 0))) {
        if((localvdrift = (AliTRDCalPad*)entry->GetObject())){
            for(int i=0;i< AliTRDCalDet::kNdet ; i++){ 
                ...
                else cout << "calroc is undefiend" << endl;
            }
        ccdb.storeAsTFileAny(o2localvdrift,"TRD_test/LocalVDrift",metadata,1000000000000+Run,1000000000000+Run+1);
        }
        else cout << "attempt to get object LocalVdrift from ocdb entry. Will not be writing LocalVDritf" << endl;
      }
      else cout << "failed to retrieve ocdb entry for LocalVdrift" << endl;
    ...
    auto o2localt0 = new o2::trd::LocalT0();
      if((entry = GetCDBentry("TRD/Calib/LocalT0", 0))) {
        ...
        else cout << "attempt to get object chamber LocalT0 from ocdb entry. Will not be writing LocalT0" << endl;
      }
      else cout << "failed to retrieve ocdb entry for LocalT0" << endl;
    ...
    if((entry = GetCDBentry("TRD/Calib/PadNoise",0))) {
        ...
        else cout << "attempt to get object PadNoise from ocdb entry. Will not be writing PadNoise" << endl;
      }
    ...
    auto o2localgainfactor =  new o2::trd::LocalGainFactor();
      if((entry = GetCDBentry("TRD/Calib/LocalGainFactor", 0))) {
        ...
        else cout << "attempt to get object LocalGainFactor from ocdb entry. Will not be writing LocalGainFactor" << endl;
      }
      else cout << "failed to retrieve ocdb entry for LocalGainFactor" << endl;
    ...
    AliTRDCalDet* chambernoise = 0;
      ...
      if((entry = GetCDBentry("TRD/Calib/DetNoise", 0))){ 
        ...
        else cout << "attempt to get object ChamberNoise from ocdb entry. Will not be writing ChamberNoise" << endl;
      }
...
//                o2padresponse->setPRF()
  ...
  //              o2padresponse->setPRF(i,j, calroc->GetValue(j));
        ...
        else cout << "attempt to get object PRFWidth from ocdb entry. Will not be writing PRFWidth" << endl;
      }
      else cout << "failed to retrieve ocdb entry for PRFWidth"<< endl;
