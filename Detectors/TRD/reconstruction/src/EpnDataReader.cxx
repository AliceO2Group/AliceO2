...
{
  auto config = ConfigParamSpec{"trd-epndatareader-config", VariantType::String, "A:TRD/RAWDATA", {"TRD raw data config"}};
  auto outputDesc = ConfigParamSpec{"trd-epndatareader-output-desc", VariantType::String, "TRDTLT", {"Output specs description string"}};
  auto verbosity = ConfigParamSpec{"trd-epndatareader-verbose", VariantType::Bool, false, {"Enable verbose epn data reading"}};
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  ...
  auto config = cfgc.options().get<std::string>("trd-epndatareader-config");
  auto verbosity = cfgc.options().get<bool>("trd-epndatareader-verbose");
  std::vector<OutputSpec> outputs;
  outputs.emplace_back(OutputSpec(ConcreteDataTypeMatcher{"TRD", "TRDTRACKLET"}));

  AlgorithmSpec algoSpec;
  algoSpec = AlgorithmSpec{adaptFromTask<o2::trd::EpnDataReaderTask>()};
  ...
  */
  ...
  while (getline(ssconfig, iconfig, ',')) { // for now we will keep the possibilty to have a device per half cru/feeid i.e. 6 per flp
    ...
    workflow.emplace_back(DataProcessorSpec{
      std::string("trd-epndatareader-") + std::to_string(idevice),
      select(iconfig.c_str()),
      outputs,
      algoSpec,
      Options{
        {"trd-epndatareader-verbose", VariantType::Bool, false, {"verbose flag"}}}});
    idevice++;
  }
