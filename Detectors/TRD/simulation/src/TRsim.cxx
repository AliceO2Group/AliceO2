...
/*
//_____________________________________________________________________________
TRsim::TRsim(FairModule* mod, int foil, int gap)
  : TObject(),
    mNFoilsDim(0),
    mNFoils(0),
    mNFoilsUp(0),
...
{
  //
  // TRsim constructor. Takes the material properties of the radiator
  ...
  mod->GetMaterial(foil, name, aFoil, zFoil, rhoFoil, rad, abs);
  mod->GetMaterial(gap, name, aGap, zGap, rhoGap, rad, abs);
...
/*
//_____________________________________________________________________________
TRsim::TRsim(const TRsim& s)
  : TObject(s),
    mNFoilsDim(s.mNFoilsDim),
    mNFoils(0),
    mNFoilsUp(0),
    mFoilThick(s.mFoilThick),
    mGapThick(s.mGapThick),
    mFoilDens(s.mFoilDens),
    mGapDens(s.mGapDens),
    mFoilOmega(s.mFoilOmega),
    mGapOmega(s.mGapOmega),
    mFoilZ(s.mFoilZ),
    mGapZ(s.mGapZ),
    mFoilA(s.mFoilA),
    mGapA(s.mGapA),
...
//_____________________________________________________________________________
TRsim& TRsim::operator=(const TRsim& s)
{
  //
  // Assignment operator
  //
...
//_____________________________________________________________________________
void TRsim::Copy(TObject& s) const
{
  //
  // Copy function
  //

  ((TRsim&)s).mFoilThick = mFoilThick;
  ((TRsim&)s).mFoilDens = mFoilDens;
  ((TRsim&)s).mFoilOmega = mFoilOmega;
  ((TRsim&)s).mFoilZ = mFoilZ;
  ((TRsim&)s).mFoilA = mFoilA;
  ((TRsim&)s).mGapThick = mGapThick;
  ((TRsim&)s).mGapDens = mGapDens;
  ((TRsim&)s).mGapOmega = mGapOmega;
  ((TRsim&)s).mGapZ = mGapZ;
  ((TRsim&)s).mGapA = mGapA;
  ((TRsim&)s).mTemp = mTemp;
  ((TRsim&)s).mSpNBins = mSpNBins;
  ((TRsim&)s).mSpRange = mSpRange;
  ((TRsim&)s).mSpBinWidth = mSpBinWidth;
  ((TRsim&)s).mSpLower = mSpLower;
  ((TRsim&)s).mSpUpper = mSpUpper;

  if (((TRsim&)s).mNFoils) {
    delete[]((TRsim&)s).mNFoils;
  }
  ((TRsim&)s).mNFoils = new int[mNFoilsDim];
  for (int iFoil = 0; iFoil < mNFoilsDim; iFoil++) {
    ((TRsim&)s).mNFoils[iFoil] = mNFoils[iFoil];
  }

  if (((TRsim&)s).mNFoilsUp) {
    delete[]((TRsim&)s).mNFoilsUp;
  }
  ((TRsim&)s).mNFoilsUp = new double[mNFoilsDim];
  for (int iFoil = 0; iFoil < mNFoilsDim; iFoil++) {
    ((TRsim&)s).mNFoilsUp[iFoil] = mNFoilsUp[iFoil];
  }

  if (((TRsim&)s).mSigma) {
    delete[]((TRsim&)s).mSigma;
  }
  ((TRsim&)s).mSigma = new double[mSpNBins];
  for (int iBin = 0; iBin < mSpNBins; iBin++) {
    ((TRsim&)s).mSigma[iBin] = mSigma[iBin];
  }
}
...
//_____________________________________________________________________________
int TRsim::CreatePhotons(int pdg, float p, int& nPhoton, float* ePhoton)
{
  //
...
//_____________________________________________________________________________
int TRsim::TrPhotons(float p, float mass, int& nPhoton, float* ePhoton)
{
  //
...
void TRsim::SetSigma()
{
  ...
  for (int iBin = 0; iBin < mSpNBins; iBin++) {
    ...
    mSigma[iBin] = Sigma(energykeV);
  }
}

//_____________________________________________________________________________
double TRsim::Sigma(double energykeV)
{
  //
...
double TRsim::GetMuPo(double energyMeV)
{
  ...
  double mu[kN] = { 1.894E+03, 5.999E+02, 2.593E+02, 7.743E+01, 3.242E+01, 1.643E+01, 9.432E+00, 3.975E+00, 2.088E+00, 7.452E-01, 4.315E-01, 2.706E-01, 2.275E-01, 2.084E-01, 1.970E-01, 1.823E-01, 1.719E-01, 1.534E-01, 1.402E-01, 1.217E-01, 1.089E-01, 9.947E-02, 9.198E-02, 8.078E-02, 7.262E-02, 6.495E-02, 5.910E-02, 5.064E-02, 4.045E-02, 3.444E-02, 3.045E-02, 2.760E-02, 2.383E-02, 2.145E-02, 1.819E-02, 1.658E-02 };

  double en[kN] = { 1.000E-03, 1.500E-03, 2.000E-03, 3.000E-03, 4.000E-03, 5.000E-03, 6.000E-03, 8.000E-03, 1.000E-02, 1.500E-02, 2.000E-02, 3.000E-02, 4.000E-02, 5.000E-02, 6.000E-02, 8.000E-02, 1.000E-01, 1.500E-01, 2.000E-01, 3.000E-01, 4.000E-01, 5.000E-01, 6.000E-01, 8.000E-01, 1.000E+00, 1.250E+00, 1.500E+00, 2.000E+00, 3.000E+00, 4.000E+00, 5.000E+00, 6.000E+00, 8.000E+00, 1.000E+01, 1.500E+01, 2.000E+01 };
...
double TRsim::GetMuCO(double energyMeV)
{
  ...
  double mu[kN] = { 0.39383E+04, 0.13166E+04, 0.58750E+03, 0.18240E+03, 0.77996E+02, 0.40024E+02, 0.23116E+02, 0.96997E+01, 0.49726E+01, 0.15543E+01, 0.74915E+00, 0.34442E+00, 0.24440E+00, 0.20589E+00, 0.18632E+00, 0.16578E+00, 0.15394E+00, 0.13558E+00, 0.12336E+00, 0.10678E+00, 0.95510E-01, 0.87165E-01, 0.80587E-01, 0.70769E-01, 0.63626E-01, 0.56894E-01, 0.51782E-01, 0.44499E-01, 0.35839E-01, 0.30825E-01, 0.27555E-01, 0.25269E-01, 0.22311E-01, 0.20516E-01, 0.18184E-01, 0.17152E-01 };
...
double TRsim::GetMuMy(double energyMeV)
{
  ...
  double mu[kN] = { 2.911E+03, 9.536E+02, 4.206E+02, 1.288E+02, 5.466E+01, 2.792E+01, 1.608E+01, 6.750E+00, 3.481E+00, 1.132E+00, 5.798E-01, 3.009E-01, 2.304E-01, 2.020E-01, 1.868E-01, 1.695E-01, 1.586E-01, 1.406E-01, 1.282E-01, 1.111E-01, 9.947E-02, 9.079E-02, 8.395E-02, 7.372E-02, 6.628E-02, 5.927E-02, 5.395E-02, 4.630E-02, 3.715E-02, 3.181E-02, 2.829E-02, 2.582E-02, 2.257E-02, 2.057E-02, 1.789E-02, 1.664E-02 };
...
double TRsim::GetMuHe(double energyMeV)
{
  ...
  double mu[kN] = { 6.084E+01, 1.676E+01, 6.863E+00, 2.007E+00, 9.329E-01, 5.766E-01, 4.195E-01, 2.933E-01, 2.476E-01, 2.092E-01, 1.960E-01, 1.838E-01, 1.763E-01, 1.703E-01, 1.651E-01, 1.562E-01, 1.486E-01, 1.336E-01, 1.224E-01, 1.064E-01, 9.535E-02, 8.707E-02, 8.054E-02, 7.076E-02, 6.362E-02, 5.688E-02, 5.173E-02, 4.422E-02, 3.503E-02, 2.949E-02, 2.577E-02, 2.307E-02, 1.940E-02, 1.703E-02, 1.363E-02, 1.183E-02 };
