...
class TrapSimulator
{
 public:
  ...
  void init(int det, int rob, int mcm);
...
*/
  ...
  void writeData();
  bool storeTracklets(); // Stores tracklets via runloader
  ...
  void addHitToFitreg(int adc, unsigned short timebin, unsigned short qtot, Short_t ypos, int label[]);
  void calcFitreg();
  void trackletSelection();
  void fitTracklet();

  int getNHits() const { return mNHits; }
  bool getHit(int index, int& channel, int& timebin, int& qtot, int& ypos, Float_t& y, int& label) const;
