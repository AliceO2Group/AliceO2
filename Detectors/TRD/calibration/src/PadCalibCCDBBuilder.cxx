...
{
    if( hDet->GetBinContent(coordinates[0], coordinates[1]) == 0 || hDet->GetBinContent(coordinates[2], coordinates[3]) == 0 ) return;
...
std::vector<int> PadCalibCCDBBuilder::CompareGain( TH2F* hDet, int column, int row, int shiftcolumn, int shiftrow, float allowedDifference)
{
    ...
    if( !(shiftcolumn == 1 && shiftrow == 0) && !(shiftcolumn == 0 && shiftrow ==  1) ) return coordinates;

    // cheks that the pad is valid
    if( (column >= colMax) || (row >= rowMax) ) return coordinates;

    if( (column == colMax && shiftcolumn == 1) || (row == rowMax && shiftrow == 1 ) ) return coordinates; // do not compare with overflow
...
{ // clones the filled 2d map
  ...
  // replaces each bin conetnt with ratio of gain over the average
    ...
    if( !hDet || hDet->GetEntries() == 0 ) return hDetNormalized;
...
{ // takes hDet and replaces content of bin (column,row) with gain
  // CHANGES THE TH2 GIVEN IN ARGUMENT!
    if( newGain <= 0 ) return; // only substitute positive gain
    int currentGain = hDet->GetBinContent( hDet->GetXaxis()->FindBin(column), hDet->GetYaxis()->FindBin(row) );
    if( currentGain != 0 ) return; // make sure we don't replace gain, just fill in gaps
...
{ // clones the map and fills the clone
  ...
  // the map is cloned to limit propagation of neighboring bin content
    ...
    if( !hDet || hDet->GetEntries() == 0 ) return hDetFilled;
...
{ // takes bin (column,row) from hDet map
  // and averages the gain in its (up to) 8 neighbours
    ...
    for( int irow = 0; irow < nbuffer; irow++ ) {
        ...
        if( rowNeighbor < 0 || rowNeighbor >= hDet->GetNbinsY()) continue; // avoids under and overflow
        for( int icol = 0; icol < nbuffer; icol++ ) {
            if( icol==1 && irow==1 ) continue; // exclude self
            int colNeighbor = (column - offset) + icol; 
            if( colNeighbor < 0 || colNeighbor >= hDet->GetNbinsX()) continue; // avoids under and overflow
            float tempGain = hDet->GetBinContent( hDet->GetXaxis()->FindBin(colNeighbor), hDet->GetYaxis()->FindBin(rowNeighbor) );
            if( tempGain <= 0 ) continue; // exclude empty/negative bins
            gainNeighbor.push_back(tempGain);
        }
    }
    int numberOfNeighbors =  (int)gainNeighbor.size();
    if( numberOfNeighbors < 1 ) return 0; // if empty, return 0
...
{ // creates a th2 map of a detector nDet
  ...
  // mingain and maxgain (default range 0-10k)
    if ( sDetName == "" ) sDetName = Form("hDet%i",nDet);
    int nTrdRows = NROWC1;
    int detInSupermodule = nDet % 30; // set the correct # of rows for stack 2
    if( detInSupermodule >= 12 && detInSupermodule <= 17 ) nTrdRows = NROWC0;
    ...
    for(int ientry = 0; ientry < nentries; ientry++ ) {
        ...
        if( (int)det != nDet ) continue; 
        if( adc < mingain || adc > maxgain ) continue;
...
bool PadCalibCCDBBuilder::IsHotAreaIsolated(TH2F* hDet, int column, int row, int matrixSize)
{
    ...
    for( int i = 0; i < matrixSize+2; i++ ) {
        ...
        for( int j = 0; j < matrixSize+2; j++ ) {
            int jrow = j-1;
            if( (-1 < icol) && (icol < matrixSize) && (-1 < jrow) && (jrow <matrixSize) ) continue;
            if( abs(icol) <= (int)(matrixSize-1)/2 ) continue;
    ...
    if( averageAround < 2*averageGain && nSurroundingNotHot < nMaxHot ) isIsolated = kFALSE;
...
void PadCalibCCDBBuilder::SmoothenTheDetector(TH2F* hDet, float allowedDifference) 
{
        ...
        if( SuspiciousBinPairs.size() == 0 ) return;
