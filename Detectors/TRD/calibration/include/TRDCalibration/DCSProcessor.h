...
class DCSProcessor
{
 ...
 public:
  ...
  const std::unordered_map<DPID, TRDDCSinfo>& getTRDDPsInfo() const { return mTRDDCS; }
 ...
 private:
  std::unordered_map<DPID, TRDDCSinfo> mTRDDCS;                // this is the object that will go to the CCDB
...
template <typename T>
void DCSProcessor::prepareCCDBobjectInfo(T& obj, CcdbObjectInfo& info, const std::string& path, TFType tf,
                                         const std::map<std::string, std::string>& md)
{
