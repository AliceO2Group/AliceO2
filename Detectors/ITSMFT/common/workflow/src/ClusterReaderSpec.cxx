...
void ClusterReader<N>::run(ProcessingContext& pc)
{
  ...
  static const std::vector<o2::itsmft::ROFRecord> noClusROFRec;
  static const std::vector<o2::itsmft::CompClusterExt> noClusters;
  static const std::vector<unsigned char> noPatterns;
  static const o2::dataformats::MCTruthContainer<o2::MCCompLabel> noLabels;
  for (uint32_t iLayer = 0; iLayer < mLayers; ++iLayer) {
    const auto& clusROFRec = noEntry ? noClusROFRec : *mClusROFRec[iLayer];
    const auto& clusters = noEntry ? noClusters : *mClusterCompArray[iLayer];
    LOG(info) << mDetName << "ClusterReader" << (mDoStaggering ? std::format(" on layer {}", iLayer) : "") << " pushes " << clusROFRec.size() << " ROFRecords, " << clusters.size() << " compact clusters at entry " << ent;
    pc.outputs().snapshot(Output{Origin, "CLUSTERSROF", iLayer}, clusROFRec);
    pc.outputs().snapshot(Output{Origin, "COMPCLUSTERS", iLayer}, clusters);
    if (mUsePatterns) {
      pc.outputs().snapshot(Output{Origin, "PATTERNS", iLayer}, noEntry ? noPatterns : *mPatternsArray[iLayer]);
    }
    if (mUseMC) {
      pc.outputs().snapshot(Output{Origin, "CLUSTERSMCTR", iLayer}, noEntry ? noLabels : *mClusterMCTruth[iLayer]);
