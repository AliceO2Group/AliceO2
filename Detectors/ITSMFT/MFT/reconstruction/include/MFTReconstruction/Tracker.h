...
class Tracker
{
  ...
  public:
    ...
    void process(const std::vector<Cluster> &clusters, std::vector<Track> &tracks);
    void processFrame(std::vector<Track> &tracks);
    const Cluster* getCluster(Int_t index) const;
    void setGeometry(o2::MFT::GeometryTGeo* geom);
    void setMCTruthContainers(const o2::dataformats::MCTruthContainer<o2::MCCompLabel> *clsLabels, o2::dataformats::MCTruthContainer<o2::MCCompLabel> *trkLabels)
    {
  ...
  protected:
    ...
    void unloadClusters();
    std::vector<Track> trackInThread(Int_t first, Int_t last);
