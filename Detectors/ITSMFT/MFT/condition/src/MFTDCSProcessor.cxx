  ...
  if (processFlags(flags, dpid.get_alias()) == 0) {
    ...
    if (type == RAW_INT) {
      ...
      if (std::strstr(dpid.get_alias(), "FEACSTATUS") != nullptr) { // DP is FEACSTATUS
        ...
        const auto offs = std::strlen("MFT_FEACSTATUS_");
      ...
      if (std::strstr(dpid.get_alias(), "HVSTATUS") != nullptr) { // DP is HVSTATUS
        ...
        const auto offs = std::strlen("MFT_HVSTATUS_SM");
        std::size_t const nn = aliasStr.find_first_of("0123456789", offs);
        std::size_t const mm = aliasStr.find_first_not_of("0123456789", nn);
...
uint64_t MFTDCSProcessor::processFlags(const uint64_t flags, const char* pid)
{
...
void MFTDCSProcessor::updateFEACCCDB()
{
  ...
  std::map<std::string, std::string> md;
  md["responsible"] = "Chiara Zampolli";
  prepareCCDBobjectInfo(mFeac, mccdbLVInfo, "MFT/Calib/LVStatus", mTF, md);
  return;
}

//______________________________________________________________________

void MFTDCSProcessor::updateHVCCDB()
{
  ...
  std::map<std::string, std::string> md;
  md["responsible"] = "Chiara Zampolli";
  prepareCCDBobjectInfo(mHV, mccdbHVInfo, "MFT/Calib/HVStatus", mTF, md);
  return;
}
*/
