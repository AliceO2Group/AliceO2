...
void MakeNoiseMapFromClusters(std::string input = "mftclusters.root", bool only1pix = false, float probT = 3e-6, std::string output = "noise.root")
{
  ...
  clusTree->SetBranchAddress("MFTClusterComp", &clusters);
  ...
  clusTree->SetBranchAddress("MFTClustersROF", &rofVec);

  o2::mft::NoiseCalibrator calib(only1pix, probT);
