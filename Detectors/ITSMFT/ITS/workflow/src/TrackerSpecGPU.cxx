...
TrackerGPUDPL::TrackerGPUDPL(bool isMC, const std::string& trModeS, o2::gpu::GPUDataTypes::DeviceType dType) : mIsMC{isMC}, mMode{trModeS}, mRecChain{o2::gpu::GPUReconstruction::CreateInstance(dType, true)}
{
  std::transform(mMode.begin(), mMode.end(), mMode.begin(), [](unsigned char c) { return std::tolower(c); });
}

void TrackerGPUDPL::init(InitContext& ic)
{
  mTimer.Stop();
  mTimer.Reset();
...
void TrackerGPUDPL::run(ProcessingContext& pc)
{
  mTimer.Start(false);
...
void TrackerGPUDPL::endOfStream(EndOfStreamContext& ec)
{
  LOGF(info, "ITS CA-Tracker total timing: Cpu: %.3e Real: %.3e s in %d slots",
       mTimer.CpuTime(), mTimer.RealTime(), mTimer.Counter() - 1);
}

DataProcessorSpec getTrackerGPUSpec(bool useMC, const std::string& trModeS, o2::gpu::GPUDataTypes::DeviceType dType)
{
  ...
  return DataProcessorSpec{
    ...
    inputs,
    outputs,
    AlgorithmSpec{adaptFromTask<TrackerGPUDPL>(useMC, trModeS, dType)},
    Options{
