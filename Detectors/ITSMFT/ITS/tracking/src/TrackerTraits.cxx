...
void TrackerTraits::computeLayerTracklets(PrimaryVertexContext& primaryVertexContext, int iteration)
{
  for (int iLayer{ 0 }; iLayer < Constants::ITS::TrackletsPerRoad; ++iLayer) {
    ...
    for (int iCluster{ 0 }; iCluster < currentLayerClustersNum; ++iCluster) {
      ...
      const int4 selectedBinsRect{ getBinsRect(currentCluster, iLayer, directionZIntersection,
                                               mTrkParams.TrackletMaxDeltaZ[iteration][iLayer], mTrkParams.TrackletMaxDeltaPhi[iteration]) };
      ...
      for (int iPhiBin{ selectedBinsRect.y }, iPhiCount{ 0 }; iPhiCount < phiBinsNum;
        ...
        for (int iNextLayerCluster{ firstRowClusterIndex }; iNextLayerCluster < maxRowClusterIndex;
          ...
          if (deltaZ < mTrkParams.TrackletMaxDeltaZ[iteration][iLayer] &&
              (deltaPhi < mTrkParams.TrackletMaxDeltaPhi[iteration] ||
               MATH_ABS(deltaPhi - Constants::Math::TwoPi) < mTrkParams.TrackletMaxDeltaPhi[iteration])) {
...
void TrackerTraits::computeLayerCells(PrimaryVertexContext& primaryVertexContext, int iteration)
{
  for (int iLayer{ 0 }; iLayer < Constants::ITS::CellsPerRoad; ++iLayer) {
    ...
    for (int iTracklet{ 0 }; iTracklet < currentLayerTrackletsNum; ++iTracklet) {
      ...
      for (int iNextLayerTracklet{ nextLayerFirstTrackletIndex };
        ...
        if (deltaTanLambda < mTrkParams.CellMaxDeltaTanLambda[iteration] &&
            (deltaPhi < mTrkParams.CellMaxDeltaPhi[iteration] ||
             std::abs(deltaPhi - Constants::Math::TwoPi) < mTrkParams.CellMaxDeltaPhi[iteration])) {
          ...
          if (deltaZ < mTrkParams.CellMaxDeltaZ[iteration][iLayer]) {
            ...
            if (distanceOfClosestApproach >
                mTrkParams.CellMaxDCA[iteration][iLayer]) {
