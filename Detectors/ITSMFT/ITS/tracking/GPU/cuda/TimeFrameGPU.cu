...
int TimeFrameGPU<NLayers>::downloadVertexCandsDevice()
{
  ...
  mPeakOffsetsHost = bounded_vector<int>(this->getNrof(1) + 1, 0, this->getMemoryPool().get());
  ...
  mVertexCandsHost = bounded_vector<VertexCand>(nPeaks, {}, this->getMemoryPool().get());
  if (nPeaks) {
...
unsigned int TimeFrameGPU<NLayers>::downloadLinesDevice()
{
  ...
  mLinesHost = bounded_vector<o2::its::Line>(nLines, {}, this->getMemoryPool().get());
  mLineRofHost = bounded_vector<int>(nLines, 0, this->getMemoryPool().get());
  mLineClustersHost = bounded_vector<int>(3 * nLines, 0, this->getMemoryPool().get());
  if (nLines) {
