...
class DBScan : Graph<T>
{
 public:
  DBScan() = delete;
  explicit DBScan(const std::size_t nThreads);
...
template <typename T>
DBScan<T>::DBScan(const std::size_t nThreads) : Graph<T>(nThreads)
{
}
...
{
  classifyVertices([nContributors](std::vector<o2::its::Edge>& edges) { return edges.size() == 0 ? 0 : edges.size() >= static_cast<std::size_t>(nContributors - 1) ? 2 : 1; },
...
void DBScan<T>::classifyVertices(std::function<unsigned char(std::vector<Edge>& edges)> classFunction)
{
  ...
  const std::size_t size = {this->mVertices->size()};
  mStates.resize(size);

  if (!this->isMultiThreading()) {
    for (std::size_t iVertex{0}; iVertex < size; ++iVertex) {
      mStates[iVertex] = std::make_pair<int, unsigned char>(iVertex, classFunction(this->getEdges()[iVertex]));
    }
  } else {
    const std::size_t stride{static_cast<std::size_t>(std::ceil(this->mVertices->size() / static_cast<std::size_t>(this->mExecutors.size())))};
    for (std::size_t iExecutor{0}; iExecutor < this->mExecutors.size(); ++iExecutor) {
      ...
      this->mExecutors[iExecutor] = std::thread(
        [iExecutor, stride, this](const auto& classFunction) {
          for (std::size_t iVertex{iExecutor * stride}; iVertex < stride * (iExecutor + 1) && iVertex < this->mVertices->size(); ++iVertex) {
            mStates[iVertex] = std::make_pair<int, unsigned char>(iVertex, classFunction(this->getEdges()[iVertex]));
          }
        },
        mClassFunction);
