...
class Vector
    ...
    public:
      ...
      Vector(const T* const, const int, const int = 0);GPU_HOST_DEVICE ~Vector();
  ...
  template<typename T>
  Vector<T>::Vector(const int capacity, const int initialSize)
      : Vector { nullptr, capacity, initialSize }
  {
    // Nothing to do
  }

  template<typename T>
  Vector<T>::Vector(const T* const source, const int size, const int initialSize)
