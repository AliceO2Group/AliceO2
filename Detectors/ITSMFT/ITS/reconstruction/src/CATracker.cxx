...
bool Tracker::CellParams(int l, const ClsInfo_t& c1, const ClsInfo_t& c2, const ClsInfo_t& c3, float& curv,
                         array<float, 3>& n)
{
...
void Tracker::CellsTreeTraversal(vector<Road>& roads, const int& iD, const int& doubl)
{
...
void Tracker::FindTracksCA(int iteration)
{
  ...
  for (int level = 5; level > itLevelLimit[iteration]; --level) {
    ...
    for (size_t iR = 0; iR < roads.size(); ++iR) {
      ...
      std::array<float, 15> cov{ 5.f * 5.f, 0.f,         5.f * 5.f, 0.f, 0.f, 0.7f * 0.7f, 0.f, 0.f,
                                 0.f,       0.7f * 0.7f, 0.f,       0.f, 0.f, 0.f,         10.f };
...
void Tracker::MakeCells(int iteration)
{
  ...
  for (int iL = 0; iL < 6; ++iL) {
    ...
    for (int iC = 0; iC < mLayer[iL]->GetNClusters(); ++iC) {
      ...
      const int nClust =
        mLayer[iL + 1]->SelectClusters(extz - 2 * mCZ, extz + 2 * mCZ, cls.phi - mCPhi, cls.phi + mCPhi);
      bool first = true;
  ...
  for (int iD = 0; iD < 5; ++iD) {
    if (mDoublets[iD + 1].size() == 0u || mDoublets[iD].size() == 0u)
      continue;
    ...
    for (size_t iD0 = 0; iD0 < mDoublets[iD].size(); ++iD0) {
      ...
      for (size_t iD1 = dLUT[iD][idx]; iD1 < mDoublets[iD + 1].size(); ++iD1) {
        ...
        if (fabs(mDoublets[iD][iD0].tanL - mDoublets[iD + 1][iD1].tanL) < mCDTanL &&
          ...
          const float extz = -tan * (*mLayer[iD])[mDoublets[iD][iD0].x].r + (*mLayer[iD])[mDoublets[iD][iD0].x].z;
          if (fabs(extz - GetZ()) < mCDCAz[iD]) {
            ...
            if (CellParams(iD, (*mLayer[iD])[mDoublets[iD][iD0].x], (*mLayer[iD + 1])[mDoublets[iD][iD0].y],
              ...
              mCells[iD].emplace_back(mDoublets[iD][iD0].x, mDoublets[iD][iD0].y, mDoublets[iD + 1][iD1].y, iD0, iD1,
                                      curv, n);
            }
          }
        }
