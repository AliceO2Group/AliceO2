...
void addAlignableVolumesMCH()
{
  ...
  for (int hc = 0; hc < 20; hc++)
  {
    ...
    TString path = Form("%sSC%s%d%s_%d",volPathName.c_str(), nCh < 10 ? "0" : "", nCh, hc%2 ? "O" : "I",hc);
    TString sname = Form("MCH/HC%d",hc);
    ...
    addAlignableVolumesHalfChamber(hc, volPathName);
  }
...
void addAlignableVolumesHalfChamber(int hc, std::string& parent)
{
  ...
  //
  // std::vector<int> nDEperHC{2,2,2,2,2,2,2,2,9,9,9,9,13,13,13,13,13,13,13,13};
  ...
  for (int i = 0; i < DEofHC[hc].size(); i++)
  {
    ...
    // std::string volPathName =  "SC01O_1/Quadrant (chamber 1)_101";  
