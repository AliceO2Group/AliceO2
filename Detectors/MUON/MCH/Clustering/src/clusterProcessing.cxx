...
int clusterProcess(const double* xyDxyi, const Mask_t* cathi, const Mask_t* saturated, const double* zi, int chId, int nPads)
{
  ...
  if (VERBOSE) {
    ...
    printXYdXY("Pads input xyDxyi, zi", xyDxyi, nPads, nPads, zi, nullptr);
  ...
  for (int g = 1; g <= nGroups; g++) {
    ...
    */
    ...
    if (VERBOSE > 0) {
      ...
      printXYdXY("  xyDxyGrp", xyDxyGrp, nbrOfPadsInTheGroup, nbrOfPadsInTheGroup, chGrp, nullptr);
    ...
    if (1 && (kOpt != K)) {
      ...
      weightedEMLoop(xyDxyGrp, saturatedGrp, chGrp, theta0, nullptr, K, nbrOfPadsInTheGroup, EMmode, EMConvergence, EMverbose, thetaEM);
    }
    if (VERBOSE > 0) {
    ...
    if (filteredK != K) {
      ...
      //
      // Final EM
      weightedEMLoop(xyDxyGrp, saturatedGrp, chGrp, thetaFiltered, nullptr, filteredK, nbrOfPadsInTheGroup, EMmode, EMConvergence, EMverbose, thetaEMFinal);
    } else {
      filteredK = K;
    ...
    if ((nFit < nbrOfPadsLimitForTheFitting) && wellSplitGroup[g]) {
      ...
      if (VERBOSE) {
        ...
        printXYdXY("- Pads input xyDxyFit, zi", xyDxyFit, nFit, nFit, zFit, nullptr);
