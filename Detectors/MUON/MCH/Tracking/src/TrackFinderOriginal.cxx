...
constexpr double TrackFinderOriginal::SDefaultChamberZ[10];
...
void TrackFinderOriginal::findTrackCandidates()
{
  ...
  for (int iSt = 4; iSt >= 3; --iSt) {
    ...
    for (; itTrack != mTracks.end();) {
      ...
      if (!TrackerParam::Instance().requestStation[7 - iSt] && itNewTrack == mTracks.end()) {
        ++itTrack;
      } else {
...
void TrackFinderOriginal::createTrack(const Cluster& cl1, const Cluster& cl2)
{
  ...
  paramCov.Zero();
  // Non bending plane
  double cl1Ex2 = mChamberResolutionX2;
  double cl2Ex2 = mChamberResolutionX2;
  paramCov(0, 0) = cl1Ex2;
  paramCov(0, 1) = cl1Ex2 / dZ;
  paramCov(1, 0) = paramCov(0, 1);
  paramCov(1, 1) = (cl1Ex2 + cl2Ex2) / dZ / dZ;
  // Bending plane
  double cl1Ey2 = mChamberResolutionY2;
  double cl2Ey2 = mChamberResolutionY2;
  paramCov(2, 2) = cl1Ey2;
  ...
  if (TrackExtrap::isFieldON()) {
    paramCov(4, 4) = ((mBendingVertexDispersion2 +
...
void TrackFinderOriginal::updateTrack(Track& track, TrackParam& trackParamAtCluster)
{
  ...
  if (TrackerParam::Instance().requestStation[trackParamAtCluster.getClusterPtr()->getChamberId() / 2]) {
...
void TrackFinderOriginal::updateTrack(Track& track, TrackParam& trackParamAtCluster1, TrackParam& trackParamAtCluster2)
{
  ...
  double localChi2 = deltaX * deltaX / mChamberResolutionX2 + deltaY * deltaY / mChamberResolutionY2;
...
std::list<Track>::iterator TrackFinderOriginal::recoverTrack(std::list<Track>::iterator& itTrack, int nextStation)
{
  ...
  for (auto itParam = itTrack->begin(); itParam != itTrack->end(); ++itParam) {
    ...
    if (TrackerParam::Instance().requestStation[nextStation + 1]) {
...
//_________________________________________________________________________________________________
void TrackFinderOriginal::refineTracks()
{
  /// Refit, improve and finalize the reconstructed tracks
