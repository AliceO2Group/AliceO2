...
void run_sim_cpv(Int_t nEvents = 10, TString mcEngine = "TGeant3")
{
  ...
  // Output file name
  char fileout[100];
  sprintf(fileout, "AliceO2_%s.cpv.mc_%i_event.root", mcEngine.Data(), nEvents);
  TString outFile = fileout;

  // Parameter file name
  char filepar[100];
  sprintf(filepar, "AliceO2_%s.cpv.params_%i.root", mcEngine.Data(), nEvents);
...
#ifdef BOX_GENERATOR
  FairBoxGenerator* boxGen = new FairBoxGenerator(11, 10); /*electrons*/
