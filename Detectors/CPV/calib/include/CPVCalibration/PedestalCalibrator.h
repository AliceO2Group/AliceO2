...
//===================================================================
  class PedestalCalibrator final : public o2::calibration::TimeSlotCalibration<o2::cpv::Digit, o2::cpv::PedestalCalibData>
  {
   public:
    ...
    bool hasEnoughData(const TimeSlot& slot) const final { LOG(INFO) << "hasEnoughData() is being called"; return slot.getContainer()->mNEvents >= mMinEvents; }
    void initOutput();
    void finalizeSlot(TimeSlot& slot);
    TimeSlot& emplaceNewSlot(bool front, uint64_t tstart, uint64_t tend);
