...
void KrBoxClusterFinder::updateTempClusterFinal()
{
  if(mTempCluster.totCharge == 0) {
    ...
    // if(int(mTempCluster.size) < 2) {
      // std::cout << "QTot:     " << mTempCluster.totCharge << std::endl;
      // std::cout << "QMax:     " << mTempCluster.maxCharge << std::endl;
      // std::cout << "size:     " << int(mTempCluster.size) << std::endl;
      // std::cout << "MeanRow:  " << mTempCluster.meanRow   << std::endl;
      // std::cout << "MeanPad:  " << mTempCluster.meanPad   << std::endl;
      // std::cout << "MeanTime: " << mTempCluster.meanTime  << std::endl;
    // }
...
void KrBoxClusterFinder::updateTempCluster(float tempCharge, int tempPad, int tempRow, int tempTime)
{
  ...
  // Set this digit to 0
  // std::cout << "Pad:  " << tempPad  << std::endl;
  // std::cout << "Time: " << tempTime << std::endl;
  // std::cout << "Row:  " << tempRow  << std::endl;
  mMapOfAllDigits[tempTime][tempRow][tempPad] = 0;
}
  ...
  for (int iTime = 0; iTime < MaxTimes; iTime++) { //mMapOfAllDigits.size()
    ...
    for (int iRow = 0; iRow < MaxRows; iRow++) { // mapRow.size()
      ...
      for (int iPad = MaxPads / 2 - padsInRow / 2; iPad < MaxPads / 2 + padsInRow / 2; iPad++) { // mapPad.size()
        ...
        // Check diagonals in pad- and row-direction
        if(iRow + 1 < MaxRows && iPad + 1 < MaxPads) {
          if(mMapOfAllDigits[iTime][iRow + 1][iPad + 1] > qMax) {
            continue;
          }
        }
        if(iRow - 1 >= 0 && iPad + 1 < MaxPads) {
          if(mMapOfAllDigits[iTime][iRow - 1][iPad + 1] > qMax) {
            continue;
          }
        }
        if(iRow + 1 < MaxRows && iPad - 1 >= 0) {
          if(mMapOfAllDigits[iTime][iRow + 1][iPad - 1] > qMax) {
            continue;
          }
        }
        if(iRow - 1 >= 0 && iPad - 1 >= 0) {
          if(mMapOfAllDigits[iTime][iRow - 1][iPad - 1] > qMax) {
            continue;
          }
        }

        // Check diagonals in pad- and time-direction
        if(iTime + 1 < MaxTimes && iPad + 1 < MaxPads) {
          if(mMapOfAllDigits[iTime + 1][iRow][iPad + 1] > qMax) {
            continue;
          }
        }
        if(iTime - 1 >= 0 && iPad + 1 < MaxPads) {
          if(mMapOfAllDigits[iTime - 1][iRow][iPad + 1] > qMax) {
            continue;
          }
        }
        if(iTime + 1 < MaxTimes && iPad - 1 >= 0) {
          if(mMapOfAllDigits[iTime + 1][iRow][iPad - 1] > qMax) {
            continue;
          }
        }
        if(iTime - 1 >= 0 && iPad - 1 >= 0) {
          if(mMapOfAllDigits[iTime - 1][iRow][iPad - 1] > qMax) {
            continue;
          }
        }

        // Check diagonals in row- and time-direction
        if(iTime + 1 < MaxTimes && iRow + 1 < MaxRows) {
          if(mMapOfAllDigits[iTime + 1][iRow + 1][iPad] > qMax) {
            continue;
          }
        }
        if(iTime - 1 >= 0 && iRow + 1 < MaxRows) {
          if(mMapOfAllDigits[iTime - 1][iRow + 1][iPad] > qMax) {
            continue;
          }
        }
        if(iTime + 1 < MaxTimes && iRow - 1 >= 0) {
          if(mMapOfAllDigits[iTime + 1][iRow - 1][iPad] > qMax) {
            continue;
          }
        }
        if(iTime - 1 >= 0 && iRow - 1 >= 0) {
          if(mMapOfAllDigits[iTime - 1][iRow - 1][iPad] > qMax) {
            continue;
          }
        }

        // Check diagonals in row-, pad- and time-direction
        if(iTime + 1 < MaxTimes && iRow + 1 < MaxRows && iPad + 1 < MaxPads) {
          if(mMapOfAllDigits[iTime + 1][iRow + 1][iPad + 1] > qMax) {
            continue;
          }
        }
        if(iTime + 1 < MaxTimes && iRow + 1 < MaxRows && iPad - 1 >= 0) {
          if(mMapOfAllDigits[iTime + 1][iRow + 1][iPad - 1] > qMax) {
            continue;
          }
        }
        if(iTime + 1 < MaxTimes && iRow - 1 >= 0 && iPad + 1 < MaxPads) {
          if(mMapOfAllDigits[iTime + 1][iRow - 1][iPad + 1] > qMax) {
            continue;
          }
        }
        if(iTime + 1 < MaxTimes && iRow - 1 >= 0 && iPad - 1 >= 0) {
          if(mMapOfAllDigits[iTime + 1][iRow - 1][iPad - 1] > qMax) {
            continue;
          }
        }
        if(iTime - 1 >= 0 && iRow + 1 < MaxRows && iPad + 1 < MaxPads) {
          if(mMapOfAllDigits[iTime - 1][iRow + 1][iPad + 1] > qMax) {
            continue;
          }
        }
        if(iTime - 1 >= 0 && iRow + 1 < MaxRows && iPad - 1 >= 0) {
          if(mMapOfAllDigits[iTime - 1][iRow + 1][iPad - 1] > qMax) {
            continue;
          }
        }
        if(iTime - 1 >= 0 && iRow - 1 >= 0 && iPad + 1 < MaxPads) {
          if(mMapOfAllDigits[iTime - 1][iRow - 1][iPad + 1] > qMax) {
            continue;
          }
        }
        if(iTime - 1 >= 0 && iRow - 1 >= 0 && iPad - 1 >= 0) {
          if(mMapOfAllDigits[iTime - 1][iRow - 1][iPad - 1] > qMax) {
            continue;
          }
        }

        for (int i = 2; (i <= mMaxClusterSizePad) && thisIsMax; i++) {
          if (iPad + i >= MaxPads) {
            continue;
          }
          for (int k = 2; (k <= mMaxClusterSizeRow) && thisIsMax; k++) {
            if (iRow + k >= MaxRows) {
              continue;
            }
            for (int j = 2; (j <= mMaxClusterSizeTime) && thisIsMax; j++) {
              if (iTime + j >= MaxTimes) {
                continue;
              }
