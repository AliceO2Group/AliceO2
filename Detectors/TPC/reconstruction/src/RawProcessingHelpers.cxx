...
bool raw_processing_helpers::processZSdata(const char* data, size_t size, rdh_utils::FEEIDType feeId, uint32_t orbit, uint32_t referenceOrbit, ADCCallback fillADC, bool useTimeBin)
{
  ...
  while (zsdata < zsdataEnd) {
    ...
    if (bcOffset < 0) {
      LOGP(debug, "skipping negative time bin with (globalBCoffset (({} - {}) * {} = {}) + bunchCrossingHeader ({}) - syncOffset({})) / 8 = {}", orbit, referenceOrbit, o2::constants::lhc::LHCMaxBunches, globalBCOffset, bunchCrossingHeader, syncOffset, timebin);
