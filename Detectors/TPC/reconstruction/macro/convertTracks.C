...
void convertTracks(TString inputBinaryFile, TString inputClusters, TString cherenkovFile, TString outputFile)
{
  ...
  while (!feof(fpInput)) {
    ...
    for (int iTrack = 0; iTrack < numTracks; iTrack++) {
      ...
      // printf("Track %d Parameters: Alpha %f, X %f, Y %f, Z %f, SinPhi %f, DzDs %f, Q/Pt %f, Number of clusters %d,
      // Fit OK %d\n", iTrack, track.Alpha, track.X, track.Y, track.Z, track.SinPhi, track.DzDs, track.QPt,
      // track.NClusters, track.FitOK);
      ...
      // ---| read cluster IDs |---
      if (size_t(track.NClusters) > ClusterIDs.size())
        ClusterIDs.resize(track.NClusters);
