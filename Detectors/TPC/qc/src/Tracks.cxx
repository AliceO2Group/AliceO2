...
void Tracks::initializeHistograms()
{
  ...
  mHist1D.emplace_back("hNClustersAfterCuts", "Number of clusters after cuts;# TPC clusters", 160, -0.5, 159.5);              //| mHist1D[1]
  ...
  mHist1D.emplace_back("hPtAfterCuts", "Transverse momentum (after cuts);p_T", logPtBinning.size() - 1, logPtBinning.data()); //| mHist1D[15]
  ...
  mHist2D.emplace_back("h2DEtaPhiAfterCuts", "Tracks in eta vs. phi (after cuts);phi;eta", 360, 0., 2 * M_PI, 400, -2., 2.);                               //| mHist2D[7]
...
bool Tracks::processTrack(const o2::tpc::TrackTPC& track)
{
  ...
  // ===| 1D histogram filling before cuts |===
  mHist1D[0].Fill(nCls);
  mHist1D[2].Fill(eta);
  ...
  // ===| 2D histogram filling before cuts |===
