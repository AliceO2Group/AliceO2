...
/// clusters are expected to be stored in separated branches per sector, the default
/// branch name is TPCCluster_n with n being the sector number
/// the base name can be configured, however not the extension '_n'
DataProcessorSpec getClusterReaderSpec(std::vector<int> const& tpcSectors, size_t fanOut)
{
  ...
  auto initFunction = [fanOut, tpcSectors](InitContext& ic) {
    ...
    auto mcbrName = ic.options().get<std::string>("clustermcbranch");
    ...
    auto processAttributes = std::make_shared<ProcessAttributes>();
    {
      ...
      for (size_t sector = 0; sector < NSectors; ++sector) {
        ...
        readers[sector] = std::make_shared<RootTreeReader>(treename.c_str(), // tree name
                                                           ...
                                                           Output{ gDataOriginTPC, "CLUSTERSIM", lane, persistency },
                                                           clusterbranchname.c_str(), // name of cluster branch
                                                           Output{ gDataOriginTPC, "CLUSTERMCLBL", lane, persistency },
  ...
  return DataProcessorSpec{ "tpc-cluster-reader",
                            ...
                            Options{
                              ...
                              { "clusterbranch", VariantType::String, "TPCCluster", { "Cluster branch" } },
                              { "clustermcbranch", VariantType::String, "TPCClusterMCTruth", { "MC label branch" } },
