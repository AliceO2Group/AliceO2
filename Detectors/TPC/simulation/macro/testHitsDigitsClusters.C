...
void testHitsDigitsClusters(int iEv = 0, std::string simFile = "", std::string digiFile = "", std::string clusFile = "",
                            std::string trackFile = "")
{
  ...
  for (auto trackObject : *arrTracks) {
    ...
    for (auto clusterObject : clCont) {
      ...
      grTrackzr->SetPoint(trackCounterXZ++, trackGlob.Z(),
                          TMath::Sqrt(trackGlob.X() * trackGlob.X() + trackGlob.Y() * trackGlob.Y()));
    }
  }
...
void drawSectorBoundaries()
{
  ...
  for (float i = 0; i < 18; ++i) {
    ...
    sectorBoundary[int(i)] =
      new TLine(80.f * std::cos(angle), 80.f * std::sin(angle), 250.f * std::cos(angle), 250.f * std::sin(angle));
