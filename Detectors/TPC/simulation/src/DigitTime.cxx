...
void DigitTime::setDigit(int eventID, size_t hitID, int cru, int row, int pad, float charge)
{
  /// Check whether the container at this spot already contains an entry
  DigitRow* result = mRows[row].get();
  if (result != nullptr) {
    mRows[row]->setDigit(eventID, hitID, pad, charge);
  } else {
...
void DigitTime::fillOutputContainer(std::vector<o2::TPC::Digit>* output,
                                    o2::dataformats::MCTruthContainer<o2::MCCompLabel>& mcTruth,
                                    std::vector<o2::TPC::DigitMCMetaData>* debug, int cru, int timeBin,
                                    float commonMode)
{
  for (auto& aRow : mRows) {
    if (aRow == nullptr)
