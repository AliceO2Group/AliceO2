...
void DigitCRU::setDigit(int eventID, size_t hitID, int timeBin, int row, int pad, float charge)
{
  mEffectiveTimeBin = timeBin - mFirstTimeBin;
  if (mEffectiveTimeBin < 0.) {
    LOG(FATAL) << "TPC DigitCRU buffer misaligned ";
    LOG(DEBUG) << "for event " << eventID << " hit " << hitID << " CRU " << mCRU << " TimeBin " << timeBin
               << " First TimeBin " << mFirstTimeBin << " Row " << row << " Pad " << pad;
  ...
  if (result != nullptr) {
    mTimeBins[mEffectiveTimeBin]->setDigit(eventID, hitID, mCRU, row, pad, charge);
  } else {
    const Mapper& mapper = Mapper::instance();
    mTimeBins[mEffectiveTimeBin] =
      std::make_unique<DigitTime>(timeBin, mapper.getPadRegionInfo(CRU(mCRU).region()).getNumberOfPadRows());
    mTimeBins[mEffectiveTimeBin]->setDigit(eventID, hitID, mCRU, row, pad, charge);
  }
}

void DigitCRU::fillOutputContainer(std::vector<o2::TPC::Digit>* output,
                                   o2::dataformats::MCTruthContainer<o2::MCCompLabel>& mcTruth,
                                   std::vector<o2::TPC::DigitMCMetaData>* debug, int cru, int eventTime,
                                   bool isContinuous)
{
  ...
  for (auto& aTime : mTimeBins) {
    ...
    if ((nProcessedTimeBins + mFirstTimeBin < eventTime) || !isContinuous) {
      ...
      if (aTime == nullptr)
        continue;
      aTime->fillOutputContainer(output, mcTruth, debug, cru, aTime->getTimeBin(),
                                 mCommonModeContainer.getCommonMode(cru, aTime->getTimeBin()));
    } else
      break;
  }
