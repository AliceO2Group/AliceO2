...
class DigitCRU
{
 public:
  ...
  void fillOutputContainer(std::vector<o2::TPC::Digit>* output,
                           ...
                           std::vector<o2::TPC::DigitMCMetaData>* debug, int cru, int eventTime = 0,
...
inline DigitCRU::DigitCRU(int CRU, CommonModeContainer& commonModeCont)
  : mFirstTimeBin(0),
    mEffectiveTimeBin(0),
    mNTimeBins(500),
    mCRU(CRU),
...
inline int DigitCRU::getNentries() const
{
  int counter = 0;
  for (auto& aTime : mTimeBins) {
    if (aTime == nullptr)
