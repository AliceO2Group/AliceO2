...
/// \class DigitRow
/// This is the forth class of the intermediate Digit Containers, in which all incoming electrons from the hits are
...
inline DigitRow::DigitRow(int row, int npads) : mRow(row), mPads(npads) {}

inline void DigitRow::reset()
{
  for (auto& aPad : mPads) {
    if (aPad == nullptr)
      continue;
...
inline int DigitRow::getNentries() const
{
  int counter = 0;
  for (auto& aPad : mPads) {
    if (aPad == nullptr)
