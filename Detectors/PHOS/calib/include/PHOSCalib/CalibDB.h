...
/// \class CalibDB
/// \brief Interface to calibration data from CCDB for PHOS
/// \author Dmitri Peresunko, adopted from EMCAL (Markus Fasel)
/// \since Aug 1, 2019
///
/// Interface handling simple access to CCDB content for
/// PHOS objects. The interface allows storing and
/// reading of the common PHOS calibration objects
/// - Bad Channel Map
/// - Gain, HG/LG ratio, Time calibration
/// - Run-by-run gaincorrections
...
class CalibDB
{
 public:
  ...
  /// \brief Store calibration parameters in the CCDB
  /// \brief prmtrs calibration parameters to be stored
  /// \brief metadata Additional metadata that can be used in the query
  /// \timestart Start of the time range of the validity of the object
  /// \timeend End of the time range of the validity of the object
  void storeCalibParams(CalibParams* prmtrs, const std::map<std::string, std::string>& metadata, ULong_t timestart, ULong_t timeend);

  /// \brief Find calibration parameters in the CCDB for given timestamp
  /// \param timestamp Timestamp used in query
  /// \param metadata Additional metadata to be used in the query
  /// \throw ObjectNotFoundException if object is not found for the given timestamp
  /// \throw TypeMismatchException if object is present but type is different (CCDB corrupted)
  CalibParams* readCalibParams(ULong_t timestamp, const std::map<std::string, std::string>& metadata);
