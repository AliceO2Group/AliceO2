...
void ZDCDataReaderDPLSpec::run(ProcessingContext& pc)
{
  ...
  for (auto it = parser.begin(), end = parser.end(); it != end; ++it) {
    ...
    if (rdhPtr == nullptr) {
      ...
      if (it.data() == nullptr) {
        nErr[1]++;
        if (nErr[1] < 5) {
          LOG(warning) << "ZDCDataReaderDPLSpec::run - Null payload pointer on page " << count;
        } else if (nErr[1] == 5) {
          LOG(warning) << "ZDCDataReaderDPLSpec::run - Null payload pointer on page " << count << " suppressing further messages";
        }
      } else if (it.size() == 0) {
        nErr[2]++;
        if (nErr[2] < 5) {
          LOG(warning) << "ZDCDataReaderDPLSpec::run - No payload on page " << count;
        } else if (nErr[2] == 5) {
          LOG(warning) << "ZDCDataReaderDPLSpec::run - No payload on page " << count << " suppressing further messages";
        }
      } else {
  ...
  if (nErr[2] > 0) {
    LOG(warning) << "ZDCDataReaderDPLSpec::run - Missing RAWDataHeader occurrences " << nErr[2];
  }
