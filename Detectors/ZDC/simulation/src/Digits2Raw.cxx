...
void Digits2Raw::emptyBunches(std::bitset<3564>& bunchPattern)
{
  ...
  for (int32_t ib = 0; ib < LHCMaxBunches; ib++) {
    ...
    int32_t m1 = (ib + 1) % LHCMaxBunches;  // next bunch is colliding
    int32_t cb = ib;                        // current bunch crossing
    int32_t p1 = (ib - 1) % LHCMaxBunches;  // previous bc is colliding (position +1)
    int32_t p2 = (ib - 2) % LHCMaxBunches;  // 2 bc after collision
    int32_t p3 = (ib - 3) % LHCMaxBunches;  // 3 bc after collision
