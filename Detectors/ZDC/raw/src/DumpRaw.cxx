...
int DumpRaw::process(const EventChData ch){
  ...
  if(f.Alice_3){
    for(Int_t i=0; i<12; i++)mSignal[ih]->Fill(i-36.,Double_t(s[i]));
  }
  if(f.Alice_2){
    for(Int_t i=0; i<12; i++)mSignal[ih]->Fill(i-24.,Double_t(s[i]));
  }
  if(f.Alice_1 || f.Auto_1){
    for(Int_t i=0; i<12; i++)mSignal[ih]->Fill(i-12.,Double_t(s[i]));
  }
  if(f.Alice_0 || f.Auto_0){
    for(Int_t i=0; i<12; i++)mSignal[ih]->Fill(i+0.,Double_t(s[i]));
