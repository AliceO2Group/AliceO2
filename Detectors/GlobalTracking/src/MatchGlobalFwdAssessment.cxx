...
void GloFwdAssessment::finalizePurityAndEff()
{
  ...
  /// Purity and efficiency vs score cuts
  ...
  for (float scoreCut = mFinalizeMinCut; scoreCut <= mFinalizeMaxCut; scoreCut += scoreStep) {
    ...
    auto& hPurityInner = mPurityPtInnerVecTEff.emplace_back(std::make_unique<TEfficiency>(*TruePtProjInner, *RecoPtProjInner)); // Global Pairing Purity = N_True / N_Reco
    hPurityInner->SetNameTitle(Form("TH2GMTrackPurityInnerEtaCut_%.2f", scoreCut), Form("%.2f", scoreCut));
    ...
    auto& hPurityOuter = mPurityPtOuterVecTEff.emplace_back(std::make_unique<TEfficiency>(*TruePtProjOuter, *RecoPtProjOuter));
    hPurityOuter->SetNameTitle(Form("TH2GMTrackPurityOuterEtaCut_%.2f", scoreCut), Form("%.2f", scoreCut));
  ...
  auto first = true;
  std::string option;
  // color palette for TEfficiency plots
  ...
  auto highptBin = mPairingPtOuterVecTH1.front()->GetXaxis()->FindBin(2.25);
  ...
  for (auto& th1 : mPairingPtInnerVecTH1) {
    if (first) {
      option = "P PLC PMC";
    } else {
      option = "P SAME PLC PMC";
