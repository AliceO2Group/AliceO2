...
void  AliAlgConstraint::SetParent(const AliAlgVol* par)
{
  ...
  TString nm = GetName();
  if (nm.IsNull()) {
    if (par) SetNameTitle(par->GetSymName(),"Automatic");
    else     SetNameTitle("GLOBAL","Automatic");
  }
}
...
void AliAlgConstraint::WriteChildrenConstraints(FILE* conOut) const
{
  ...
  for (int ich=0;ich<nch;ich++) {
	...
	if (child->IsFreeDOF(ip) && child->GetParErr(ip)>=0) nContCh[ip]++;
...
void AliAlgConstraint::CheckConstraint() const
{
  ...
  printf(" chld| ");
  for (int jp=0;jp<kNDOFGeom;jp++) printf("  %3s:%3s An/Ex  |",GetDOFName(jp),IsDOFConstrained(jp) ? "ON ":"OFF");
  printf(" | ");
  ...
  printf(" | ");
  if (fParent) for (int jp=0;jp<kNDOFGeom;jp++) printf("%+.1e ",fParent->GetParVal(jp)); // parent proper corrections
  else printf(" no parent -> %s ",doJac ? "Global":"Simple");
  printf(" ! <----- %s\n",GetName());
  //
  printf(" Sig | ");
  for (int jp=0;jp<kNDOFGeom;jp++) {
...
void AliAlgConstraint::Print(const Option_t *) const
{
  ...
  printf(" | %s %s\n",GetName(),GetTitle());
  if (GetNoJacobian()) printf("!!! This is explicit constraint on sum of DOFs (no Jacobian)!!!\n");
