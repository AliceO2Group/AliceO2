...
class AliAlgDet : public TNamed
{
 public:
  ...
  //
  void setAlgSteer(AliAlgSteer* s) { mAlgSteer = s; }
  AliAlgSens* getSensor(int id) const { return (AliAlgSens*)mSensors.UncheckedAt(id); }
  AliAlgSens* getSensorByVolId(int vid) const
  {
    int sid = volID2SID(vid);
    return sid < 0 ? 0 : getSensor(sid);
  }
  AliAlgSens* getSensor(const char* symname) const { return (AliAlgSens*)mSensors.FindObject(symname); }
  AliAlgVol* getVolume(int id) const { return (AliAlgVol*)mVolumes.UncheckedAt(id); }
  AliAlgVol* getVolume(const char* symname) const { return (AliAlgVol*)mVolumes.FindObject(symname); }
  //
 ...
 protected:
  ...
  //
  // Track selection
  bool mDisabled[AliAlgAux::kNTrackTypes];         // detector disabled/enabled in the track
  bool mObligatory[AliAlgAux::kNTrackTypes];       // detector must be present in the track
  uint64_t mTrackFlagSel[AliAlgAux::kNTrackTypes]; // flag for track selection
  int mNPointsSel[AliAlgAux::kNTrackTypes];        // min number of points to require
  //
  ...
  TObjArray mPointsPool;  //! pool of aligment points
                          //
  AliAlgSteer* mAlgSteer; // pointer to alignment steering object
  //
