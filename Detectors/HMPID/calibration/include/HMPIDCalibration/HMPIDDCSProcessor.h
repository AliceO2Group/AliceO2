...
class HMPIDDCSProcessor
{
 ...
 public:
  ...
  // Fill entries of DPs==================================================
  void fillChPressure(const DPCOM& dpcom); // fill element[0-6] in chamber-pressure vector
  ...
  // functions return true if there is an entry in the array of DPCOM-vectors at the given element
  ...
  // loops over all the arrays of DPCOM-vectors, and calls the relevant fill()-methods above
  void finalize();
  ...
  bool evalCorrFactor(double dRefArgon, double dCellArgon, double dRefFreon, double dCellFreon, double photEn, int i);
  double dpVector2Double(std::vector<DPCOM> dpVec, const char* dpString, int i);
  double calculateWaveLength(int i);

  //===== help-functions ================================================================================
  void setStartValidity(long t) { mStartValidity = t; } // used in HMPIDDCSDataProcessorSpec
  ...
  TimeStampType getMaxTime(const std::vector<DPCOM> dps)
  {
    ...
    for (const auto& dp : dps) {
      ...
      // check if tme of DP is greater (i.e. later) than previously latest fetched DP:
      const auto time = dp.data.get_epoch_time();
  ...
  void checkEntries(const std::vector<TF1> arQthresh, const std::vector<TF1> arrayNmean)
  {
    int cnt = 0;
    ...
    for (int iCh = 0; iCh < 7; ++iCh) {
      for (int iRad = 0; iRad < 3; iRad += 2) {
        ...
        const char* strCcdbinOut = (arrayNmean.at(6 * iCh + 2 * iRad + 1)).GetName();
 ...
 private:
  ...
  double aCorrFactor[30] = {0.937575212, 0.93805688, 0.938527113, 0.938986068, 0.939433897, 0.939870746, 0.940296755, 0.94071206,
                            0.941116795, 0.941511085, 0.941895054, 0.942268821, 0.942632502, 0.942986208, 0.943330047, 0.943664126,
                            0.943988544, 0.944303401, 0.944608794, 0.944904814, 0.945191552, 0.945469097, 0.945737533, 0.945996945,
                            0.946247412, 0.946489015, 0.94672183, 0.946945933, 0.947161396, 0.947368291};
  ...
  // Temperatures
  std::size_t indexChTemp = 6;
  std::size_t indexRadTemp = 21;
