...
namespace o2::hmpid
{
  ...
  struct TimeRange {
    ...
    void fillInTemperature(const DPCOM& dpcom);			// fill element[0-20] in tempIn vector
    void fillOutTemperature(const DPCOM& dpcom); 			// fill element[0-20] in tempOut vector
  ...
  private:
    ...
    std::vector<DPCOM> dpVecEnvPress; 	 // environment-pressure vector
    TF1* mEnvPressTF;   

    // ch pressure 
    std::vector<DPCOM> dpVecChPress[7];  //  chamber-pressure vector [0..6]
    int cntChPressure = 0; 	 // cnt chamber-pressure entries in element iCh[0..6] 
    std::vector<TF1*> mChamPressTF;//[7];   

    int cntTin = 0, cntTOut = 0; 	  // cnt tempereature entries in element i[0..20]; i = 3*iCh+iSec
