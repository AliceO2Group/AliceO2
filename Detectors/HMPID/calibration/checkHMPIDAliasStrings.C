  ...
  std::vector<std::string> tempInString, tempOutString, chamberPressureString, highVoltageString;
   ...
   aliases.push_back("HMP_DET/HMP_ENV/HMP_ENV_PENV.actual.value"); // environment pressure 
	...
	temp = Form("HMP_DET/HMP_INFR/HMP_INFR_TRANPLANT/HMP_INFR_TRANPLANT_MEASURE.mesure%i.",i);
        waveLenghtString.push_back(temp+wLen);processIR(temp+wLen);
        argonReferenceString.push_back(temp+argRef);processIR(temp+argRef);
        argonCellString.push_back(temp+argCell);processIR(temp+argCell);
        c6f14CellString.push_back(temp+c6f14Cell);processIR(temp+c6f14Cell);
       c6f14ReferenceString.push_back(temp+c6f14Ref);processIR(temp+c6f14Ref);
