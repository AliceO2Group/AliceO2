...
// if the string of the dp contains the Transparency-specifier "HMP_TRANPLANT_MEASURE_"
  ...
  if (type == DeliveryType::DPVAL_DOUBLE) {
    ...
    if (chNum < 7 && chNum >= 0) {
      if (radNum < 3 && radNum >= 0) {
        ...
        LOG(debug) << "Radiator Number out of range for TempOut DP : " << radNum;
      }
    } else {
...
double HMPIDDCSProcessor::procTrans()
{
  for (int i = 0; i < 30; i++) {
    ...
    if (photEn < o2::hmpid::Param::ePhotMin() || photEn > o2::hmpid::Param::ePhotMax()) {
  ...
  if (eMean < o2::hmpid::Param::ePhotMin() || eMean > o2::hmpid::Param::ePhotMax()) {
...
double HMPIDDCSProcessor::calculateWaveLength(int i)
{
  ...
  if (dp.id.get_type() == DeliveryType::DPVAL_DOUBLE) {
    ...
    LOG(info) << Form("Not correct datatype for HMP_TRANPLANT_MEASURE_%i_WAVELENGTH  --> Default E mean used!", i);
    return defaultEMean();
  }

  if (lambda < 150. || lambda > 230.) {
    LOG(info) << Form("Wrong value for HMP_TRANPLANT_MEASURE_%i_WAVELENGTH  --> Default E mean used!", i);
    return defaultEMean();
  }
...
double HMPIDDCSProcessor::dpVector2Double(std::vector<DPCOM> dpVec, const char* dpString, int i)
{

  double dpVal;
  if (dpVec.size() == 0) {
    LOG(info) << Form("No Data Point values for HMP_TRANPLANT_MEASURE_%s,%i  ---> Default E mean used!", dpString, i);
    return defaultEMean();
  }
  ...
  if (dp.id.get_type() == DeliveryType::DPVAL_DOUBLE) {
    ...
    LOG(info) << Form("Not correct datatype for HMP_TRANPLANT_MEASURE_%s,%i  -----> Default E mean used!", dpString, i);
    return defaultEMean();
  }
  return dpVal;
}

bool HMPIDDCSProcessor::evalCorrFactor(double dRefArgon, double dCellArgon, double dRefFreon, double dCellFreon, double dPhotEn, int i)
{
  ...
  if (dRefFreon * dRefArgon > 0) {
    aTransRad = TMath::Power((dCellFreon / dRefFreon) / (dCellArgon / dRefArgon) * aCorrFactor[i], aConvFactor);
  } else {
...
std::unique_ptr<TF1> HMPIDDCSProcessor::finalizeEnvPressure()
{
  ...
  if (dpVecEnv.size() != 0) {
    ...
    if (cntEnvPressure == 1) {
      ...
      pEnv.reset(new TF1("Penv", Form("%f", yP), envPrFirstTime, envPrLastTime));
    } else {
      // envPrLastTime -= envPrFirstTime;
      // envPrFirstTime = 0;
...
std::unique_ptr<TF1> HMPIDDCSProcessor::finalizeChPressure(int iCh)
{
  ...
  if (dpVecCh[iCh].size() != 0) {
    ...
    if (cntChPressure == 1) {
      ...
      (pCh).reset(new TF1(Form("P%i", iCh), Form("%f", yP), chPrFirstTime, chPrLastTime));
      pArrCh[iCh] = *(pCh.get());
    } else {
      (pCh).reset(new TF1(Form("P%i", iCh), "[0] + x*[1]", chPrFirstTime, chPrLastTime));
...
void HMPIDDCSProcessor::finalizeTempOut(int iCh, int iRad)
{
  if (dpVecTempOut[3 * iCh + iRad].size() != 0) {
    ...
    std::vector<TF1>::iterator itTout = arNmean.begin() + 6 * iCh + 2 * iRad + 1;
...
std::unique_ptr<TF1> HMPIDDCSProcessor::finalizeHv(int iCh, int iSec)
{
  ...
  if (dpVecHV[3 * iCh + iSec].size() != 0) {
    ...
    if (cntHV == 1) {
      ...
      (pHvTF).reset(new TF1(Form("HV%i_%i", iCh, iSec), Form("%f", yP), hvFirstTime, hvLastTime));
      pArrHv[3 * iCh + iSec] = *(pHvTF.get());
    } else {
      (pHvTF).reset(new TF1(Form("HV%i_%i", iCh, iSec), "[0]+x*[1]", hvFirstTime, hvLastTime));
...
void HMPIDDCSProcessor::finalize()
{
  ...
  for (int iCh = 0; iCh < 7; iCh++) {
    ...
    for (int iSec = 0; iSec < 6; iSec++) {
      ...
      if (pEnv != nullptr && pChPres != nullptr && pHV != nullptr) {
        ...
        const char* fFormula = "3*10^(3.01e-3*%s - 4.72)+170745848*exp(-(%s+%s)*0.0162012)";
        ...
        pQthre.reset(new TF1(Form("HMP_QthreC%iS%i", iCh, iSec), fName, minTime, maxTime));
        pQthre->SetTitle(Form("Charge-Threshold Ch%iSec%i; Time [mS]; Threshold ", iCh, iSec));
  ...
  pPhotMean.reset(new TF1("HMP_PhotEmean", Form("%f", eMean), 0, mTimeEMean.last - mTimeEMean.first));
  ...
  o2::calibration::Utils::prepareCCDBobjectInfo(arNmean, mccdbRefInfo, "HMP/Calib/RefIndex", md, mStartValidity, o2::ccdb::CcdbObjectInfo::INFINITE_TIMESTAMP);
  LOG(info) << "mStartValidity = " << mStartValidity;
  // charge threshold;
  o2::calibration::Utils::prepareCCDBobjectInfo(arQthre, mccdbChargeInfo, "HMP/Calib/ChargeCut", md, mStartValidity, o2::ccdb::CcdbObjectInfo::INFINITE_TIMESTAMP);
}
