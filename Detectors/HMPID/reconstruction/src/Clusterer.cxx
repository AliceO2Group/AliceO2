...
//  Returns: none
  ...
  for(int iCh = Param::kMinCh; iCh<= Param::kMaxCh; iCh++){                                //chambers loop 
    ...
    for(int iDig = 0; iDig < mDigs->size(); iDig++) {                                   //digits loop for current chamber
      ...
      if(pUsedDig == -1) continue;                      //this digit is already taken in FormClu(), go after next digit
...
//   Returns: none   
  ...
  if(padChX > Param::kMinPx){cx[cnt]=padChX-1; cy[cnt]=padChY  ; cnt++;}       //left
  if(padChX < Param::kMaxPcx){cx[cnt]=padChX+1; cy[cnt]=padChY  ; cnt++;}       //right
  if(padChY > Param::kMinPy){cx[cnt]=padChX  ; cy[cnt]=padChY-1; cnt++;}       //down
  if(padChY < Param::kMaxPcy){cx[cnt]=padChX  ; cy[cnt]=padChY+1; cnt++;}       //up
  
  for (int i = 0; i < cnt; i++){//neighbours loop
    pDig = UseDig(cx[i], cy[i], pDigMap);
    if(pDig != -1) FormClu(pClu, pDig, pDigList, pDigMap);   //check if this neighbour pad fired and mark it as taken  
  }//neighbours loop  
}//FormClu()
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
