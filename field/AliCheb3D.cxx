...
void AliCheb3D::PrepareBoundaries(const Float_t  *bmin, const Float_t  *bmax)
{
  ...
  for (int i=3;i--;) {
    ...
    if (fBScale[i]<=0) { 
      fLogger->Fatal(MESSAGE_ORIGIN,"Boundaries for %d-th dimension are not increasing: %+.4e %+.4e\nStop\n",i,fBMin[i],fBMax[i]);
    }
...
void AliCheb3D::LoadData(FILE* stream)
{
  ...
  //
  if (!stream) { fLogger->Fatal(MESSAGE_ORIGIN,"No stream provided.\nStop");}
  TString buffs;
  Clear();
  AliCheb3DCalc::ReadLine(buffs,stream);
  if (!buffs.BeginsWith("START")) {fLogger->Fatal(MESSAGE_ORIGIN,"Expected: \"START <fit_name>\", found \"%s\"\nStop\n",buffs.Data());}
  SetName(buffs.Data()+buffs.First(' ')+1);
  //
  AliCheb3DCalc::ReadLine(buffs,stream); // N output dimensions
  fDimOut = buffs.Atoi(); 
  if (fDimOut<1) {fLogger->Fatal(MESSAGE_ORIGIN,"Expected: '<number_of_output_dimensions>', found \"%s\"\nStop\n",buffs.Data());}
  //
  SetDimOut(fDimOut);
  //
  AliCheb3DCalc::ReadLine(buffs,stream); // Interpolation abs. precision
  fPrec = buffs.Atof();
  if (fPrec<=0) {fLogger->Fatal(MESSAGE_ORIGIN,"Expected: '<abs.precision>', found \"%s\"\nStop\n",buffs.Data());}
  //
  ...
  if (!buffs.BeginsWith("END") || !buffs.Contains(GetName())) {
    fLogger->Fatal(MESSAGE_ORIGIN,"Expected \"END %s\", found \"%s\".\nStop\n",GetName(),buffs.Data());
  }
  //
}
