...
fBegSegYDip(0),fNSegYDip(0),fBegSegXDip(0),fNSegXDip(0),fSegIDDip(0),fMinZDip(1.e6),fMaxZDip(-1.e6),fParamsDip(0), fLogger(FairLogger::GetLogger())
//
{
  // default constructor
}
...
void AliMagWrapCheb::GetTPCIntCyl(const Double_t *rphiz, Double_t *b) const
{
  ...
  if (id>=fNParamsTPC) {
   // AliError(Form("Wrong TPCParam segment %d",id));
    fLogger->Error(MESSAGE_ORIGIN,"Wrong TPCParam segment %d",id);
    b[0] = b[1] = b[2] = 0;
    return;
  }
...
void AliMagWrapCheb::GetTPCRatIntCyl(const Double_t *rphiz, Double_t *b) const
{
  ...
  //  AliError(Form("Wrong TPCRatParam segment %d",id));
      fLogger->Error(MESSAGE_ORIGIN,"Wrong TPCRatParam segment %d",id);
    b[0] = b[1] = b[2] = 0;
    return;
  }
