  ...
  fLogger(FairLogger::GetLogger())
{
  ...
  SetTitle(title);
  if(integ<0 || integ > 2) {
    fLogger->Warning(MESSAGE_ORIGIN,"Invalid magnetic field flag: %5d; Helix tracking chosen instead",integ);
    fInteg = 2;
  }
  ...
  else fLogger->Fatal(MESSAGE_ORIGIN,"Unknown field identifier %d is requested\n",fMapType);
  //
...
{
  if (fMeasuredMap) {
    fLogger->Fatal(MESSAGE_ORIGIN,"Field data %s are already loaded from %s\n",GetParamName(),GetDataFileName());
  }
  //
  char* fname = gSystem->ExpandPathName(GetDataFileName());
  TFile* file = TFile::Open(fname);
  if (!file){
    fLogger->Fatal(MESSAGE_ORIGIN,"Failed to open magnetic field data file %s\n",fname);
  }
  //
  fMeasuredMap = dynamic_cast<AliMagWrapCheb*>(file->Get(GetParamName()));
  if (!fMeasuredMap) {
    fLogger->Fatal(MESSAGE_ORIGIN,"Did not find field %s in %s\n",GetParamName(),fname);
  }
  file->Close();
  delete file;
  return kTRUE;
				 ...
				 Float_t beamenergy, const Char_t *beamtype, const Char_t *path) 
{
  ...
  if (sclDip!=0 && map!=k5kGUniform) {
    if ( (l3Cur<=zero) || ((convention==kConvLHC && l3Pol!=diPol) || (convention==kConvDCS2008 && l3Pol==diPol)) ) { 
      FairLogger::GetLogger()->Fatal(MESSAGE_ORIGIN,"Wrong combination for L3/Dipole polarities (%c/%c) for convention %d",
...
//  else AliInfoGeneral("AliMagF",Form("Assume no LHC magnet field for the beam type %s, ",beamtype));
  else FairLogger::GetLogger()->Info(MESSAGE_ORIGIN,"Assume no LHC magnet field for the beam type %s,",beamtype);
  char ttl[80];
...
void AliMagF::Print(Option_t *opt) const
{
  ...
  fLogger->Info(MESSAGE_ORIGIN,"%s:%s",GetName(),GetTitle());
  fLogger->Info(MESSAGE_ORIGIN,"Solenoid (%+.2f*)%.0f kG, Dipole %s (%+.2f) %s",
	       GetFactorSol(),(fMapType==k5kG||fMapType==k5kGUniform)?5.:2.,
	       fDipoleOFF ? "OFF":"ON",GetFactorDip(),fMapType==k5kGUniform?" |Constant Field!":"");
  if (opts.Contains("a")) {
    fLogger->Info(MESSAGE_ORIGIN,"Machine B fields for %s beam (%.0f GeV): QGrad: %.4f Dipole: %.4f",
		 GetBeamTypeText(),
		 fBeamEnergy,fQuadGradient,fDipoleField);
    fLogger->Info(MESSAGE_ORIGIN,"Uses %s of %s",GetParamName(),GetDataFileName());
  }
}
