...
class AliGPUDisplayBackendGlfw : public AliGPUDisplayBackend {};
#endif
...
int AliGPUTPCO2Interface::Initialize(const AliGPUO2InterfaceConfiguration& config, std::unique_ptr<TPCFastTransform>&& fastTrans)
{
	if (fInitialized) return(1);
	mConfig.reset(new AliGPUO2InterfaceConfiguration(config));
...
int AliGPUTPCO2Interface::Initialize(const char* options, std::unique_ptr<TPCFastTransform>&& fastTrans)
{
	...
	if (options && *options)
	{
		...
		while (optPtr && *optPtr)
		{
			...
			{
				mDisplayBackend.reset(new AliGPUDisplayBackendGlfw);
