...
void GPUChainTracking::LoadClusterErrors()
{
...
void GPUChainTracking::SetTRDGeometry(const o2::trd::TRDGeometryFlat& geo)
{
...
#pragma omp parallel for num_threads(doGPU ? 1 : GetDeviceProcessingSettings().nThreads)
	...
	for (unsigned int iSlice = 0;iSlice < NSLICES;iSlice++)
	{
		...
		if (doGPU) runKernel<GPUTPCStartHitsSorter>({BlockCount(), ThreadCount(), useStream}, &timerTPCtracking[iSlice][4], {iSlice});
...
int GPUChainTracking::DoTRDGPUTracking()
{
	...
	TrackerShadow.SetGeometry((GPUTRDGeometry*) mFlatObjectsDevice.fTrdGeometry);
...
int GPUChainTracking::HelperReadEvent(int iSlice, int threadId, GPUReconstructionHelpers::helperParam* par)
{
