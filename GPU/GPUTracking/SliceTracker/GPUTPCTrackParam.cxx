...
MEM_CLASS_PRE()
GPUd() void MEM_LG(GPUTPCTrackParam)::GetDCAPoint(float x, float y, float z,
...
GPUd() bool MEM_LG(GPUTPCTrackParam)::TransportToX(float x, GPUTPCTrackLinearisation &t0, float Bz, float maxSinPhi, float *DL)
{
	...
	fC[0] = c00 + h2 * h2 * c22 + h4 * h4 * c44 + 2 * (h2 * c20 + h4 * c40 + h2 * h4 * c42);
...
GPUd() bool MEM_LG(GPUTPCTrackParam)::TransportToX(float x, float sinPhi0, float cosPhi0, float Bz, float maxSinPhi)
{
	...
	fC[0] = CAMath::Max(c00, c00 + h2 * h2 * c22 + h4 * h4 * c44 + 2 * (h2 * c20 + h4 * c40 + h2 * h4 * c42));
...
//*
MEM_CLASS_PRE()
GPUd() float MEM_LG(GPUTPCTrackParam)::BetheBlochGeant(float bg2,
                                                            float kp0,
                                                            float kp1,
                                                            float kp2,
                                                            float kp3,
...
GPUd() bool MEM_LG(GPUTPCTrackParam)::Filter(float y, float z, float err2Y, float err2Z, float maxSinPhi, bool paramOnly)
{
	...
	float c00 = fC[0], c11 = fC[2], c20 = fC[3], c31 = fC[7], c40 = fC[10];
