...
class AliGPUTPCTracker : public AliGPUProcessor
{
  public:
	...
	MEM_CLASS_PRE2() void InitializeRows( const MEM_LG2(AliGPUParam) *param ) { fData.InitializeRows(*param); }
	...
	GPUhd() MakeType(const MEM_LG(AliGPUParam)&) Param() const { return *mCAParam; }
	GPUhd() MakeType(const MEM_LG(AliGPUParam)*) pParam() const { return mCAParam; }
