...
/**
 * @class AliGPUTPCMCPoint
 * store MC point information for AliGPUTPCPerformance
 */
class AliGPUTPCMCPoint
{
  public:
	...
	static bool Compare(const AliGPUTPCMCPoint &p1, const AliGPUTPCMCPoint &p2)
	{
	...
	static bool CompareSlice(const AliGPUTPCMCPoint &p, int slice)
	{
		return (p.ISlice() < slice);
	}

	static bool CompareX(const AliGPUTPCMCPoint &p, float X)
	{
		return (p.Sx() < X);
	}
