// @(#) $Id: GPUTPCTrackletConstructor.cxx 27042 2008-07-02 12:06:02Z richterm $
// **************************************************************************
...
MEM_CLASS_PRE23()
GPUd() void GPUTPCTrackletConstructor::StoreTracklet(int /*nBlocks*/, int /*nThreads*/, int /*iBlock*/, int /*iThread*/,
                                                          GPUsharedref() MEM_LOCAL(GPUTPCSharedMemory) & s, GPUTPCThreadMemory &r, GPUconstantref() MEM_LG2(GPUTPCTracker) & tracker, MEM_LG3(GPUTPCTrackParam) & tParam)
{
	...
	if (r.fNHits > 0)
	{
		...
		for (int iRow = r.fFirstRow; iRow <= r.fLastRow; iRow++)
		{
			...
			if (ih != CALINK_INVAL)
			{
				MAKESharedRef(GPUTPCRow, row, tracker.Row(iRow), s.fRows[iRow]);
...
MEM_CLASS_PRE2()
GPUd() void GPUTPCTrackletConstructor::UpdateTracklet(int /*nBlocks*/, int /*nThreads*/, int /*iBlock*/, int /*iThread*/,
                                                           GPUsharedref() MEM_LOCAL(GPUTPCSharedMemory) & s, GPUTPCThreadMemory &r, GPUconstantref() MEM_CONSTANT(GPUTPCTracker) & tracker, MEM_LG2(GPUTPCTrackParam) & tParam, int iRow)
{
	// reconstruction of tracklets, tracklets update step
#ifndef EXTERN_ROW_HITS
	GPUTPCTracklet &tracklet = tracker.Tracklets()[r.fItr];
...
GPUd() void GPUTPCTrackletConstructor::DoTracklet(GPUconstantref() MEM_CONSTANT(GPUTPCTracker) & tracker, GPUsharedref() GPUTPCTrackletConstructor::MEM_LOCAL(GPUTPCSharedMemory) & s, GPUTPCThreadMemory &r)
{
	...
	if (r.fGo)
	{
		GPUTPCHitId id = tracker.TrackletStartHits()[r.fItr];
...
template <> GPUd() void GPUTPCTrackletConstructor::Thread<0>(int nBlocks, int nThreads, int iBlock, int iThread, GPUsharedref() MEM_LOCAL(GPUTPCSharedMemory) &sMem, workerType &tracker)
{
...
template <> GPUd() void GPUTPCTrackletConstructor::Thread<1>(int nBlocks, int nThreads, int iBlock, int iThread, GPUsharedref() MEM_LOCAL(GPUTPCSharedMemory) &sMem, workerType &tracker0)
{
#ifdef GPUCA_GPUCODE
