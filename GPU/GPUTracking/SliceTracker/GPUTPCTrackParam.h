...
class GPUTPCTrackParam
{
  public:
	...
	GPUd() MakeType(const MEM_LG(GPUTPCBaseTrackParam) &) GetParam() const { return fParam; }
	GPUd() void SetParam(const MEM_LG(GPUTPCBaseTrackParam) & v) { fParam = v; }
	GPUd() void InitParam();
	...
	GPUd() bool TransportToX(float x, GPUTPCTrackLinearisation &t0,
	...
	GPUd() bool TransportToXWithMaterial(float x, GPUTPCTrackLinearisation &t0,
	                                     GPUTPCTrackFitParam &par, float Bz, float maxSinPhi = GPUCA_MAX_SIN_PHI);
