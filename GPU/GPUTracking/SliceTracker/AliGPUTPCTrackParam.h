...
class AliGPUTPCTrackLinearisation;

/**
 * @class AliGPUTPCTrackParam
 *
 * AliGPUTPCTrackParam class describes the track parametrisation
 * which is used by the AliGPUTPCTracker slice tracker.
 *
 */
MEM_CLASS_PRE()
class AliGPUTPCTrackParam
{
  public:
	struct AliGPUTPCTrackFitParam
	{
		float fBethe, fE, fTheta2, fEP2, fSigmadE2, fK22, fK33, fK43, fK44; // parameters
	};

	GPUd() MakeType(const MEM_LG(AliGPUTPCBaseTrackParam) &) GetParam() const { return fParam; }
	GPUd() void SetParam(const MEM_LG(AliGPUTPCBaseTrackParam) & v) { fParam = v; }
	GPUd() void InitParam();
	...
	GPUd() float GetDist2(const AliGPUTPCTrackParam &t) const;
	GPUd() float GetDistXZ2(const AliGPUTPCTrackParam &t) const;
	...
	GPUd() bool TransportToX(float x, AliGPUTPCTrackLinearisation &t0,
	                         float Bz, float maxSinPhi = GPUCA_MAX_SIN_PHI, float *DL = 0);

	GPUd() bool TransportToX(float x, float sinPhi0, float cosPhi0, float Bz, float maxSinPhi = GPUCA_MAX_SIN_PHI);

	GPUd() bool TransportToXWithMaterial(float x, AliGPUTPCTrackLinearisation &t0,
	                                     AliGPUTPCTrackFitParam &par, float Bz, float maxSinPhi = GPUCA_MAX_SIN_PHI);
	...
	GPUd() bool Rotate(float alpha, AliGPUTPCTrackLinearisation &t0, float maxSinPhi = GPUCA_MAX_SIN_PHI);
...
GPUd() MEM_CLASS_PRE() inline void MEM_LG(AliGPUTPCTrackParam)::InitParam()
{
