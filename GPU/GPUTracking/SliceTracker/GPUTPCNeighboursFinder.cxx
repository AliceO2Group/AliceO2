...
#define MaxTotal MaxShared + MaxGlobal
...
#if MaxGlobal > 0
  constexpr int tmpUnrollGlobal = UnrollGlobal; // constexpr needed for CUDA
#endif
#if MaxShared > 0
  constexpr int tmpMaxShared = MaxShared;
#endif
  ...
  for (int ih = iThread; ih < s.mNHits; ih += nThreads) {
    ...
    if (N4 < Nrest) {
      N4 += tmpUnrollGlobal;
#pragma unroll(tmpUnrollGlobal - 1)
