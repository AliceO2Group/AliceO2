...
GPUd() bool GPUTPCGMTrackParam::Fit(GPUTPCGMMerger* GPUrestrict() merger, int32_t iTrk, GPUTPCGMMergedTrackHit* GPUrestrict() clusters, int32_t& GPUrestrict() N, int32_t& GPUrestrict() NTolerated, float& GPUrestrict() Alpha, GPUTPCGMMergedTrack& GPUrestrict() track, bool rebuilt)
{
  ...
  for (int32_t iWay = rebuilt ? nWays - 1 : 0; iWay < nWays; iWay++) { // TODO DR: Unrolling has no performance improvement on GPU, why?
    ...
    for (int32_t ihit = ihitStart; ihit >= 0 && ihit < maxN; ihit += wayDirection) {
      ...
      // clang-format on
      if (rebuilt && merger->ClusterCandidates()[(iTrk * GPUTPCGeometry::NROWS + clusters[ihit].row) * param.rec.tpc.rebuildTrackInFitClusterCandidates + 0].id > 0) {
        while (true) {
        ...
        const auto& interHit = merger->ClusterCandidates()[(iTrk * GPUTPCGeometry::NROWS + clusters[ihit].row) * param.rec.tpc.rebuildTrackInFitClusterCandidates + 0];
        if (interHit.id == 1) {
          nMissed++;
          nMissed2++;
          continue;
                ...
                if (dy * dy < sy2 && dz * dz < sz2) {
                  ...
                  if (ok) {
                    ...
                    if (insert < param.rec.tpc.rebuildTrackInFitClusterCandidates) {
                      ...
                      merger->ClusterCandidates()[(iTrk * GPUTPCGeometry::NROWS + cluster.row) * param.rec.tpc.rebuildTrackInFitClusterCandidates + insert] = {.id = (uint32_t)(idOffset + ids[ih] + 2), .row = cluster.row, .sector = cluster.sector, .error = err};
