...
GPUd() bool GPUTPCGMTrackParam::Fit(const GPUTPCGMMerger *merger, int iTrk, GPUTPCGMMergedTrackHit *clusters, int &N, int &NTolerated, float &Alpha, int attempt, float maxSinPhi, GPUTPCOuterParam *outerParam)
{
	...
	for(;ihit >= 0 && ihit<maxN;ihit += wayDirection)
	{
		...
		CADEBUG(if ((unsigned int) merger->GetTrackingChain()->mIOPtrs.nMCLabelsTPC > clusters[ihit].fNum))
		CADEBUG({printf(" MC:"); for (int i = 0;i < 3;i++) {int mcId = merger->GetTrackingChain()->mIOPtrs.mcLabelsTPC[clusters[ihit].fNum].fClusterID[i].fMCID; if (mcId >= 0) printf(" %d", mcId);}}printf("\n"));
		if ((param.rec.RejectMode > 0 && nMissed >= param.rec.RejectMode) || nMissed2 >= GPUCA_MERGER_MAXN_MISSED_HARD || clusters[ihit].fState & GPUTPCGMMergedTrackHit::flagReject)
		{
			CADEBUG(printf("\tSkipping hit, %d hits rejected, flag %X\n", nMissed, (int) clusters[ihit].fState));
			if (iWay + 2 >= nWays && !(clusters[ihit].fState & GPUTPCGMMergedTrackHit::flagReject)) clusters[ihit].fState |= GPUTPCGMMergedTrackHit::flagRejectErr;
			continue;
		}
		...
		{
			if (allowModification) MarkClusters(clusters, ihitMergeFirst, ihit, wayDirection, GPUTPCGMMergedTrackHit::flagRejectDistance);
			nMissed++;
			nMissed2++;
		}
	...
	if (ihit + wayDirection >= 0 && ihit + wayDirection < maxN && clusters[ihit].fRow == clusters[ihit + wayDirection].fRow && clusters[ihit].fSlice == clusters[ihit + wayDirection].fSlice && clusters[ihit].fLeg == clusters[ihit + wayDirection].fLeg)
	{
		...
		while(true)
		{
			...
			if (noReject == 0 && (dy * dy > maxDistY || dz * dz > maxDistZ))
			{
				...
				if (rejectChi2) clusters[ihit].fState |= GPUTPCGMMergedTrackHit::flagRejectDistance;
			}
			else
			{
...
GPUd() void GPUTPCGMTrackParam::AttachClusters(const GPUTPCGMMerger *Merger, int slice, int iRow, int iTrack, bool goodLeg)
{
...
GPUd() void GPUTPCGMTrackParam::AttachClusters(const GPUTPCGMMerger *Merger, int slice, int iRow, int iTrack, bool goodLeg, float Y, float Z)
{
	...
	for (int k = 0;k <= nz;k++)
	{
		...
		for (unsigned int ih = hitFst;ih < hitLst;ih++)
		{
			...
			if (*weight & GPUTPCGMMerger::attachGood) continue;
			...
			if (dy * dy < sy2 && dz * dz < sz2)
			{
				...
				if (goodLeg) myWeight |= GPUTPCGMMerger::attachGoodLeg;
...
GPUd() void GPUTPCGMTrackParam::RefitTrack(GPUTPCGMMergedTrack &track, int iTrk, const GPUTPCGMMerger* merger, GPUTPCGMMergedTrackHit* clusters)
{
	...
	for (int attempt = 0;;)
	{
		...
		{
			for (unsigned int i = 0;i < track.NClusters();i++) clusters[track.FirstClusterRef() + i].fState &= GPUTPCGMMergedTrackHit::hwcfFlags;
