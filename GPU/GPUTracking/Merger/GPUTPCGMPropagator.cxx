// $Id: GPUTPCGMPropagator.cxx 41769 2010-06-16 13:58:00Z sgorbuno $
// **************************************************************************
...
GPUd() int GPUTPCGMPropagator::RotateToAlpha(float newAlpha)
{
	...
	GPUTPCGMPhysicalTrackModel t0 = fT0;
...
GPUd() int GPUTPCGMPropagator::PropagateToXAlpha(float posX, float posAlpha, bool inFlyDirection)
{
	...
	GPUTPCGMPhysicalTrackModel t0e(fT0);
	float dLp = 0;
...
GPUd() void GPUTPCGMPropagator::GetErr2(float& err2Y, float& err2Z, const GPUParam &param, float posZ, int iRow, short clusterState) const
{
	...
	if (clusterState & GPUTPCGMMergedTrackHit::flagEdge) {err2Y += 0.35f;err2Z += 0.15f;}
	if (clusterState & GPUTPCGMMergedTrackHit::flagSingle) {err2Y += 0.2f;err2Z += 0.2f;}
	if (clusterState & (GPUTPCGMMergedTrackHit::flagSplitPad | GPUTPCGMMergedTrackHit::flagShared | GPUTPCGMMergedTrackHit::flagSingle)) {err2Y += 0.03f;err2Y *= 3;}
	if (clusterState & (GPUTPCGMMergedTrackHit::flagSplitTime | GPUTPCGMMergedTrackHit::flagShared | GPUTPCGMMergedTrackHit::flagSingle)) {err2Z += 0.03f;err2Z *= 3;}
