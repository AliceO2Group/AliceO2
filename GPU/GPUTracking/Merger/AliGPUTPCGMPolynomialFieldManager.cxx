...
int AliGPUTPCGMPolynomialFieldManager::GetPolynomialField( StoredField_t fieldType, float nominalFieldkG, AliGPUTPCGMPolynomialField &field )
{
  //
...
int AliGPUTPCGMPolynomialFieldManager::GetPolynomialField( float nominalFieldkG, AliGPUTPCGMPolynomialField &field )
{
  //
...
int AliGPUTPCGMPolynomialFieldManager::GetPolynomialField( AliGPUTPCGMPolynomialField &field )
{
  //
...
int AliGPUTPCGMPolynomialFieldManager::FitFieldTpc( AliMagF* inputFld, AliGPUTPCGMPolynomialField &polyField, double step  )
{
  ...
  const int nRows = AliGPUTPCGeometry::GetNRows();

  double xMin = AliGPUTPCGeometry::Row2X(0);
  double xMax = AliGPUTPCGeometry::Row2X(nRows-1);
  double rMin = xMin;
  ...
  for( int sector=0; sector<18; sector++){
    ...
    for( double al=alMin; al<alMax; al+=dA ){
      ...
      for( int row=0; row<AliGPUTPCGeometry::GetNRows(); row++){
	double xl = AliGPUTPCGeometry::Row2X(row);
	double yl = xl*tg;
...
int AliGPUTPCGMPolynomialFieldManager::FitFieldTrd( AliMagF* inputFld, AliGPUTPCGMPolynomialField &polyField, double step )
{
  ...
  double zMax = 751.0/2.;
  double zMin = -zMax;
  double xMin = AliGPUTPCGeometry::Row2X( AliGPUTPCGeometry::GetNRows()-1 );
  ...
  const int M = AliGPUTPCGMPolynomialField::fkTrdM;
  AliHLTTPCPolynomFit fitBx(M);
  AliHLTTPCPolynomFit fitBy(M);
  AliHLTTPCPolynomFit fitBz(M);
  ...
  for( int sector=0; sector<AliTRDgeometry::Nsector(); sector++){
	...
	for( double z=zMin; z<=zMax; z+=dZ ){ // 1 cm step in Z
	  ...
	  float f[M];
	  AliGPUTPCGMPolynomialField::GetPolynomsTrd(x,y,z,f);
	  fitBx.AddMeasurement( f, B[0]);
	  fitBy.AddMeasurement( f, B[1]);
	  fitBz.AddMeasurement( f, B[2]);
