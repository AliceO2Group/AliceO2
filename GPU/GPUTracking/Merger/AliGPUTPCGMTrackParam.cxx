// $Id: AliGPUTPCGMTrackParam.cxx 41769 2010-06-16 13:58:00Z sgorbuno $
// **************************************************************************
...
#include "AliGPUTPCClusterErrorStat.h"
...
GPUd() bool AliGPUTPCGMTrackParam::Fit(const AliGPUTPCGMMerger *merger, int iTrk, AliGPUTPCGMMergedTrackHit *clusters, int &N, int &NTolerated, float &Alpha, int attempt, float maxSinPhi, AliGPUTPCOuterParam *outerParam)
{
	const AliGPUParam &param = merger->SliceParam();

	AliGPUTPCClusterErrorStat errorStat(N);
	...
	for(;ihit >= 0 && ihit<maxN;ihit += wayDirection)
	{
		...
		CADEBUG(AliGPUTPCStandaloneFramework &hlt = AliGPUTPCStandaloneFramework::Instance();if (configStandalone.resetids && (unsigned int) hlt.GetNMCLabels() > clusters[ihit].fNum))
		...
		if ((param.rec.RejectMode > 0 && nMissed >= param.rec.RejectMode) || nMissed2 >= GPUCA_MERGER_MAXN_MISSED_HARD || clusters[ihit].fState & AliGPUTPCGMMergedTrackHit::flagReject)
		{
			CADEBUG(printf("\tSkipping hit, %d hits rejected, flag %X\n", nMissed, (int) clusters[ihit].fState);)
			if (iWay + 2 >= nWays && !(clusters[ihit].fState & AliGPUTPCGMMergedTrackHit::flagReject)) clusters[ihit].fState |= AliGPUTPCGMMergedTrackHit::flagRejectErr;
			continue;
		}
		...
		CADEBUG(printf("\tLeg %3d%14sTrack   Alpha %8.3f %s, X %8.3f - Y %8.3f, Z %8.3f   -   QPt %7.2f (%7.2f), SP %5.2f (%5.2f) %28s    ---   Cov sY %8.3f sZ %8.3f sSP %8.3f sPt %8.3f   -   YPt %8.3f\n", (int) clusters[ihit].fLeg, "", prop.GetAlpha(), (CAMath::Abs(prop.GetAlpha() - clAlpha) < 0.01 ? "   " : " R!"), fX, fP[0], fP[1], fP[4], prop.GetQPt0(), fP[2], prop.GetSinPhi0(), "", sqrtf(fC[0]), sqrtf(fC[2]), sqrtf(fC[5]), sqrtf(fC[14]), fC[10]);)
		if (allowModification && changeDirection && !noFollowCircle && !noFollowCircle2)
		{
			const AliGPUTPCGMTrackParam backup = *this;
		...
		else if (allowModification && lastRow != 255 && CAMath::Abs(clusters[ihit].fRow - lastRow) > 1)
		{
		...
		if (lastRow == 255 || CAMath::Abs((int) lastRow - (int) clusters[ihit].fRow) > 5 || lastSlice != clusters[ihit].fSlice || (param.rec.RejectMode < 0 && -nMissed <= param.rec.RejectMode)) goodRows = 0;
		else goodRows++;
		if (err == 0)
		{
		...
		if (err == 0 && changeDirection)
		{
			...
			if (CAMath::Abs(yy - fP[0]) > CAMath::Abs(yy - mirrordY))
			{
		...
		if ( err || err2 )
		{
			...
			MarkClusters(clusters, ihitMergeFirst, ihit, wayDirection, AliGPUTPCGMMergedTrackHit::flagNotFit);
			nMissed2++;
			NTolerated++;
		...
		int retVal;
		float threshold = 3.f + (lastUpdateX >= 0 ? (CAMath::Abs(fX - lastUpdateX) / 2) : 0.f);
		if (fNDF > 5 && (CAMath::Abs(yy - fP[0]) > threshold || CAMath::Abs(zz - fP[1]) > threshold)) retVal = 2;
		...
		if (retVal == 0) // track is updated
		{
			...
			covYYUpd = fC[0];
			nMissed = nMissed2 = 0;
			UnmarkClusters(clusters, ihitMergeFirst, ihit, wayDirection, AliGPUTPCGMMergedTrackHit::flagNotFit);
			N++;
			ihitStart = ihit;
		...
		{
			if (allowModification) MarkClusters(clusters, ihitMergeFirst, ihit, wayDirection, AliGPUTPCGMMergedTrackHit::flagRejectDistance);
			nMissed++;
			nMissed2++;
		}
	...
	if (param.rec.TrackReferenceX <= 500)
	{
		for (int k = 0;k < 3;k++) //max 3 attempts
		{
			...
			ConstrainSinPhi();
			if (CAMath::Abs(fP[0]) <= fX * CAMath::Tan(kSectAngle / 2.f)) break;
			float dAngle = floor(CAMath::ATan2(fP[0], fX) / kDeg2Rad / 20.f + 0.5f) * kSectAngle;
			Alpha += dAngle;
			if (err || k == 2)
			{
				Rotate(dAngle);
	...
	else if (CAMath::Abs(fP[0]) > fX * CAMath::Tan(kSectAngle / 2.f))
	{
		float dAngle = floor(CAMath::ATan2(fP[0], fX) / kDeg2Rad / 20.f + 0.5f) * kSectAngle;
		Rotate(dAngle);
		ConstrainSinPhi();
...
GPUd() void AliGPUTPCGMTrackParam::MirrorTo(AliGPUTPCGMPropagator& prop, float toY, float toZ, bool inFlyDirection, const AliGPUParam& param, unsigned char row, unsigned char clusterState, bool mirrorParameters)
{
	...
	if (CAMath::Abs(fC[5]) < 0.1f) fC[5] = fC[5] > 0 ? 0.1f : -0.1f;
...
GPUd() int AliGPUTPCGMTrackParam::MergeDoubleRowClusters(int ihit, int wayDirection, AliGPUTPCGMMergedTrackHit* clusters, const AliGPUParam &param, AliGPUTPCGMPropagator& prop, float& xx, float& yy, float& zz, int maxN, float clAlpha, unsigned char& clusterState, bool rejectChi2)
{
	if (ihit + wayDirection >= 0 && ihit + wayDirection < maxN && clusters[ihit].fRow == clusters[ihit + wayDirection].fRow && clusters[ihit].fSlice == clusters[ihit + wayDirection].fSlice && clusters[ihit].fLeg == clusters[ihit + wayDirection].fLeg)
	{
		...
		while(true)
		{
			...
			if (noReject == 0 && (dy * dy > maxDistY || dz * dz > maxDistZ))
			{
				...
				if (rejectChi2) clusters[ihit].fState |= AliGPUTPCGMMergedTrackHit::flagRejectDistance;
			}
			else
			{
...
GPUd() void AliGPUTPCGMTrackParam::AttachClusters(const AliGPUTPCGMMerger *Merger, int slice, int iRow, int iTrack, bool goodLeg)
{
	...
	for (int k = 0;k <= nz;k++)
	{
		...
		for (unsigned int ih = hitFst;ih < hitLst;ih++)
		{
			...
			if (dy * dy < sy2 && dz * dz < sz2)
			{
				...
				int myWeight = Merger->TrackOrder()[iTrack] | AliGPUTPCGMMerger::attachAttached | AliGPUTPCGMMerger::attachTube;
				if (goodLeg) myWeight |= AliGPUTPCGMMerger::attachGoodLeg;
...
GPUd() void AliGPUTPCGMTrackParam::AttachClustersPropagate(const AliGPUTPCGMMerger *Merger, int slice, int lastRow, int toRow, int iTrack, bool goodLeg, AliGPUTPCGMPropagator &prop, bool inFlyDirection, float maxSinPhi)
{
	...
	{
		if (CAMath::Abs(fP[2]) > maxSinPhi) return;
		if (CAMath::Abs(fX) > CAMath::Abs(fP[0]) * CAMath::Tan(kSectAngle / 2.f)) return;
...
{
	return CAMath::Abs(fX * lrFactor - toY) > 1.f &&                                                                              //transport further in Y
	       CAMath::Abs(fP[2]) < 0.7f &&                                                                                            //rotate back
...
GPUd() int AliGPUTPCGMTrackParam::FollowCircle(const AliGPUTPCGMMerger *Merger, AliGPUTPCGMPropagator &prop, int slice, int iRow, int iTrack, bool goodLeg, float toAlpha, float toX, float toY, int toSlice, int toRow, bool inFlyDirection)
{
	...
	{
		while ((slice != toSlice) ? (CAMath::Abs(fX) <= CAMath::Abs(fP[0]) * CAMath::Tan(kSectAngle / 2.f)) : FollowCircleChk(lrFactor, toY, toX, up, right))
		{
			...
			for (int j = 0;j < GPUCA_ROW_COUNT && found < 3;j++)
			{
				...
				if (CAMath::Abs(rowX - (-fP[0] * lrFactor)) < 1.5f)
				{
...
GPUd() void AliGPUTPCGMTrackParam::AttachClustersMirror(const AliGPUTPCGMMerger* Merger, int slice, int iRow, int iTrack, float toY, AliGPUTPCGMPropagator& prop)
{
	...
	float Z = fP[1];
	if (CAMath::Abs(fP[2]) >= GPUCA_MAX_SIN_PHI_LOW) return;
	float SinPhi = CAMath::Sqrt(1 - fP[2] * fP[2]) * (fP[2] > 0 ? -1 : 1);
	if (CAMath::Abs(SinPhi) >= GPUCA_MAX_SIN_PHI_LOW) return;
	...
	while (count--)
	{
		...
		if (CAMath::Abs(newSinPhi) > GPUCA_MAX_SIN_PHI_LOW) return;
		float dS = dx * exi;
		float h2 = dS * exi * exi;
		...
		Z += dS * fP[3];
		SinPhi = newSinPhi;
		if (CAMath::Abs(X) > CAMath::Abs(Y) * CAMath::Tan(kSectAngle / 2.f)) continue;
...
GPUd() void AliGPUTPCGMTrackParam::ShiftZ(const AliGPUTPCGMPolynomialField* field, const AliGPUTPCGMMergedTrackHit* clusters, const AliGPUParam &param, int N)
{
	...
	const float cosPhi = CAMath::Abs(fP[2]) < 1.f ? CAMath::Sqrt(1 - fP[2] * fP[2]) : 0.f;
	const float dxf = -CAMath::Abs(fP[2]);
	const float dyf = cosPhi * (fP[2] > 0 ? 1.f : -1.f);
	const float r = 1.f / CAMath::Abs(fP[4] * field->GetNominalBz());
	float xp = fX + dxf * r;
	float yp = fP[0] + dyf * r;
...
GPUd() bool AliGPUTPCGMTrackParam::CheckCov() const
{
	const float *c = fC;
...
GPUd() bool AliGPUTPCGMTrackParam::CheckNumericalQuality(float overrideCovYY) const
{
	...
	if ( CAMath::Abs( fP[2] ) > GPUCA_MAX_SIN_PHI ) ok = 0;
...
bool AliGPUTPCGMTrackParam::GetExtParam( AliExternalTrackParam &T, double alpha ) const
{
	//* Convert from AliGPUTPCGMTrackParam to AliExternalTrackParam parameterisation,
...
void AliGPUTPCGMTrackParam::SetExtParam( const AliExternalTrackParam &T )
{
...
GPUd() void AliGPUTPCGMTrackParam::RefitTrack(AliGPUTPCGMMergedTrack &track, int iTrk, const AliGPUTPCGMMerger* merger, AliGPUTPCGMMergedTrackHit* clusters)
{
	if( !track.OK() ) return;
	...
	for (int attempt = 0;;)
	{
		...
		if ( CAMath::Abs( t.QPt() ) < 1.e-4f ) t.QPt() = 1.e-4f;
