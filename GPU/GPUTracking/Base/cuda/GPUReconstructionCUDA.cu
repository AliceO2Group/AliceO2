...
template <class T, int I, typename... Args> int GPUReconstructionCUDABackend::runKernelBackend(const krnlExec& x, const krnlRunRange& y, const krnlEvent& z, const Args&... args)
{
...
GPUReconstruction* GPUReconstruction_Create_CUDA(const GPUSettingsProcessing& cfg)
{
	return new GPUReconstructionCUDA(cfg);
}
...
void GPUReconstructionCUDABackend::RecordMarker(deviceEvent* ev, int stream)
{
	GPUFailedMsg(cudaEventRecord(*(cudaEvent_t*) ev, mInternals->CudaStreams[stream]));
}

void GPUReconstructionCUDABackend::ActivateThreadContext()
{
	cuCtxPushCurrent(mInternals->CudaContext);
}
void GPUReconstructionCUDABackend::ReleaseThreadContext()
{
	cuCtxPopCurrent(&mInternals->CudaContext);
}

void GPUReconstructionCUDABackend::SynchronizeGPU()
{
	GPUFailedMsg(cudaDeviceSynchronize());
}

void GPUReconstructionCUDABackend::SynchronizeStream(int stream)
{
