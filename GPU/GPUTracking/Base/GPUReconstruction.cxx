...
GPUReconstruction::GPUReconstruction(const GPUSettingsProcessing& cfg) : mHostConstantMem(new GPUConstantMem)
{
...
{
	if (mDeviceProcessingSettings.memoryAllocationStrategy == GPUMemoryResource::ALLOCATION_AUTO) mDeviceProcessingSettings.memoryAllocationStrategy = IsGPU() ? GPUMemoryResource::ALLOCATION_GLOBAL : GPUMemoryResource::ALLOCATION_INDIVIDUAL;
...
void GPUReconstruction::RegisterGPUDeviceProcessor(GPUProcessor* proc, GPUProcessor* slaveProcessor)
{
	proc->InitGPUProcessor(this, GPUProcessor::PROCESSOR_TYPE_DEVICE, slaveProcessor);
}
...
size_t GPUReconstruction::AllocateRegisteredMemory(GPUProcessor* proc)
{
	...
	for (unsigned int i = 0;i < mMemoryResources.size();i++)
	{
		if ((proc == nullptr ? !mMemoryResources[i].mProcessor->mAllocateAndInitializeLate :  mMemoryResources[i].mProcessor == proc) && !(mMemoryResources[i].mType & GPUMemoryResource::MEMORY_CUSTOM)) total += AllocateRegisteredMemory(i);
	}
...
size_t GPUReconstruction::AllocateRegisteredPermanentMemory()
{
	...
	for (unsigned int i = 0;i < mMemoryResources.size();i++)
	{
		if ((mMemoryResources[i].mType & GPUMemoryResource::MEMORY_PERMANENT) && mMemoryResources[i].mPtr == nullptr) total += AllocateRegisteredMemory(i);
	}
...
{
	if (type != GPUMemoryResource::MEMORY_HOST && (!IsGPU() || type != GPUMemoryResource::MEMORY_GPU)) throw std::bad_alloc();
	if (mDeviceProcessingSettings.memoryAllocationStrategy == GPUMemoryResource::ALLOCATION_INDIVIDUAL)
	{
...
void GPUReconstruction::ResetRegisteredMemoryPointers(short ires)
{
	GPUMemoryResource* res = &mMemoryResources[ires];
	res->SetPointers(res->mPtr);
	if (IsGPU() && (res->mType & GPUMemoryResource::MEMORY_GPU)) res->SetDevicePointers(res->mPtrDevice);
}
...
void GPUReconstruction::FreeRegisteredMemory(GPUProcessor* proc, bool freeCustom)
{
	for (unsigned int i = 0;i < mMemoryResources.size();i++)
	{
		if ((proc == nullptr || mMemoryResources[i].mProcessor == proc) && (freeCustom || !(mMemoryResources[i].mType & GPUMemoryResource::MEMORY_CUSTOM))) FreeRegisteredMemory(i);
	}
}
...
{
	GPUMemoryResource* res = &mMemoryResources[ires];
	if (mDeviceProcessingSettings.memoryAllocationStrategy == GPUMemoryResource::ALLOCATION_INDIVIDUAL) operator delete(res->mPtrDevice);
	res->mPtr = nullptr;
...
void GPUReconstruction::ClearAllocatedMemory()
{
	for (unsigned int i = 0;i < mMemoryResources.size();i++)
	{
		if (!(mMemoryResources[i].mType & GPUMemoryResource::MEMORY_PERMANENT)) FreeRegisteredMemory(i);
	}
...
int GPUReconstruction::GetMaxThreads()
{
...
GPUReconstruction* GPUReconstruction::CreateInstance(const GPUSettingsProcessing& cfg)
{
	...
	if (retVal == 0)
	{
		...
		{
			printf("Error: Could not load GPUReconstruction for specified device: %s (%d)\n", DEVICE_TYPE_NAMES[type], type);
		}
		else
		{
			printf("Could not load GPUReconstruction for device type %s (%d), falling back to CPU version\n", DEVICE_TYPE_NAMES[type], type);
	...
	{
		printf("Created GPUReconstruction instance for device type %s (%d)\n", DEVICE_TYPE_NAMES[type], type);
	}
...
std::shared_ptr<GPUReconstruction::LibraryLoader> GPUReconstruction::sLibCUDA(new GPUReconstruction::LibraryLoader("lib" LIBRARY_PREFIX "GPUTracking" "CUDA" LIBRARY_EXTENSION, "GPUReconstruction_Create_" "CUDA"));
std::shared_ptr<GPUReconstruction::LibraryLoader> GPUReconstruction::sLibHIP(new GPUReconstruction::LibraryLoader("lib" LIBRARY_PREFIX "GPUTracking" "HIP" LIBRARY_EXTENSION, "GPUReconstruction_Create_" "HIP"));
std::shared_ptr<GPUReconstruction::LibraryLoader> GPUReconstruction::sLibOCL(new GPUReconstruction::LibraryLoader("lib" LIBRARY_PREFIX "GPUTracking" "OCL" LIBRARY_EXTENSION, "GPUReconstruction_Create_" "OCL"));
...
GPUReconstruction::LibraryLoader::~LibraryLoader()
{
	CloseLibrary();
}
