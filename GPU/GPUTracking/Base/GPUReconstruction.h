...
template <class T> inline void GPUReconstruction::AllocateIOMemoryHelper(unsigned int n, const T* &ptr, std::unique_ptr<T[]> &u)
{
	if (n == 0)
	{
		u.reset(nullptr);
...
template <class T> inline T* GPUReconstruction::AddChain()
{
	mChains.emplace_back(new T(this));
	return (T*) mChains.back().get();
}

template <class T> inline short GPUReconstruction::RegisterMemoryAllocation(T* proc, void* (T::* setPtr)(void*), int type, const char* name)
{
	...
	if (proc->mGPUProcessorType == GPUProcessor::PROCESSOR_TYPE_CPU) type &= ~GPUMemoryResource::MEMORY_GPU;
...
template <class T> inline void GPUReconstruction::RegisterGPUProcessor(T* proc, bool deviceSlave)
{
...
template <class T> inline void GPUReconstruction::SetupGPUProcessor(T* proc, bool allocate)
{
...
template <class T> inline void GPUReconstruction::DumpData(FILE* fp, const T* const* entries, const unsigned int* num, InOutPointerType type)
{
	int count;
...
template <class T> inline size_t GPUReconstruction::ReadData(FILE* fp, const T** entries, unsigned int* num, std::unique_ptr<T[]>* mem, InOutPointerType type)
{
	if (feof(fp)) return 0;
...
template <class T> inline void GPUReconstruction::DumpFlatObjectToFile(const T* obj, const char* file)
{
...
template <class T> inline std::unique_ptr<T> GPUReconstruction::ReadFlatObjectFromFile(const char* file)
{
...
template <class T> inline void GPUReconstruction::DumpStructToFile(const T* obj, const char* file)
{
	FILE* fp = fopen(file, "w+b");
	if (fp == nullptr) return;
...
template <class T> inline std::unique_ptr<T> GPUReconstruction::ReadStructFromFile(const char* file)
{
	FILE* fp = fopen(file, "rb");
	if (fp == nullptr) return nullptr;
	size_t size, r;
...
template <class T> inline void GPUReconstruction::ReadStructFromFile(const char* file, T* obj)
{
	FILE* fp = fopen(file, "rb");
	if (fp == nullptr) return;
	size_t size, r;
