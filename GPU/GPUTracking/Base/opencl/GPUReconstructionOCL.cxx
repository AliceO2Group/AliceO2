...
template <class T, int I, typename... Args> int GPUReconstructionOCLBackend::runKernelBackend(const krnlExec& x, const krnlRunRange& y, const krnlEvent& z, const Args&... args)
{
	if (x.device == krnlDeviceType::CPU) return GPUReconstructionCPU::runKernelImpl(classArgument<T, I>(), x, y, z, args...);
...
GPUReconstruction* GPUReconstruction_Create_OCL(const GPUSettingsProcessing& cfg)
{
	return new GPUReconstructionOCL(cfg);
}
...
int GPUReconstructionOCLBackend::InitDevice_Runtime()
{
	...
	if (AddKernel<GPUMemClean16>()) return 1;
	if (AddKernel<GPUTPCNeighboursFinder>()) return 1;
	if (AddKernel<GPUTPCNeighboursCleaner>()) return 1;
	if (AddKernel<GPUTPCStartHitsFinder>()) return 1;
	if (AddKernel<GPUTPCStartHitsSorter>()) return 1;
	if (AddKernel<GPUTPCTrackletConstructor>()) return 1;
	if (AddKernel<GPUTPCTrackletConstructor, 1>()) return 1;
	if (AddKernel<GPUTPCTrackletSelector>(false)) return 1;
	if (AddKernel<GPUTPCTrackletSelector>(true)) return 1;
...
void GPUReconstructionOCLBackend::ReleaseEvent(deviceEvent* ev)
{
	GPUFailedMsg(clReleaseEvent(*(cl_event*) ev));
}

void GPUReconstructionOCLBackend::RecordMarker(deviceEvent* ev, int stream)
{
...
void GPUReconstructionOCLBackend::SynchronizeStream(int stream)
{
	GPUFailedMsg(clFinish(mInternals->command_queue[stream]));
}

void GPUReconstructionOCLBackend::SynchronizeEvents(deviceEvent* evList, int nEvents)
{
...
template <class T, int I = 0> int GPUReconstructionOCLBackend::FindKernel(int num)
{
...
template <class S, class T, int I = 0> S& GPUReconstructionOCLBackend::getKernelObject(int num)
{
...
template <class T, int I> int GPUReconstructionOCLBackend::AddKernel(bool multi)
{
	std::string name("GPUTPCProcess_");
