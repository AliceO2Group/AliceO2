...
void AliGPUParam::SetDefaults(float solenoidBz)
{
...
void AliGPUParam::UpdateEventSettings(const AliGPUSettingsEvent* e, const AliGPUSettingsDeviceProcessing* p)
{
...
void AliGPUParam::SetDefaults(const AliGPUSettingsEvent *e, const AliGPUSettingsRec *r, const AliGPUSettingsDeviceProcessing *p)
{
...
void AliGPUParam::LoadClusterErrors(bool Print)
{
}
...
void AliGPUParam::LoadClusterErrors(bool Print)
{
	...
	if (!clparam)
	{
		std::cout << "Error: AliGPUParam::LoadClusterErrors():: No AliTPCClusterParam instance found !!!! " << std::endl;
		return;
	}
...
MEM_CLASS_PRE()
void MEM_LG(AliGPUParam)::Slice2Global(int iSlice, float x, float y, float z, float *X, float *Y, float *Z) const
{
...
MEM_CLASS_PRE()
void MEM_LG(AliGPUParam)::Global2Slice(int iSlice, float X, float Y, float Z, float *x, float *y, float *z) const
{
...
MEM_CLASS_PRE()
GPUd() float MEM_LG(AliGPUParam)::GetClusterRMS(int yz, int type, float z, float angle2) const
{
...
MEM_CLASS_PRE()
GPUd() void MEM_LG(AliGPUParam)::GetClusterRMS2(int iRow, float z, float sinPhi, float DzDs, float &ErrY2, float &ErrZ2) const
{
...
MEM_CLASS_PRE()
GPUd() float MEM_LG(AliGPUParam)::GetClusterError2(int yz, int type, float z, float angle2) const
{
	//* recalculate the cluster error wih respect to the track slope
...
MEM_CLASS_PRE()
GPUd() void MEM_LG(AliGPUParam)::GetClusterErrors2(int iRow, float z, float sinPhi, float DzDs, float &ErrY2, float &ErrZ2) const
{
