  ...
  for (unsigned int i = 0; i < NSLICES; i++) {
    ...
    for (unsigned int k = 0; k <= tmpBuffer.size(); k++) {
      ...
      if (endpoint != lastEndpoint) {
        ...
        hdr->timeOffset = ZSEncoderGetTime(tmpBuffer[k]) - (hbf * o2::constants::lhc::LHCMaxBunches + Constants::LHCBCPERTIMEBIN - 1 - bcShiftInFirstHBF) / Constants::LHCBCPERTIMEBIN;
        lastTime = -1;
        tbHdr = nullptr;
