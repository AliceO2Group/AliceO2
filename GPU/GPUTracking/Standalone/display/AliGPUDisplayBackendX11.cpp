...
int AliGPUDisplayBackendX11::GetKey(int key)
{
	if (key == 65453) return('-');
	if (key == 65451) return('+');
...
void AliGPUDisplayBackendX11::GetKey(XEvent& event, int& keyOut, int& keyPressOut)
{
	char tmpString[9];
	KeySym sym;
...
void AliGPUDisplayBackendX11::OpenGLPrint(const char* s, float x, float y, float r, float g, float b, float a, bool fromBotton)
{
...
int AliGPUDisplayBackendX11::OpenGLMain()
{
...
void AliGPUDisplayBackendX11::DisplayExit()
{
	pthread_mutex_lock(&semLockExit);
...
void AliGPUDisplayBackendX11::SwitchFullscreen(bool set)
{
	XEvent xev;
...
void AliGPUDisplayBackendX11::ToggleMaximized(bool set)
{
	XEvent xev;
	memset(&xev, 0, sizeof(xev));
	xev.type = ClientMessage;
...
void AliGPUDisplayBackendX11::SetVSync(bool enable)
{
	glXSwapIntervalEXT(g_pDisplay, glXGetCurrentDrawable(), (int) enable);
}

int AliGPUDisplayBackendX11::StartDisplay()
{
