...
static const AliGPUDisplay::configDisplay& AliGPUDisplay_GetConfig(AliGPUChainTracking* rec)
{
	...
	if (rec->mConfigDisplay) return *((const AliGPUDisplay::configDisplay*) rec->mConfigDisplay);
...
inline void AliGPUDisplay::insertVertexList(std::pair<vecpod<GLint>*, vecpod<GLsizei>*>& vBuf, size_t first, size_t last)
{
	if (first == last) return;
	vBuf.first->emplace_back(first);
	vBuf.second->emplace_back(last - first);
}
inline void AliGPUDisplay::insertVertexList(int iSlice, size_t first, size_t last)
{
...
void AliGPUDisplay::SetCollisionFirstCluster(unsigned int collision, int slice, int cluster)
{
...
void AliGPUDisplay::animationCloseAngle(float& newangle, float lastAngle)
{
...
void AliGPUDisplay::animateCloseQuaternion(float* v, float lastx, float lasty, float lastz, float lastw)
{
...
void AliGPUDisplay::setAnimationPoint()
{
...
inline void AliGPUDisplay::SetColorClusters() { if (cfg.colorCollisions) return; if (invertColors) glColor3f(0, 0.3, 0.7); else glColor3f(0, 0.7, 1.0); }
inline void AliGPUDisplay::SetColorTRD() { if (cfg.colorCollisions) return; if (invertColors) glColor3f(0.7, 0.3, 0); else glColor3f(1.0, 0.7, 0); }
inline void AliGPUDisplay::SetColorInitLinks() { if (invertColors) glColor3f(0.42, 0.4, 0.1); else glColor3f(0.42, 0.4, 0.1); }
inline void AliGPUDisplay::SetColorLinks() { if (invertColors) glColor3f(0.6, 0.1, 0.1); else glColor3f(0.8, 0.2, 0.2); }
inline void AliGPUDisplay::SetColorSeeds() { if (invertColors) glColor3f(0.6, 0.0, 0.65); else glColor3f(0.8, 0.1, 0.85); }
inline void AliGPUDisplay::SetColorTracklets() { if (invertColors) glColor3f(0, 0, 0); else glColor3f(1, 1, 1); }
inline void AliGPUDisplay::SetColorTracks() { if (invertColors) glColor3f(0.6, 0, 0.1); else glColor3f(0.8, 1., 0.15); }
inline void AliGPUDisplay::SetColorGlobalTracks() { if (invertColors) glColor3f(0.8, 0.2, 0); else glColor3f(1.0, 0.4, 0); }
inline void AliGPUDisplay::SetColorFinal() { if (cfg.colorCollisions) return; if (invertColors) glColor3f(0, 0.6, 0.1); else glColor3f(0, 0.7, 0.2); }
inline void AliGPUDisplay::SetColorGrid() { if (invertColors) glColor3f(0.5, 0.5, 0); else glColor3f(0.7, 0.7, 0.0); }
inline void AliGPUDisplay::SetColorMarked() { if (invertColors) glColor3f(0.8, 0, 0); else glColor3f(1.0, 0.0, 0.0); }
inline void AliGPUDisplay::SetCollisionColor(int col)
{
	int red = (col * 2) % 5;
	int blue =  (2 + col * 3) % 7;
	int green = (4 + col * 5) % 6;
...
void AliGPUDisplay::setDepthBuffer()
{
	if (cfg.depthBuffer)
	{
...
void AliGPUDisplay::createFB_texture(GLuint& id, bool msaa, GLenum storage, GLenum attachment)
{
...
void AliGPUDisplay::createFB_renderbuffer(GLuint& id, bool msaa, GLenum storage, GLenum attachment)
{
...
void AliGPUDisplay::createFB(GLfb& fb, bool tex, bool withDepth, bool msaa)
{
	fb.tex = tex;
	fb.depth = withDepth;
	fb.msaa = msaa;
...
void AliGPUDisplay::deleteFB(GLfb& fb)
{
...
void AliGPUDisplay::setFrameBuffer(int updateCurrent, GLuint newID)
{
...
void AliGPUDisplay::UpdateOffscreenBuffers(bool clean)
{
...
void AliGPUDisplay::ReSizeGLScene(int width, int height, bool init) // Resize And Initialize The GL Window
{
...
void AliGPUDisplay::updateConfig()
{
	setQuality();
	setDepthBuffer();
}

int AliGPUDisplay::InitGL(bool initFailure)
{
...
int AliGPUDisplay::InitGL_internal()
{
	int glVersion[2] = {0, 0};
...
inline void AliGPUDisplay::drawPointLinestrip(int iSlice, int cid, int id, int id_limit)
{
...
AliGPUDisplay::vboList AliGPUDisplay::DrawSpacePointsTRD(int iSlice, int select, int iCol)
{
...
AliGPUDisplay::vboList AliGPUDisplay::DrawClusters(const AliGPUTPCTracker &tracker, int select, int iCol)
{
...
AliGPUDisplay::vboList AliGPUDisplay::DrawLinks(const AliGPUTPCTracker &tracker, int id, bool dodown)
{
	int iSlice = tracker.ISlice();
...
AliGPUDisplay::vboList AliGPUDisplay::DrawSeeds(const AliGPUTPCTracker &tracker)
{
	int iSlice = tracker.ISlice();
	if (config.clustersOnly) return(vboList(0, 0, iSlice));
	size_t startCount = vertexBufferStart[iSlice].size();
...
AliGPUDisplay::vboList AliGPUDisplay::DrawTracklets(const AliGPUTPCTracker &tracker)
{
	int iSlice = tracker.ISlice();
	if (config.clustersOnly) return(vboList(0, 0, iSlice));
	size_t startCount = vertexBufferStart[iSlice].size();
...
AliGPUDisplay::vboList AliGPUDisplay::DrawTracks(const AliGPUTPCTracker &tracker, int global)
{
	int iSlice = tracker.ISlice();
	if (config.clustersOnly) return(vboList(0, 0, iSlice));
	size_t startCount = vertexBufferStart[iSlice].size();
...
void AliGPUDisplay::DrawFinal(int iSlice, int /*iCol*/, AliGPUTPCGMPropagator* prop, std::array<vecpod<int>, 2>& trackList, threadVertexBuffer& threadBuffer)
{
...
AliGPUDisplay::vboList AliGPUDisplay::DrawGrid(const AliGPUTPCTracker &tracker)
{
	int iSlice = tracker.ISlice();
	size_t startCount = vertexBufferStart[iSlice].size();
	size_t startCountInner = vertexBuffer[iSlice].size();
...
int AliGPUDisplay::DrawGLScene(bool mixAnimation, float animateTime)
{
	try
	{
...
int AliGPUDisplay::DrawGLScene_internal(bool mixAnimation, float animateTime) // Here's Where We Do All The Drawing
{
...
void AliGPUDisplay::DoScreenshot(char *filename, float animateTime)
{
...
void AliGPUDisplay::showInfo(const char* info)
{
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glMatrixMode(GL_PROJECTION);
...
void AliGPUDisplay::ShowNextEvent()
{
	semLockDisplay.Unlock();
...
void AliGPUDisplay::WaitForNextEvent()
{
	semLockDisplay.Lock();
}

int AliGPUDisplay::StartDisplay()
{
