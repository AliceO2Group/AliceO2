...
static AliGPUDisplayBackendGlfw* me = nullptr;

int AliGPUDisplayBackendGlfw::GetKey(int key)
{
...
void AliGPUDisplayBackendGlfw::GetKey(int key, int scancode, int mods, int& keyOut, int& keyPressOut)
{
...
void AliGPUDisplayBackendGlfw::error_callback(int error, const char* description)
{
    fprintf(stderr, "Error: %s\n", description);
}

void AliGPUDisplayBackendGlfw::key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
...
void AliGPUDisplayBackendGlfw::mouseButton_callback(GLFWwindow* window, int button, int action, int mods)
{
	if (action == GLFW_PRESS)
	{
		if (button == 0)
...
void AliGPUDisplayBackendGlfw::scroll_callback(GLFWwindow* window, double x, double y)
{
	me->mouseWheel += y * 100;
}

void AliGPUDisplayBackendGlfw::cursorPos_callback(GLFWwindow* window, double x, double y)
{
	me->mouseMvX = x;
	me->mouseMvY = y;
}

void AliGPUDisplayBackendGlfw::resize_callback(GLFWwindow* window, int width, int height)
{
	me->ReSizeGLScene(width, height);
}

void AliGPUDisplayBackendGlfw::DisplayLoop()
{
#ifdef GPUCA_O2_LIB
...
int AliGPUDisplayBackendGlfw::OpenGLMain()
{
	me = this;
...
void AliGPUDisplayBackendGlfw::DisplayExit()
{
	pthread_mutex_lock(&semLockExit);
...
void AliGPUDisplayBackendGlfw::OpenGLPrint(const char* s, float x, float y, float r, float g, float b, float a, bool fromBotton)
{
#ifdef GPUCA_O2_LIB
...
void AliGPUDisplayBackendGlfw::SwitchFullscreen(bool set)
{
...
void AliGPUDisplayBackendGlfw::ToggleMaximized(bool set)
{
	if (set) glfwMaximizeWindow(window);
	else glfwRestoreWindow(window);
}

void AliGPUDisplayBackendGlfw::SetVSync(bool enable)
{
	glfwSwapInterval(enable);
}

int AliGPUDisplayBackendGlfw::StartDisplay()
{
...
bool AliGPUDisplayBackendGlfw::EnableSendKey()
{
#ifdef GPUCA_O2_LIB
	return false;
#else
