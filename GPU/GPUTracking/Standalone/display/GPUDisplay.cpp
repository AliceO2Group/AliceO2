...
static const GPUDisplay::configDisplay& GPUDisplay_GetConfig(GPUChainTracking* rec)
{
	...
	if (rec->mConfigDisplay) return *((const GPUDisplay::configDisplay*) rec->mConfigDisplay);
...
GPUDisplay::GPUDisplay(GPUDisplayBackend* backend, GPUChainTracking* rec, GPUQA* qa) : mBackend(backend), mChain(rec), config(GPUDisplay_GetConfig(rec)), mQA(qa), merger(rec->GetTPCMerger())
{
...
void GPUDisplay::animationCloseAngle(float& newangle, float lastAngle)
{
...
void GPUDisplay::animateCloseQuaternion(float* v, float lastx, float lasty, float lastz, float lastw)
{
...
inline void GPUDisplay::SetColorClusters() { if (cfg.colorCollisions) return; if (invertColors) glColor3f(0, 0.3, 0.7); else glColor3f(0, 0.7, 1.0); }
inline void GPUDisplay::SetColorTRD() { if (cfg.colorCollisions) return; if (invertColors) glColor3f(0.7, 0.3, 0); else glColor3f(1.0, 0.7, 0); }
inline void GPUDisplay::SetColorInitLinks() { if (invertColors) glColor3f(0.42, 0.4, 0.1); else glColor3f(0.42, 0.4, 0.1); }
inline void GPUDisplay::SetColorLinks() { if (invertColors) glColor3f(0.6, 0.1, 0.1); else glColor3f(0.8, 0.2, 0.2); }
inline void GPUDisplay::SetColorSeeds() { if (invertColors) glColor3f(0.6, 0.0, 0.65); else glColor3f(0.8, 0.1, 0.85); }
inline void GPUDisplay::SetColorTracklets() { if (invertColors) glColor3f(0, 0, 0); else glColor3f(1, 1, 1); }
inline void GPUDisplay::SetColorTracks() { if (invertColors) glColor3f(0.6, 0, 0.1); else glColor3f(0.8, 1., 0.15); }
inline void GPUDisplay::SetColorGlobalTracks() { if (invertColors) glColor3f(0.8, 0.2, 0); else glColor3f(1.0, 0.4, 0); }
inline void GPUDisplay::SetColorFinal() { if (cfg.colorCollisions) return; if (invertColors) glColor3f(0, 0.6, 0.1); else glColor3f(0, 0.7, 0.2); }
inline void GPUDisplay::SetColorGrid() { if (invertColors) glColor3f(0.5, 0.5, 0); else glColor3f(0.7, 0.7, 0.0); }
inline void GPUDisplay::SetColorMarked() { if (invertColors) glColor3f(0.8, 0, 0); else glColor3f(1.0, 0.0, 0.0); }
...
int GPUDisplay::InitGL_internal()
{
	...
	CHKERR(glCreateBuffers(GPUChainTracking::NSLICES, vbo_id));
...
void GPUDisplay::ExitGL()
{
	...
	CHKERR(glDeleteBuffers(GPUChainTracking::NSLICES, vbo_id));
	...
	for (int cidInSlice = firstCluster;cidInSlice < lastCluster;cidInSlice++)
	{
		...
		if (markAdjacentClusters)
		{
			...
			if (attach)
			{
				...
				else if ((markAdjacentClusters & 2) && (attach & GPUTPCGMMerger::attachTube)) draw = select == tMARKED;
				else if ((markAdjacentClusters & 1) && (attach & (GPUTPCGMMerger::attachGood | GPUTPCGMMerger::attachTube)) == 0) draw = select == tMARKED;
				else if ((markAdjacentClusters & 4) && (attach & GPUTPCGMMerger::attachGoodLeg) == 0) draw = select == tMARKED;
				else if (markAdjacentClusters & 8)
				{
					if (fabsf(merger.OutputTracks()[attach & GPUTPCGMMerger::attachTrackMask].GetParam().GetQPt()) > 20.f) draw = select == tMARKED;
				}
			}
		}
...
void GPUDisplay::DrawFinal(int iSlice, int /*iCol*/, GPUTPCGMPropagator* prop, std::array<vecpod<int>, 2>& trackList, threadVertexBuffer& threadBuffer)
{
	...
	for (unsigned int ii = 0;ii < nTracks;ii++)
	{
		...
		while (true)
		{
			...
			{
				if (hideRejectedClusters && (merger.Clusters()[track->FirstClusterRef() + k].fState & GPUTPCGMMergedTrackHit::flagReject)) continue;
...
int GPUDisplay::DrawGLScene(bool mixAnimation, float animateTime)
{
	try
	{
...
int GPUDisplay::DrawGLScene_internal(bool mixAnimation, float animateTime) // Here's Where We Do All The Drawing
{
...
void GPUDisplay::DoScreenshot(char *filename, float animateTime)
{
