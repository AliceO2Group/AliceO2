...
#include "utils/timer.h"
	...
	{ \
		...
		if (trackMCLabelsReverse[mcLabel] != id) attach &= (~GPUTPCGMMerger::attachGoodLeg); \
	}
#else
	#define CHECK_CLUSTER_STATE_INIT_LEG_BY_MC()
#endif
...
#define CHECK_CLUSTER_STATE_CHK_COUNT() \
	...
	else if ((attach & GPUTPCGMMerger::attachGoodLeg) == 0) recClustersLoopers++; \
	else if ((attach & GPUTPCGMMerger::attachTube) && mev200) recClustersTube200++; \
	else if (attach & GPUTPCGMMerger::attachHighIncl) recClustersHighIncl++; \
	else if (attach & GPUTPCGMMerger::attachTube) {protect = true; recClustersTube++;} \
	else if ((attach & GPUTPCGMMerger::attachGood) == 0) {protect = true; recClustersRejected++;} \
	else {physics = true;} \
...
#define CHECK_CLUSTER_STATE_CHK_NOCOUNT() \
	...
	else if (attach & GPUTPCGMMerger::attachTube) {protect = true;} \
	else if ((attach & GPUTPCGMMerger::attachGood) == 0) {protect = true;} \
	else {physics = true;} \
...
static const GPUQA::configQA& GPUQA_GetConfig(GPUChainTracking* rec)
{
	...
	if (rec->mConfigQA) return *((const GPUQA::configQA*) rec->mConfigQA);
...
GPUQA::GPUQA(GPUChainTracking* rec) : mRec(rec), config(GPUQA_GetConfig(rec))
{
}
...
bool GPUQA::SuppressTrack(int iTrack) const
{
	return (config.matchMCLabels.size() && !goodTracks[nEvents][iTrack]);
}

bool GPUQA::SuppressHit(int iHit) const
{
	return (config.matchMCLabels.size() && !goodHits[nEvents - 1][iHit]);
}

int GPUQA::GetMCLabel(unsigned int trackId) const
{
...
#pragma omp parallel for
		...
		for (int i = 0; i < merger.NOutputTracks(); i++)
		{
			...
			{
				if (merger.Clusters()[track.FirstClusterRef() + k].fState & GPUTPCGMMergedTrackHit::flagReject) continue;
				nClusters++;
		...
		if (config.matchMCLabels.size())
		{
			...
			for (int i = 0;i < merger.NOutputTracks();i++)
			{
				...
				if (config.matchDisplayMinPt > 0)
				{
					...
					const GPUTPCMCInfo& info = mRec->mIOPtrs.mcInfosTPC[abs(trackMCLabels[i])];
...
#pragma omp parallel for
		...
		for (int i = 0; i < merger.NOutputTracks(); i++)
		{
			...
			const GPUTPCMCInfo& mc1 = mRec->mIOPtrs.mcInfosTPC[trackMCLabels[i]];
	...
	if (config.csvDump)
	{
		...
		if (totalNCls == 0) for (unsigned int iSlice = 0; iSlice < GPUChainTracking::NSLICES; iSlice++) totalNCls += mRec->mIOPtrs.nClusterData[iSlice];
...
template <class T> T* GPUQA::GetHist(T* &ee, std::vector<TFile*>& tin, int k, int nNewInput)
{
	T* e = ee;
