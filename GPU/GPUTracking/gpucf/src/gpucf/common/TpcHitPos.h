...
struct TpcHitPos {
  ...
  {
    return sector == other.sector && row == other.row && label == other.label;
  }
};
...
struct hash<gpucf::TpcHitPos> {
  size_t operator()(const gpucf::TpcHitPos& p) const
  {
    ...
    size_t h = (size_t(p.sector) << (8 * 3 * sizeof(short))) | (size_t(p.row) << (8 * 2 * sizeof(short))) | (size_t(p.label.event) << (8 * 1 * sizeof(short))) | size_t(p.label.track);
