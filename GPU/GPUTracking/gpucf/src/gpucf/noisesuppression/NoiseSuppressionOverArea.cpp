...
NoiseSuppressionOverArea::NoiseSuppressionOverArea(
  ...
  int cutoff,
  int epsilon)
  : NoiseSuppression("Bereich " + std::to_string(radPad * 2 + 1) + "x" + std::to_string(radTime * 2 + 1)), radPad(radPad), radTime(radTime), cutoff(cutoff), epsilon(epsilon)
{
  outerToInner = {
    {{-2, -2}, {{-1, -1}}},
    {{-2, 2}, {{-1, 1}}},
  ...
  for (auto& p : outerToInner) {
    if (std::abs(p.first.pad) <= radPad && std::abs(p.first.time) <= radTime) {
