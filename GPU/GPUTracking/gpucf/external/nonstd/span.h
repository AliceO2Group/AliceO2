...
#define span_DISABLE_MSVC_WARNINGS(codes) __pragma(warning(push)) __pragma(warning(disable \
                                                                                   : codes))
...
template <
  class Container, class ElementType span_REQUIRES_T((!detail::is_span<Container>::value && !detail::is_array<Container>::value && !detail::is_std_array<Container>::value && (std::is_convertible<typename std::remove_pointer<decltype(std::declval<Container>().data())>::type (*)[], ElementType (*)[]>::value)))
#if span_HAVE(DATA)
  // data(cont) and size(cont) well-formed:
  ,
  class = decltype(std::data(std::declval<Container>())), class = decltype(std::size(std::declval<Container>()))
#endif
  >
...
class span
{
 public:
  ...
  span_constexpr_exp span(pointer firstElem, pointer lastElem)
    : data_(firstElem), size_(to_size(std::distance(firstElem, lastElem)))
  {
    span_EXPECTS(
...
#if span_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
              span_REQUIRES_T((
                (Extent == dynamic_extent || Extent == static_cast<extent_t>(N)) && std::is_convertible<value_type (*)[], element_type (*)[]>::value))
#endif
            >
  ...
  span_constexpr span(Container& cont)
    : data_(cont.data()), size_(to_size(cont.size()))
  {
  }

  template <class Container
              span_REQUIRES_T((
                std::is_const<element_type>::value && can_construct_from<Container, element_type>::value))>
  ...
  span_constexpr span(with_container_t, Container const& cont)
    : data_(cont.size() == 0 ? span_nullptr : const_cast<pointer>(span_ADDRESSOF(cont[0]))), size_(to_size(cont.size()))
  {
  }
#endif
...
#if span_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG)
                                      span_REQUIRES_T((
                                        (Extent == dynamic_extent || Extent == OtherExtent) && std::is_convertible<OtherElementType (*)[], element_type (*)[]>::value))
#endif
            >
...
{
  return detail::is_same<T1, T2>::value && l.size() == r.size() && static_cast<void const*>(l.data()) == r.data();
}
