...
GPUdii() bool GPUEC0FitterKernel::fitTrack(GPUEC0Fitter& GPUrestrict() Fitter, GPUTPCGMPropagator& GPUrestrict() prop, GPUEC0Track& GPUrestrict() track, int start, int end, int step)
{
  for (int iLayer{start}; iLayer != end; iLayer += step) {
    if (track.mClusters[iLayer] == o2::ecl::constants::ecl::UnusedIndex) {
      continue;
    }
...
template <>
GPUdii() void GPUEC0FitterKernel::Thread<0>(int nBlocks, int nThreads, int iBlock, int iThread, GPUsharedref() GPUSharedMemory& GPUrestrict() smem, processorType& GPUrestrict() processors)
{
  ...
  for (int iRoad = get_global_id(0); iRoad < Fitter.NumberOfRoads(); iRoad += get_global_size(0)) {
    ...
    int clusters[7] = {o2::ecl::constants::ecl::UnusedIndex, o2::ecl::constants::ecl::UnusedIndex, o2::ecl::constants::ecl::UnusedIndex, o2::ecl::constants::ecl::UnusedIndex, o2::ecl::constants::ecl::UnusedIndex, o2::ecl::constants::ecl::UnusedIndex, o2::ecl::constants::ecl::UnusedIndex};
    int lastCellLevel = o2::ecl::constants::ecl::UnusedIndex;
    CA_DEBUGGER(int nClusters = 2);

    for (int iCell{0}; iCell < o2::ecl::constants::ecl::CellsPerRoad; ++iCell) {
      const int cellIndex = road[iCell];
      if (cellIndex == o2::ecl::constants::ecl::UnusedIndex) {
        continue;
      } else {
    ...
    if (lastCellLevel == o2::ecl::constants::ecl::UnusedIndex) {
      continue;
    }

    /// From primary vertex context index to event index (== the one used as input of the tracking code)
    for (int iC{0}; iC < 7; iC++) {
      if (clusters[iC] != o2::ecl::constants::ecl::UnusedIndex) {
    ...
    GPUEC0Track temporaryTrack;
    {
      ...
      const float& tz = fy;
      const float cy = (math_utils::computeCurvature(x1, y1, x2, y2 + o2::ecl::constants::ecl::Resolution, x3, y3) - crv) / (o2::ecl::constants::ecl::Resolution * bz * constants::math::B2C) * 20.f; // FIXME: MS contribution to the cov[14] (*20 added)
      constexpr float s2 = o2::ecl::constants::ecl::Resolution * o2::ecl::constants::ecl::Resolution;
    ...
    bool fitSuccess = fitTrack(Fitter, prop, temporaryTrack, o2::ecl::constants::ecl::LayersNumber - 4, -1, -1);
    if (!fitSuccess) {
      continue;
    }
    CA_DEBUGGER(fitCounters[nClusters - 4]++);
    temporaryTrack.ResetCovariance();
    fitSuccess = fitTrack(Fitter, prop, temporaryTrack, 0, o2::ecl::constants::ecl::LayersNumber, 1);
    if (!fitSuccess) {
      continue;
    }
    ...
    temporaryTrack.ResetCovariance();
    fitSuccess = fitTrack(Fitter, prop, temporaryTrack, o2::ecl::constants::ecl::LayersNumber - 1, -1, -1);
    if (!fitSuccess) {
      continue;
    }
