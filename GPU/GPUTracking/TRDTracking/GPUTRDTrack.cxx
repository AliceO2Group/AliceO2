...
template <typename T>
GPUd() GPUTRDTrack_t<T>::GPUTRDTrack_t(const AliHLTExternalTrackParam &t) :
  T(t),
  mChi2(0.f),
  mMass(0.f),
  mLabel(-1),
...
template <typename T>
GPUd() GPUTRDTrack_t<T>::GPUTRDTrack_t(const T& t) :
  T(t),
  mChi2(0.f),
  mMass(0.f),
  mLabel(-1),
...
GPUd() void GPUTRDTrack_t<T>::ConvertFrom( const GPUTRDTrackDataRecord &t )
{
  ...
  for ( int iLayer=0; iLayer < kNLayers; iLayer++ ){
    ...
    if( mAttachedTracklets[iLayer]>=0 ) mNTracklets++;
  }
  for (int j=0; j<4; ++j) {
    mNTrackletsOffline[j] = 0;
  }
