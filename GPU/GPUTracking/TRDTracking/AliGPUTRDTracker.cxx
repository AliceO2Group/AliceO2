...
{
  mMemoryPermanent = mRec->RegisterMemoryAllocation(this, &AliGPUTRDTracker::SetPointersBase, AliGPUMemoryResource::MEMORY_PERMANENT, "TRDInitialize");
  mMemoryTracklets = mRec->RegisterMemoryAllocation(this, &AliGPUTRDTracker::SetPointersTracklets, AliGPUMemoryResource::MEMORY_INPUT, "TRDTracklets");
  ...
  mMemoryTracks = mRec->RegisterMemoryAllocation(this, &AliGPUTRDTracker::SetPointersTracks, type, "TRDTracks");
}

void AliGPUTRDTracker::InitializeProcessor()
{
  Init((TRD_GEOMETRY_CONST AliGPUTRDGeometry*) mChainTracking->GetTRDGeometry());
}
...
GPUd() bool AliGPUTRDTracker::CalculateSpacePoints()
{
  ...
  for (int iDet=0; iDet<kNChambers; ++iDet) {
    ...
    const AliGPUTRDpadPlane *pp = mGeo->GetPadPlane(iDet);
    ...
    for (int iTrklt=0; iTrklt<nTracklets; ++iTrklt) {
      ...
      int modId   = mGeo->GetSector(iDet) * AliGPUTRDGeometry::kNstack + mGeo->GetStack(iDet); // global TRD stack number
...
GPUd() void AliGPUTRDTracker::FindChambersInRoad(const GPUTRDTrack *t, const float roadY, const float roadZ, const int iLayer, int* det, const float zMax, const float alpha) const
{
  ...
  if (currStack > -1) {
    ...
    det[nDets++] = currDet;
    const AliGPUTRDpadPlane *pp = mGeo->GetPadPlane(iLayer, currStack);
...
GPUd() bool AliGPUTRDTracker::IsGeoFindable(const GPUTRDTrack *t, const int layer, const float alpha) const
{
  ...
  const AliGPUTRDpadPlane *pp = mGeo->GetPadPlane(det);
...
GPUd() void AliGPUTRDTracker::SwapTracklets(const int left, const int right)
{
  ...
  //--------------------------------------------------------------------
  AliGPUTRDTrackletWord tmp = mTracklets[left];
...
GPUd() int AliGPUTRDTracker::PartitionTracklets(const int left, const int right)
{
  ...
  AliGPUTRDTrackletWord pivot = mTracklets[mid];
