...
class AliHLTTPCCATrackParam
{
  public:
	...
	GPUd() bool TransportToXWithMaterial(float x, AliHLTTPCCATrackLinearisation &t0,
	                                     AliHLTTPCCATrackFitParam &par, float Bz, float maxSinPhi = GPUCA_MAX_SIN_PHI);
	...
	GPUd() bool Rotate(float alpha, AliHLTTPCCATrackLinearisation &t0, float maxSinPhi = GPUCA_MAX_SIN_PHI);
