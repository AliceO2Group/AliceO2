...
class AliHLTTPCCATracker
{
  public:
	...
	int ReadEvent( const AliHLTTPCCAClusterData *clusterData );
	...
	MEM_TEMPLATE() GPUd() cahit HitDataY( const MEM_TYPE(AliHLTTPCCARow) &row, int hitIndex) const { return fData.HitDataY(row, hitIndex); }
	MEM_TEMPLATE() GPUd() cahit HitDataZ( const MEM_TYPE(AliHLTTPCCARow) &row, int hitIndex) const { return fData.HitDataZ(row, hitIndex); }
	MEM_TEMPLATE() GPUd() cahit2 HitData( const MEM_TYPE(AliHLTTPCCARow) &row, int hitIndex) const { return fData.HitData(row, hitIndex); }
  ...
  private:
	...
	* merge clusters from inside the SliceTracker code and recreate the SliceData. */
	GPUglobalref() const AliHLTTPCCAClusterData *fClusterData; // ^
