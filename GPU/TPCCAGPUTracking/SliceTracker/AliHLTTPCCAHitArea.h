...
class AliHLTTPCCAHitArea
{
  public:
	MEM_TEMPLATE()
	GPUd() void Init(const MEM_TYPE(AliHLTTPCCARow) & row, GPUglobalref() const MEM_GLOBAL(AliHLTTPCCASliceData) & slice, float y, float z, float dy, float dz);
     ...
     */
	MEM_TEMPLATE()
	GPUd() int GetNext(GPUconstant() const MEM_CONSTANT(AliHLTTPCCATracker) & tracker, const MEM_TYPE(AliHLTTPCCARow) & row,
