...
MEM_CLASS_PRE() class AliHLTTPCCASliceData
{
  public:
	...
	MEM_TEMPLATE() GPUd() void SetHitLinkUpData  ( const MEM_TYPE(AliHLTTPCCARow) &row, const calink &hitIndex, const calink &value );
	MEM_TEMPLATE() GPUd() void SetHitLinkDownData( const MEM_TYPE(AliHLTTPCCARow) &row, const calink &hitIndex, const calink &value );
	/**
