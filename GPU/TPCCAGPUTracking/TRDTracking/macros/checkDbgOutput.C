...
Double_t AngleToDy(Double_t tanPhi)
{
  return fitTrackletDy->Eval(tanPhi);
}

Double_t DyToSnp(Double_t dy)
{
  return ((dy/3) / TMath::Sqrt( 1 + TMath::Power((dy/3), 2) ));
}

Double_t SnpToDy(Double_t snp)
{
  return (3. * snp / TMath::Sqrt( 1 - TMath::Power(snp, 2) ));
}

Double_t GetAngularResolution(Double_t tanPhi)
{
  return fitRMSAngle->Eval(tanPhi);
}
...
void FitAngularResolution()
{
  ...
  TGraph *gr = TStatToolkit::MakeGraphErrors(tree, "trackletDy.fElements:trackPhi.fElements", "isGold&&LoadBranches(Entry$)",
                                              25, 1, 1, 0, 100000);
  gr->Fit("pol2", "rob=0.9", "", -0.3, 0.3);
  tree->SetAlias("trackletDyFit", TString::Format("(%f)+(%f)*trackPhi.fElements+(%f)*trackPhi.fElements^2",
                                  gr->GetFunction("pol2")->GetParameter(0),
                                  gr->GetFunction("pol2")->GetParameter(1),
                                  gr->GetFunction("pol2")->GetParameter(2)));
  gr->Draw("ap");
  fitTrackletDy->SetParameters(gr->GetFunction("pol2")->GetParameter(0),
                               gr->GetFunction("pol2")->GetParameter(1),
                               gr->GetFunction("pol2")->GetParameter(2));
  tree->Draw("trackletDy.fElements-trackletDyFit:trackPhi.fElements>>hisDyPhi(20, -0.3, 0.3, 50, -0.3, 0.3)",
              "isGold", "colz");
      ...
      if ((*update)[iLy] < 1) {
        ...
        if ((*nMatchingTracklets)[iLy] > 0) {
          ...
          if ( (*trackSec)[iLy] == (*trackletSecReal)[iLy] && 
                TMath::Abs((*trackYreal)[iLy] - (*trackletYreal)[iLy]) < (*roadY)[iLy] &&
                TMath::Abs((*trackZreal)[iLy] - (*trackletZreal)[iLy]) < (*roadZ)[iLy] )
          {
    ...
    for (Int_t iLy = 1; iLy < 6; iLy++) {
      ...
      if ((*nMatchingTracklets)[iLy] > 0) {
        ...
        if ((*update)[iLy] > 0) {
          ...
          if ( TMath::Abs((*trackYreal)[iLy] - (*trackletYreal)[iLy]) < (*roadY)[iLy] &&
               TMath::Abs((*trackZreal)[iLy] - (*trackletZreal)[iLy]) < (*roadZ)[iLy] )
          {
