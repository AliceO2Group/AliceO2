...
int AliHLTTPCCAGPUTrackerNVCC::Reconstruct(AliHLTTPCCASliceOutput** pOutput, AliHLTTPCCAClusterData* pClusterData, int firstSlice, int sliceCountLocal)
{
	...
	for (int iSlice = 0;iSlice < sliceCountLocal;iSlice++)
	{
		...
		PreInitRowBlocks<<<fConstructorBlockCount, HLTCA_GPU_THREAD_COUNT, 0, cudaStreams[HLTCA_GPU_NUM_STREAMS == 0 ? 2 : useStream]>>>(fGpuTracker[iSlice].RowBlockPos(), fGpuTracker[iSlice].RowBlockTracklets(), fGpuTracker[iSlice].Data().HitWeights(), fSlaveTrackers[firstSlice + iSlice].Data().NumberOfHitsPlusAlign());
...
int AliHLTTPCCAGPUTrackerNVCC::RefitMergedTracks(AliHLTTPCGMMerger* Merger)
{
	...
	if (fDebugLevel > 0)
	{
		...
		double speed = (double) copysize / times[0] / 1e9;
		printf("GPU Fit:\tCopy To:\t%1.0f us (%lf GB/s)\n", times[0] * 1000000, speed);
		printf("\t\tFit:\t%1.0f us\n", times[1] * 1000000);
		speed = (double) copysize / times[2] / 1e9;
		printf("\t\tCopy From:\t%1.0f us (%lf GB/s)\n", times[2] * 1000000, speed);
	}
