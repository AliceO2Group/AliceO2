...
struct AliGPUTPCGMOfflineStatisticalErrors
{
	void SetCurCluster(AliGPUTPCGMMergedTrackHit* c) {fCurCluster = c;}
	...
	void GetOfflineStatisticalErrors(float& err2Y, float& err2Z, float sinPhi, float dzds, unsigned char clusterState) const
	{
		...
		int type = 0;
		if (clusterState & AliGPUTPCGMMergedTrackHit::flagSplit) type = 50;
		if (clusterState & AliGPUTPCGMMergedTrackHit::flagEdge) type = -type - 3;
		cl.SetType(type);
		cl.SetSigmaY2(0.5);
		cl.SetSigmaZ2(0.5);
	...
	AliGPUTPCGMMergedTrackHit* fCurCluster;
};
#else
struct AliGPUTPCGMOfflineStatisticalErrors
{
	GPUd() void SetCurCluster(AliGPUTPCGMMergedTrackHit* /*c*/) {}
