 ...
 *
 * The class describes TPC slice tracks used in AliGPUTPCGMMerger
 */
class AliGPUTPCGMSliceTrack
{
  ...
  public:
	...
	const AliGPUTPCSliceOutTrack *OrigTrack() const { return fOrigTrack; }
	...
	{
		const AliGPUTPCBaseTrackParam &t = sliceTr->Param();
		fOrigTrack = sliceTr;
		fX = t.GetX();
		fY = t.GetY();
		fZ = t.GetZ();
	...
	void CopyParamFrom(const AliGPUTPCGMSliceTrack &t)
	{
		fX = t.fX;
		fY = t.fY;
		fZ = t.fZ;
	...
	bool FilterErrors(const AliGPUCAParam &param, float maxSinPhi = GPUCA_MAX_SIN_PHI, float sinPhiMargin = 0.f);

	bool TransportToX(float x, float Bz, AliGPUTPCGMBorderTrack &b, float maxSinPhi, bool doCov = true) const;

	bool TransportToXAlpha(float x, float sinAlpha, float cosAlpha, float Bz, AliGPUTPCGMBorderTrack &b, float maxSinPhi) const;

  private:
	const AliGPUTPCSliceOutTrack *fOrigTrack;               // pointer to original slice track
