...
void AliGPUTPCGMTracksToTPCSeeds::CreateSeedsFromHLTTracks(TObjArray* seeds, AliTPCtracker* tpctracker)
{
	const AliGPUTPCGMMerger* merger = AliGPUTPCGlobalMergerComponent::GetCurrentMerger();
...
void AliGPUTPCGMTracksToTPCSeeds::UpdateParamsOuter(TObjArray* seeds)
{
	const AliGPUTPCGMMerger* merger = AliGPUTPCGlobalMergerComponent::GetCurrentMerger();
	if (merger == NULL) return;
	int index = 0;
	for (int i = 0;i < merger->NOutputTracks();i++)
	{
		...
		const AliGPUTPCGMTrackParam::AliGPUTPCGMTrackParam::AliGPUTPCOuterParam& param = track.OuterParam();
		seed->Set(param.fX, param.fAlpha, param.fP, param.fC);
	}
}

void AliGPUTPCGMTracksToTPCSeeds::UpdateParamsInner(TObjArray* seeds)
{
	const AliGPUTPCGMMerger* merger = AliGPUTPCGlobalMergerComponent::GetCurrentMerger();
	if (merger == NULL) return;
	int index = 0;
