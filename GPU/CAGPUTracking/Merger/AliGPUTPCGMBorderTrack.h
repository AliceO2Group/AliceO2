...
class AliGPUTPCGMBorderTrack
{
  ...
  public:
	...
	bool CheckChi2Y(const AliGPUTPCGMBorderTrack &t, float chi2cut) const
	{
		float d = fP[0] - t.fP[0];
		return (d * d < chi2cut * (fC[0] + t.fC[0]));
	}

	bool CheckChi2Z(const AliGPUTPCGMBorderTrack &t, float chi2cut) const
	{
		float d = fP[1] - t.fP[1] + (fZOffset - t.fZOffset);
		return (d * d < chi2cut * (fC[1] + t.fC[1]));
	}

	bool CheckChi2QPt(const AliGPUTPCGMBorderTrack &t, float chi2cut) const
	{
		float d = fP[4] - t.fP[4];
		return (d * d < chi2cut * (fC[4] + t.fC[4]));
	}

	bool CheckChi2YS(const AliGPUTPCGMBorderTrack &t, float chi2cut) const
	{
		return CheckChi2(fP[0], fP[2], fC[0], fD[0], fC[2],
		                 t.fP[0], t.fP[2], t.fC[0], t.fD[0], t.fC[2], chi2cut);
	}

	bool CheckChi2ZT(const AliGPUTPCGMBorderTrack &t, float chi2cut) const
	{
