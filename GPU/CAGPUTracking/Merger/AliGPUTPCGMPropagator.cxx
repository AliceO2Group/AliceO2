// $Id: AliGPUTPCGMPropagator.cxx 41769 2010-06-16 13:58:00Z sgorbuno $
// **************************************************************************
...
GPUd()void AliGPUTPCGMPropagator::GetBxByBz(float Alpha, float X, float Y, float Z, float B[3]) const
{
...
GPUd() float AliGPUTPCGMPropagator::GetBz(float Alpha, float X, float Y, float Z) const
{
	if (fToyMCEvents)
	{ // special treatment for toy monte carlo
		float B[3];
...
GPUd() int AliGPUTPCGMPropagator::RotateToAlpha(float newAlpha)
{
	//
	...
	AliGPUTPCGMPhysicalTrackModel t0 = fT0;
	...
	if (t0.SetDirectionAlongX())
	{ // change direction if Px < 0
		fT->SinPhi() = -fT->SinPhi();
		fT->DzDs() = -fT->DzDs();
		fT->QPt() = -fT->QPt();
...
GPUd() int AliGPUTPCGMPropagator::PropagateToXAlpha(float posX, float posAlpha, bool inFlyDirection)
{
	...
	AliGPUTPCGMPhysicalTrackModel t0e(fT0);
	float dLp = 0;
...
GPUd() int AliGPUTPCGMPropagator::GetPropagatedYZ(float x, float &projY, float &projZ)
{
...
GPUd() void AliGPUTPCGMPropagator::GetErr2(float& err2Y, float& err2Z, const AliGPUCAParam &param, float posZ, int iRow, short clusterState) const
{
...
GPUd()float AliGPUTPCGMPropagator::PredictChi2(float posY, float posZ, int iRow, const AliGPUCAParam &param, short clusterState) const
{
	float err2Y, err2Z;
...
GPUd() float AliGPUTPCGMPropagator::PredictChi2(float posY, float posZ, float err2Y, float err2Z) const
{
...
GPUd() int AliGPUTPCGMPropagator::Update(float posY, float posZ, int iRow, const AliGPUCAParam &param, short clusterState, bool rejectChi2, bool refit)
{
	float err2Y, err2Z;
	GetErr2(err2Y, err2Z, param, posZ, iRow, clusterState);
...
GPUd() int AliGPUTPCGMPropagator::Update(float posY, float posZ, short clusterState, bool rejectChi2, float err2Y, float err2Z)
{
	float *fC = fT->Cov();
	float *fP = fT->Par();

	float d00 = fC[0], d01 = fC[1], d02 = fC[3], d03 = fC[6], d04 = fC[10];
	float d10 = fC[1], d11 = fC[2], d12 = fC[4], d13 = fC[7], d14 = fC[11];
...
GPUd() float AliGPUTPCGMPropagator::ApproximateBetheBloch(float beta2)
{
	//------------------------------------------------------------------
...
GPUd() void AliGPUTPCGMPropagator::CalculateMaterialCorrection()
{
	//*!
...
GPUd() void AliGPUTPCGMPropagator::Rotate180()
{
	fT0.X() = -fT0.X();
	fT0.Y() = -fT0.Y();
	fT0.Q() = -fT0.Q();
...
GPUd() void AliGPUTPCGMPropagator::ChangeDirection()
{
	fT0.Py() = -fT0.Py();
	fT0.Pz() = -fT0.Pz();
	fT0.Q() = -fT0.Q();
...
GPUd() void AliGPUTPCGMPropagator::Mirror(bool inFlyDirection)
{
