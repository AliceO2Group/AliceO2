 ...
 public:
  ...
  void run(framework::ProcessingContext& pc)
  {
    ...
    for (uint32_t iLayer = 0; iLayer < 7; ++iLayer) {
      ...
      const int64_t bcShift = mDigitizer.getParams().getROFrameBiasInBC(iLayer); // this accounts the misalignment and the opt. imposed rof delay
      ...
      for (int collID = 0; collID < timesview.size(); ++collID) {
        ...
        for (const auto& part : eventParts[collID]) {
          ...
          if (mHits.size() > 0) {
            ...
            mDigitizer.process(&mHits, part.entryID, part.sourceID, iLayer); // call actual digitization procedure
          }
        }
        accumulate();
      }
      ...
      if (nROFsTF != mROFRecordsAccum[iLayer].size()) {
        ...
        for (const auto& rof : mROFRecordsAccum[iLayer]) {
          ...
          if (ir < firstIR) {
            LOGP(warn, "Discard ROF {} preceding TF 1st orbit {} on layer {}", ir.asString(), mFirstOrbitTF, iLayer);
            continue;
          }
          auto irToFirst = ir - firstIR;
          if (irToFirst.toLong() - par.getROFDelayInBC(iLayer) < 0) {
            LOGP(warn, "Discard ROF {} preceding TF 1st orbit {} due to imposed ROF delay on layer {}", ir.asString(), mFirstOrbitTF, iLayer);
            continue;
          }
...
DataProcessorSpec getITS3DigitizerSpec(int channel, bool mctruth)
{
  ...
  return DataProcessorSpec{.name = "IT3Digitizer",
                           .inputs = inputs,
                           .outputs = makeOutChannels(detOrig, mctruth),
