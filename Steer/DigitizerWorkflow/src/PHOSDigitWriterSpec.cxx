...
/// @brief  Processor spec for a ROOT file writer for PHOS digits
...
/// describing a processor receiving digits for PHOS writing them to file
DataProcessorSpec getPHOSDigitWriterSpec()
{
  auto initFunction = [](InitContext& ic) {
    // get the option from the init context
    auto filename = ic.options().get<std::string>("phos-digit-outfile");
    ...
    auto processingFct = [outputfile, outputtree, digits, labels](ProcessingContext& pc) {
      ...
      auto indata = pc.inputs().get<std::vector<o2::phos::Digit>>("phosdigits");
      LOG(INFO) << "RECEIVED DIGITS SIZE " << indata.size();
      *digits.get() = std::move(indata);

      // connect this to a particular branch
      auto br = getOrMakeBranch(*outputtree.get(), "PHOSDigit", digits.get());
      br->Fill();
      ...
      // connect this to a particular branch
      auto labelbr = getOrMakeBranch(*outputtree.get(), "PHOSDigitMCTruth", &labeldataraw);
      labelbr->Fill();
