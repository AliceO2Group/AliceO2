...
void O2HitMerger::InitTask()
{
  ...
  if (o2::querySimConfig(GetChannels().at("o2sim-primserv-info").at(0))) {
...
bool O2HitMerger::setWorkingDirectory(std::string const& dir)
{
...
bool O2HitMerger::ReInit(o2::conf::SimReconfigData const& reconfig)
{
  if (reconfig.stop) {
    return false;
  }
...
V O2HitMerger::insertAdd(std::map<T, V>& m, T const& key, V value)
{
...
template <typename T>
bool O2HitMerger::isDataComplete(T checksum, T nparts)
{
  return checksum == nparts * (nparts + 1) / 2;
}

void O2HitMerger::consumeHits(int eventID, fair::mq::Parts& data, int& index)
{
...
void O2HitMerger::consumeData(int eventID, fair::mq::Parts& data, int& index, BT& buffer)
{
...
void O2HitMerger::fillSubEventInfoEntry(o2::data::SubEventInfo& info)
{
...
bool O2HitMerger::waitForControlInput()
{
...
bool O2HitMerger::ConditionalRun()
{
...
bool O2HitMerger::handleSimData(fair::mq::Parts& data, int /*index*/)
{
...
void O2HitMerger::cleanEvent(int eventID)
{
...
template <typename T>
void O2HitMerger::backInsert(T const& from, T& to)
{
  std::copy(from.begin(), from.end(), std::back_inserter(to));
}

void O2HitMerger::reorderAndMergeMCTracks(int eventID, TTree* target, const std::vector<int>& nprimaries, const std::vector<int>& nsubevents, std::function<void(std::vector<MCTrack> const&)> tracks_analysis_hook, o2::dataformats::MCEventHeader const* mceventheader)
{
...
void O2HitMerger::remapTrackIdsAndMerge(std::string brname, int eventID, TTree& target, const std::vector<int>& trackoffsets, const std::vector<int>& nprimaries, const std::vector<int>& subevOrdered, M& mapOfVectorOfTs)
{
  //
...
void O2HitMerger::updateTrackIdWithOffset(MCTrack& track, Int_t nprim, Int_t idelta0, Int_t idelta1)
{
  Int_t cId = track.getMotherTrackId();
...
void O2HitMerger::updateTrackIdWithOffset(TrackReference& ref, Int_t nprim, Int_t idelta0, Int_t idelta1)
{
  ref.setTrackID(o2::base::Detector::offsetTrackIndex(ref.getTrackID(), nprim, idelta0, idelta1));
}

void O2HitMerger::initHitTreeAndOutFile(std::string prefix, int detID)
{
...
bool O2HitMerger::mergeAndFlushData()
{
