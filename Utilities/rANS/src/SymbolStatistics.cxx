...
void SymbolStatistics::rescaleToNBits(std::size_t bits)
{
  const std::size_t newCumulatedFrequency = bitsToRange(bits);
  assert(newCumulatedFrequency >= mFrequencyTable.size());

  std::size_t cumulatedFrequencies = mCumulativeFrequencyTable.back();

  // resample distribution based on cumulative frequencies_
  for (std::size_t i = 1; i <= mFrequencyTable.size(); i++)
  ...
  // mind.
  for (std::size_t i = 0; i < mFrequencyTable.size(); i++) {
    if (mFrequencyTable[i] &&
      ...
      // ones)
      std::pair<std::size_t, std::size_t> stealFromEntry{mFrequencyTable.size(), ~0u};
      for (std::size_t j = 0; j < mFrequencyTable.size(); j++) {
        uint32_t frequency =
      ...
      if (stealFromEntry.first < i) {
        for (std::size_t j = stealFromEntry.first + 1; j <= i; j++)
          mCumulativeFrequencyTable[j]--;
      } else {
        assert(stealFromEntry.first > i);
        for (std::size_t j = i + 1; j <= stealFromEntry.first; j++)
          mCumulativeFrequencyTable[j]++;
      }
    }
  }
...
std::size_t SymbolStatistics::getAlphabetSize() const { return mFrequencyTable.size(); }

std::size_t SymbolStatistics::getAlphabetRangeBits() const
{
  return std::max(std::ceil(std::log2(mMax - mMin)), 1.0);
}

std::size_t SymbolStatistics::getNUsedAlphabetSymbols() const
{
  return mNUsedAlphabetSymbols;
}

std::size_t SymbolStatistics::getMessageLength() const
{
  return mMessageLength;
}

std::pair<uint32_t, uint32_t> SymbolStatistics::operator[](std::size_t index) const
{
...
SymbolStatistics::Iterator::Iterator(std::size_t index,
