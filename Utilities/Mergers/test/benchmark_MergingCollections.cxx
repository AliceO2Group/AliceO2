...
static void BM_mergingCollectionsTHNSparse(benchmark::State& state)
{
  ...
  for (auto _ : state) {
    ...
    for (std::size_t i = 0; i < collectionSize; i++) {
      ...
      for (std::size_t entry = 0; entry < 50000; entry++) {
...
static void BM_mergingPODCollections(benchmark::State& state)
{
  ...
  const std::size_t randoms = 50000;
  TRandomMT64 gen;
  gen.SetSeed(std::time(nullptr));
...
static void BM_mergingBoostRegular1DCollections(benchmark::State& state)
{
  ...
  const std::size_t collectionSize = state.range(0);
...
static void BM_mergingBoostRegular2DCollections(benchmark::State& state)
{
  ...
  const double max = 1000000.0;
  const std::size_t collectionSize = state.range(0);
  const std::size_t bins = 250; // 250 bins * 250 bins * 4B makes 250kB
  ...
  for (std::size_t i = 0; i < collectionSize; i++) {
    ...
    for (std::size_t r = 0; r < randoms; r++) {
