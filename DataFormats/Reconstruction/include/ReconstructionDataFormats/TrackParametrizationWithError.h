...
{ // track+error parameterization
 public:
  ...
  GPUd() bool rotateWithRef(value_t alpha, TrackParametrization<value_T>& linRef, value_t bz);
  GPUd() bool propagateTo(value_t xk, value_t bz);
  GPUd() bool propagateToWithRef(value_t xk, TrackParametrization<value_T>& linRef0, value_t bz);
