...
void Cluster::coG()
{
  ...
  if(mDigs.size() == 0) return; //no digits in this cluster
  ...
  if(iflag==2) {
    ...
    for(int i=0; i<nPads; i++) { //loop on all pads of the cluster
      for(int j=0; j<iNshape; j++) { //Mathiesons loop as all of them may contribute to this pad
        ...
        derivPart[3*j][i] += par[3*j+2] * (o2::hmpid::Digit::mathiesonX(par[3*j] - lx - 0.5 * o2::hmpid::Param::sizePadX())-
                                           o2::hmpid::Digit::mathiesonX(par[3*j] - lx + 0.5 * o2::hmpid::Param::sizePadX()))*
                                           o2::hmpid::Digit::intPartMathiY(par[3*j+1], pClu->dig(i)->getPadID());
        derivPart[3*j+1][i] += par[3*j+2]*(o2::hmpid::Digit::mathiesonY(par[3*j+1] - ly - 0.5 * o2::hmpid::Param::sizePadY())-
                                           o2::hmpid::Digit::mathiesonY(par[3*j+1] - ly + 0.5 * o2::hmpid::Param::sizePadY()))*
...
Bool_t Cluster::isInPc()
{
  ...
  if ( mXX < Param::minPcX(pc) || mXX > Param::maxPcX(pc) || 
       mYY < Param::minPcY(pc) || mYY > Param::maxPcY(pc) ) return false;
