...
GlobalOffsetContainer::GlobalOffsetContainer(std::size_t minEntries)
...
bool GlobalOffsetContainer::hasEnoughEntries() const
{
  return *std::min_element(mEntriesCollTime.begin(), mEntriesCollTime.end()) > mMinEntries;
}
void GlobalOffsetContainer::fill(const gsl::span<const GlobalOffsetsCalibrationObject>& data)
{
...
void GlobalOffsetContainer::merge(GlobalOffsetContainer* prev)
{
  mHistogram += prev->mHistogram;
}

int16_t GlobalOffsetContainer::getMeanGaussianFitValue(std::size_t channelID) const
{
...
void GlobalOffsetContainer::print() const
{
