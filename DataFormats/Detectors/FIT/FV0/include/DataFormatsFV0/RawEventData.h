...
namespace fv0
{
constexpr int N_FV0_CHANNELS = 48;
constexpr int N_PM_CHANNELS = 12;
constexpr int N_PMS = 5;
...
class RawEventData
{
 public:
  ...
  std::vector<char> to_vector(bool tcm)
  {
    ...
    if (!tcm) {
      ...
      LOG(DEBUG) << "write header words " << (int)mEventHeader.nGBTWords << " orbit " << int(mEventHeader.orbit)
                 << " bc " << int(mEventHeader.bc) << " out " << result.size();
      if (mIsPadded) {
      ...
      for (int i = 0; i < mEventHeader.nGBTWords; ++i) {
        ...
        LOG(DEBUG) << " 1st word " << mEventData[2 * i].channelID << " charge " << mEventData[2 * i].charge
                   << " time " << mEventData[2 * i].time << " out " << result.size();
        ...
        LOG(DEBUG) << " 2nd word " << mEventData[2 * i + 1].channelID << " charge " << mEventData[2 * i + 1].charge
                   << " time " << mEventData[2 * i + 1].time << " out " << result.size();
        if (mIsPadded) {
      ...
      std::memcpy(out, &mEventHeader, EventHeader::PayloadSize);
      out += EventHeader::PayloadSize;
      LOG(DEBUG) << "write TCM header words " << (int)mEventHeader.nGBTWords << " orbit " << int(mEventHeader.orbit)
                 << " bc " << int(mEventHeader.bc) << " out " << result.size();
      std::memcpy(out, &mTCMdata, sizeof(TCMdata));
      out += sizeof(TCMdata);
      LOG(DEBUG) << "write TCM words " << sizeof(mTCMdata) << " orbit " << int(mEventHeader.orbit) << " bc "
                 << int(mEventHeader.bc) << " out " << result.size() << " sum time A " << mTCMdata.timeA;
    }
 ...
 public:
  ...
  EventData mEventData[N_PM_CHANNELS];
  TCMdata mTCMdata;
  bool mIsPadded = true;
  /////////////////////////////////////////////////
...
class DataPageWriter
{
  ...
  static constexpr int mMaxPageSize = 8192;
 ...
 public:
  ...
  {
    if (mBuffer.size() + new_data.size() + mRDH.headerSize > mMaxPageSize) {
