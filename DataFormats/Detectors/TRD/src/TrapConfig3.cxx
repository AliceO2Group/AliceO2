...
TrapConfig3::TrapConfig3()
{
  initialiseRegisters();
  setConfigSavedVersion(2);
}

void TrapConfig3::initialiseRegisters()
{
...
uint32_t TrapConfig3::getRegisterValue(const uint32_t regidx, const int mcmidx)
{
  ...
  int regoffset = regbase + trapreg->getDataWordNumber();        // get the offset to the register in question
...
bool TrapConfig3::setRegisterValue(uint32_t data, uint32_t regidx, int mcmidx)
{
  if (regidx < 0 || regidx >= kLastReg || mcmidx < 0 || mcmidx >= o2::trd::constants::MAXMCMCOUNT) {
    return false;
  }
...
uint32_t TrapConfig3::getRegisterValueByIdx(uint32_t regidx, int mcmidx)
{
  return getRegisterValue(regidx, mcmidx);
}

bool TrapConfig3::setRegisterValueByIdx(uint32_t data, uint32_t regidx, int mcmidx)
{
  return setRegisterValue(data, regidx, mcmidx);
}

uint32_t TrapConfig3::getRegisterValueByAddr(uint32_t addr, int mcmidx)
{
  int index = getRegIndexByAddr(addr);
  return getRegisterValueByIdx(index, mcmidx);
}

bool TrapConfig3::setRegisterValueByAddr(uint32_t data, uint32_t addr, int mcmidx)
{
  int index = getRegIndexByAddr(addr);
  return setRegisterValueByIdx(data, index, mcmidx);
}

uint32_t TrapConfig3::getRegisterValueByName(const std::string& regname, int mcmidx)
{
  int index = getRegIndexByName(regname);
  return getRegisterValueByIdx(index, mcmidx);
}

bool TrapConfig3::setRegisterValueByName(uint32_t data, const std::string& regname, int mcmidx)
{
  int index = getRegIndexByName(regname);
  if (index < 0) {
    return false;
...
uint32_t TrapConfig3::getRegisterValueByIdx(uint32_t regidx, int detector, int rob, int mcm)
{
...
int32_t TrapConfig3::getRegIndexByName(const std::string& name)
{
...
int32_t TrapConfig3::getRegAddrByIdx(unsigned int regidx)
{
  if ((regidx < 0) && (regidx > kLastReg)) {
    return -1;
  }
...
int32_t TrapConfig3::getRegAddrByName(const std::string& name)
{
  // there is no index for this but its not used online
  for (auto& reg : mTrapRegisters) {
    if (reg.getName() == name) {
...
o2::trd::TrapRegInfo* TrapConfig3::getTrapRegInfoByIdx(uint32_t idx)
{
  if ((idx < 0) && (idx > kLastReg)) {
    return nullptr;
  }
  return &mTrapRegisters[idx];
}
...
void TrapConfig3::getAllMCMByName(std::string registername, std::array<uint32_t, o2::trd::constants::MAXMCMCOUNT>& mcms) // return all the mcm values for a particular register name
{
...
void TrapConfig3::getAllMCMByAddress(int registeraddress, std::array<uint32_t, o2::trd::constants::MAXMCMCOUNT>& mcms)
{
...
bool TrapConfig3::printRegister(int addr, int det, int rob, int mcm)
{
  // print the value stored at the given address in the MCM chip
  TrapRegInfo* reg = getTrapRegInfoByAddr(addr);
  return printRegister(reg, det, rob, mcm);
}

uint32_t TrapConfig3::getDmemUnsigned(uint32_t address, int detector, int rob, int mcm)
{
  uint32_t data = 0;
  uint32_t mcmidx;
...
uint32_t TrapConfig3::getTrapReg(uint32_t index, int detector, int rob, int mcm)
{
...
{
  LOGP(info, " TrapReg : {} with nbits={} addr {:08x} mask {:04x} word number {} and baseword {} max {} ", getName(), getNbits(), getAddr(), getMask(), getWordNumber(), getBase(), getMax());
}
