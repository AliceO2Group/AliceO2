...
std::vector<TrapRegInfo*> registersOfInterest; // store of the registers we want to look at.
...
void trapregCheck(std::unique_ptr<TrapConfig3>& trapconfig, uint32_t mcmidx, int mcmidxcount)
{
  ...
  for (const auto& [key, value] : registervalues) {
    ...
    auto trapreg = trapconfig->getTrapRegInfoByIdx(index);
    if (trapconfig->setRegisterValueByIdx(value, index, mcmidx)) {
      ...
      BOOST_CHECK_EQUAL(trapconfig->getRegisterValueByIdx(index, mcmfullindex[mcmidxcount].mSector, mcmfullindex[mcmidxcount].mStack, mcmfullindex[mcmidxcount].mLayer, mcmfullindex[mcmidxcount].mRob, mcmfullindex[mcmidxcount].mMcm), value);
      BOOST_CHECK_EQUAL(trapconfig->getRegisterValueByAddr(address, mcmfullindex[mcmidxcount].mSector, mcmfullindex[mcmidxcount].mStack, mcmfullindex[mcmidxcount].mLayer, mcmfullindex[mcmidxcount].mRob, mcmfullindex[mcmidxcount].mMcm), value);
...
BOOST_AUTO_TEST_CASE(TRDTrapConfig3GetSet)
{
  ...
  std::unique_ptr<TrapConfig3> trapconfig(new TrapConfig3());
  ...
  std::unique_ptr<std::array<uint32_t, constants::MAXMCMCOUNT * kLastReg>> allregisters(new std::array<uint32_t, constants::MAXMCMCOUNT * kLastReg>()); // all data
  ...
  std::unique_ptr<std::array<uint32_t, kLastReg>> allmcmregisters(new std::array<uint32_t, kLastReg>());                                                // data for a single mcm, all registers.
    ...
    for (auto& reg : registersOfInterest) {
      ...
      for (auto& mcmidx : mcmids) {
        ...
        if (mcmidx < constants::MAXMCMCOUNT) {
          ...
          BOOST_CHECK_EQUAL(allregisters->at(mcmidx * kLastReg + regidx), value); // do the big one at the same time as the little one, pointless repeating the loop seperately
        }
      }
