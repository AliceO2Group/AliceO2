...
class TrackITS : public o2::track::TrackParCov
{
 ...
 public:
  ...
  uint32_t getLastClusterLayer() const { uint32_t r{0}, v{mPattern & ((1 << 16) - 1)}; while (v >>= 1) { r++; } return r; }
  uint32_t getFirstClusterLayer() const { int s{0}; while (!(mPattern & (1 << s))) { s++; } return s; }
